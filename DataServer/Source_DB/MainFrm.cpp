...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	wsprintf(szText, "Data Server Port: %d Version: %s Compiled Date: %s", DataApp->GetServerPort(), szVersion, DATASERVER_DATE);
	...
	{
		MsgBox("Item File not found. %s", szKorItemTextFileName);
	}
...
BOOL CMainFrame::ServerCreate()
{
	...
	{
		MsgBox("Mu Online Data Base Id Unknown.");
	}
	else
	{
		CString szTitle;
	...
	{
		MsgBox("4.0 VER_CHATWINDOW_OPTION");
	}
...
void CMainFrame::OnClose()
{
	...
	for(n = 0; n < MAX_SERVEROBJECT; n++)
	{
		...
		{
			LogAdd(" %s GameServer Connecting... ", &gSObj[n].ServerCode);
			count++;
		}
	}
	...
	if ( AfxMessageBox("Are you sure to Close?", 36, 0) == 7 )
	{
		return;
	}
