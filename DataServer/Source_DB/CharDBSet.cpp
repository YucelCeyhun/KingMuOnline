...
{
	CHARACTER_INFO_STRUCT tCharInfo;
	BYTE defclass;
	int n;
...
BOOL CCharDBSet::DefaultCreateCharacterInsert(LPCHARACTER_INFO_STRUCT lpObj)
{
	CString qSql;
...
BYTE CCharDBSet::SaveCharacter(char* Name,LPCHARACTER_INFO_STRUCT lpObj)
{
	CString qSql;
	...
	qSql.Format(chBuffer, lpObj->Level, lpObj->Class, lpObj->LevelUpPoint, lpObj->Experience, lpObj->Strength, lpObj->Dexterity, lpObj->Vitality, lpObj->Energy, lpObj->Money, lpObj->Life, lpObj->MaxLife, lpObj->Mana, 
		lpObj->MaxMana, lpObj->MapNumber, lpObj->MapX, lpObj->MapY, lpObj->Dir, lpObj->PkCount, lpObj->PkLevel, lpObj->PkTime, lpObj->Leadership, lpObj->ChatLitmitTime, lpObj->FruitPoint, /*lpObj->Reset, lpObj->GrandReset, */Name);
...
BOOL CCharDBSet::GetCharacter(char* szAccountID, char* Name,LPCHARACTER_INFO_STRUCT lpObj)
{
	CString qSql;
	...
	{
		lpObj->FruitPoint = 0;
		LogAddC(LOGC_RED,"[Fruit System][%s][%s] Fruit number %d -> 0 value setted",lpObj->AccountID,Name,iFruitPoint);
	}
	else
	{
	...
	/*
	int iReset = m_DBQuery.GetInt("Resets");

	if(iReset < 0)
	{
		lpObj->Reset = 0;
		LogAddC(LOGC_RED,"[Reset System][%s][%s] Reset number %d -> 0 value setted",lpObj->AccountID,Name,iReset);
	}
	else
	{
		lpObj->Reset = iReset;
	...
	int iGrandReset = m_DBQuery.GetInt("GrandResets");

	if(iGrandReset < 0)
	{
		lpObj->GrandReset = 0;
		LogAddC(LOGC_RED,"[Grand Reset System][%s][%s] Grand Reset number %d -> 0 value setted",lpObj->AccountID,Name,iGrandReset);
	}
	else
	{
		lpObj->GrandReset = iGrandReset;
	}
	*/
