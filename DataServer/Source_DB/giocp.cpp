...
DWORD WINAPI ServerWorkerThread(LPVOID CompletionPortID)
{
	...
	while(TRUE)
	{
		...
		{
			EnterCriticalSection(&criti);
			LogAdd( "GetQueuedCompletionStatus failed with error %d", GetLastError() );
			CloseClient(lpPerSocketContext, FALSE);
			LeaveCriticalSection(&criti);
			continue;
		}
	...
	while(1)
	{
		...
		{	// recv 데이터가 아직 완성되지 않았다면..
			if( lOfs > 0 )  // 하나이상 데이터를 보낸 후라면..
			{
				...
				{
					LogAdd("error : recvbuflen 이 1보다 작다..");
					break;
				}
				else 
				{
...
{	
	...
	{	
		...
		else
		{
			...
			//LogAdd("세컨드 복사.. %d+=%d (%s)", lpIoCtxt->nSecondOfs, dwSize, gSObj[aIndex].AccountID);
			LeaveCriticalSection(&criti);
			return TRUE;
		}
	}
	else
	{
		...
		else
		{
			...
			//LogAdd("메시지 복사.. %d", lpIoCtxt->nTotalBytes);
		}
	}
	// 쓰기작업이 완료되었다면..
...
//	LogAddTD("error-L2:CloseClient %d (강제종료??)", index);
