...
{
	this->Port = GetPrivateProfileInt("General", "TCPPort",44405,CONFIGFILE ) ;
	this->UDPPort = GetPrivateProfileInt("General", "UDPPort",55557,CONFIGFILE ) ;
	this->_MaxConnections = GetPrivateProfileInt("General", "MaxConnections",300,CONFIGFILE ) ;
	this->ShowAllServers = GetPrivateProfileInt("General", "ShowAllServers",1,CONFIGFILE ) ;
	this->Encrypt = GetPrivateProfileInt("General", "EncryptKey",0,CONFIGFILE ) ;
	this->DisconnectAfter = GetPrivateProfileInt("AntiFlood", "DisconnectAfter",60,CONFIGFILE ) ;
	this->MaxSendListCount = GetPrivateProfileInt("AntiFlood", "MaxSendListCount",10,CONFIGFILE ) ;
	this->AutoBLCount = GetPrivateProfileInt("AntiFlood", "BlackListAutoLearn",2,CONFIGFILE ) ;
...
void ServerManager::ReadUpdateConfig()
{
	...
	GetPrivateProfileString("FtpServerInfo", "Address", "none", this->FTP.Address, 200, CONFIGFILE);
	this->FTP.Port = GetPrivateProfileInt("FtpServerInfo", "Port", 21, CONFIGFILE);
	GetPrivateProfileString("FtpServerInfo", "ID", "", this->FTP.ID, 20, CONFIGFILE);
	GetPrivateProfileString("FtpServerInfo", "PASS", "", this->FTP.Password, 20, CONFIGFILE);
	GetPrivateProfileString("GameServerInfo", "VersionFileName", "version.wvd", this->FTP.Folder, 20, CONFIGFILE);

	GetPrivateProfileString("HttpServerInfo", "HttpAddress", "none", this->http.Address, 200, CONFIGFILE);
	this->http.Port = GetPrivateProfileInt("HttpServerInfo", "Port", 21, CONFIGFILE);
	GetPrivateProfileString("HttpServerInfo", "ID", "", this->http.ID, 20, CONFIGFILE);
	GetPrivateProfileString("HttpServerInfo", "PASS", "", this->http.Password, 20, CONFIGFILE);
	GetPrivateProfileString("GameServerInfo", "VersionFileName", "version.wvd", this->http.Folder, 20, CONFIGFILE);

	GetPrivateProfileString("GameServerInfo", "ClientVersion", "00.00", ClientVersion, 10, CONFIGFILE);
...
void ServerManager::ReadList(LPSTR filename)
{
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			this->Server[index][subindex].State = TRUE;
		}
	}
