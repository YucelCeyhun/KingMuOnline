			...
			case 0xF1:
				{
					...
					switch ( lpMsg2->subcode )
					{
						case 0x01:
							if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
							{
								CSPJoinIdPassRequest((PMSG_IDPASS *)aRecv, aIndex); //password
							}
							break;
						case 0x02:
							if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			...
			case 0xF3:
				{
					...
					switch ( lpDef->subcode )
					{
						...
						case 0x30:
							...
							break;
						case 0x52: //Season3 Add-on
							g_MasterSkillSystem.CGMasterSkillSend((PMSG_ML_SKILL_RECV_INFO *)aRecv, aIndex);
							break;
							//
					}
				}
		...
		switch ( protoNum )
		{
			...
			case 0xD0:
				{
					...
					//LogAddC(2, "HEAD: %x, SUB: %x",protoNum, lpDef->subcode);
			...
			/*case 0xFB:
			{
				...
				switch (lpDef->subcode)
				{
					case 0x0:
					{
						gWareHouseManager.WarehouseChangeRequest((LPMSG_REQ_GS_WAREHOUSE_CHANGE)aRecv, aIndex);
						break;
					}
				}
				break;
...
#if(ENABLE_WEBZEN_SPEED_CHECK == TRUE)
	if (g_bCheckSpeedHack == FALSE) // Even without fix, this shoud work on commonserver too
	{
		return;
	}
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex) 
{
	...
	if ( strcmp(gObj[aIndex].Ip_addr, "218.234.76.254") == 0 )
	{
		gObj[aIndex].CheckSumTime = 0;
		gObj[aIndex].m_InWebzen = true;
		return;
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	//Commands Manager
	Commands.Manager(aIndex,(PMSG_CHATDATA*)lpChat);
	...
	switch ( lpChat->chatmsg[0] )
	{
		...
		case '/':	// Command
			if ( slen > 2 )
			{
				...
				{
					if(gObjCheckIsInPCBang(lpObj) != FALSE) //season4 add-on :)
					{
...
void SCPJoinResultSend(int aIndex, BYTE result) //join_jay
{
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex) //password
{
	char serial[17];
	char id[11];
	LogAddTD("BUNA BAK [%c][%c][%c][%c][%c]",lpMsg->CliVersion[0],lpMsg->CliVersion[1],lpMsg->CliVersion[2],lpMsg->CliVersion[3],lpMsg->CliVersion[4]);
	...
	gObj[aIndex].PcBangGuid = 0; //season4.5 add-on
...
void CGLevelUpPointAdd(PMSG_LVPOINTADD * lpMsg, int aIndex) 
{
	...
	if(checkMaxStat(lpMsg->Type,&gObj[aIndex]) == false)
	{
		char Message[255];
...
void GCMagicListOneSend(int aIndex, char Pos, WORD type, BYTE level, WORD skill, BYTE btListType) //0043F420
{
	...
	pList.Type = type;
	pList.MagicInfo[0] = level<<3;
	pList.MagicInfo[0] |= (BYTE)skill & 0x07;

	memcpy(&sendbuf[lOfs], &pList, sizeof(pList)); //size 4
	lOfs+= sizeof(pList); //size 4

	pCount.h.size = lOfs;
	memcpy(sendbuf, &pCount, sizeof(pCount)); //size 6
...
void GCMagicListMultiSend(LPOBJ lpObj, BYTE btListType)
{
	...
	for ( int n=0;n<MAX_MAGIC;n++)
	{
		...
		{
			if(g_MasterSkillSystem.GetWeaponMagicSearch(lpObj, n) == FALSE) //season4 add-on
			{
				continue;
			}

			pList.Pos = n;
			pList.Type = (WORD)lpObj->Magic[n].m_Skill;

			pList.MagicInfo[0] = lpObj->Magic[n].m_Level<<3;
			pList.MagicInfo[0] |= lpObj->Magic[n].m_Skill & 0x07;
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex) //00438C70
{
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
		...
		else
		{
			...
			if ( lpItem->m_Type == ITEMGET(13,150)) // -> Fragment of Fire
			{
				int pos = gObjOverlapItemUsingDur((lpItem)?((CItem *)&lpItem->m_Number):NULL, map_num, item_num, aIndex, 30, lpItem->m_Type, 0);
				...
				if ( MAIN_INVENTORY_RANGE(pos) != FALSE )
				{
					...
					gObj[aIndex].pInventory[pos].m_Durability += lpItem->m_Durability;
					// ----
					if( gObj[aIndex].pInventory[pos].m_Durability >= 30.0f )
					{
						::gObjInventoryItemSet(aIndex,pos,-1);
						gObj[aIndex].pInventory[pos].Clear();
						::GCInventoryItemDeleteSend(aIndex,pos,1);
						::ItemSerialCreateSend(aIndex,235,gObj[aIndex].X,gObj[aIndex].Y,ItemGetNumberMake(14,200),lpItem->m_Level,0,0,0,0,aIndex,0,0);
						LogAddTD("[Elemental Frags] [%s][%s] Make Gem of Life (Fire)",gObj[aIndex].AccountID,gObj[aIndex].Name);
					}
					else
					{
						GCItemDurSend(aIndex,pos,gObj[aIndex].pInventory[pos].m_Durability,0);
			...
			if ( lpItem->m_Type == ITEMGET(13,151)) // -> Fragment of Earth
			{
				int pos = gObjOverlapItemUsingDur((lpItem)?((CItem *)&lpItem->m_Number):NULL, map_num, item_num, aIndex, 30, lpItem->m_Type, 0);
				...
				if ( MAIN_INVENTORY_RANGE(pos) != FALSE )
				{
					...
					if( gObj[aIndex].pInventory[pos].m_Durability >= 30.0f )
					{
						...
						gObj[aIndex].pInventory[pos].Clear();
						::GCInventoryItemDeleteSend(aIndex,pos,1);
						::ItemSerialCreateSend(aIndex,235,gObj[aIndex].X,gObj[aIndex].Y,ItemGetNumberMake(14,201),lpItem->m_Level,0,0,0,0,aIndex,0,0);
						LogAddTD("[Elemental Frags] [%s][%s] Make Gem of Life (Earth)",gObj[aIndex].AccountID,gObj[aIndex].Name);
					}
					else
					{
						GCItemDurSend(aIndex,pos,gObj[aIndex].pInventory[pos].m_Durability,0);
			...
			if ( lpItem->m_Type == ITEMGET(13,152)) // -> Fragment of Water
			{
				int pos = gObjOverlapItemUsingDur((lpItem)?((CItem *)&lpItem->m_Number):NULL, map_num, item_num, aIndex, 30, lpItem->m_Type, 0);
				...
				if ( MAIN_INVENTORY_RANGE(pos) != FALSE )
				{
					...
					if( gObj[aIndex].pInventory[pos].m_Durability >= 30.0f )
					{
						...
						gObj[aIndex].pInventory[pos].Clear();
						::GCInventoryItemDeleteSend(aIndex,pos,1);
						::ItemSerialCreateSend(aIndex,235,gObj[aIndex].X,gObj[aIndex].Y,ItemGetNumberMake(14,202),lpItem->m_Level,0,0,0,0,aIndex,0,0);
						LogAddTD("[Elemental Frags] [%s][%s] Make Gem of Life (Water)",gObj[aIndex].AccountID,gObj[aIndex].Name);
					}
					else
					{
						GCItemDurSend(aIndex,pos,gObj[aIndex].pInventory[pos].m_Durability,0);
			...
			if (lpItem->m_Type == ITEMGET(14,70) ||	lpItem->m_Type == ITEMGET(14,71) ||	lpItem->m_Type == ITEMGET(14,94)) //season 3.5 add-on
			{
			...
			if (lpItem->m_Type == ITEMGET(14,88) ||	lpItem->m_Type == ITEMGET(14,89) ||	lpItem->m_Type == ITEMGET(14,90) || lpItem->m_Type == ITEMGET(14,85) ||	lpItem->m_Type == ITEMGET(14,86) ||	lpItem->m_Type == ITEMGET(14,87)) //season 3.5 add-on
			{
				...
				{
				case ITEMGET(14,88):	iMaxOverLapped = 10;	break;
				case ITEMGET(14,89):	iMaxOverLapped = 30;	break;
				case ITEMGET(14,90):	iMaxOverLapped = 50;	break;
			...
			if ( lpItem->m_Type == ITEMGET(14,21) && lpItem->m_Level == 3)
			{
				for ( int n=0;n<MAIN_INVENTORY_SIZE;n++)
				{
					...
					else
					{
						...
						if ( BC_MAP_RANGE(map_num) )
						{
							...
							if ( g_BloodCastle.CheckQuestItemSerial(iBridgeIndex, lpItem) )
							{
								...
								int iMapNumber = g_BloodCastle.GetBridgeIndex(gObj[aIndex].MapNumber); //WZ Fix-me plz
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type) //004427B0
{
	...
	if ( lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(12,240) && //Season 4.7 add-on
		 lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(12,255) &&
		 gPkLimitFree == 0) //season 4.5 add-on
	{
		pResult.Result = false;
	}
	...
	if ( pResult.Result == 1 )
	{
		...
		BYTE SocketOption[5]; //
		BYTE SocketIndex; //
		g_SocketOption.GetActiveSocketOption(&lpObj->pInventory[lpMsg->Ipos],SocketOption,SocketIndex);
		...
		if ( bIsItem == TRUE )
		{
			...
			else if(type == ITEMGET(14,210)) // Summon Book Dark Phoenix
			{
				if(g_SummonBooks.CreateDarkPhoenixMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Dark Phoenix Monster (Summon Book of Dark Phoenix Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
				else
				{
					pResult.Result = false;
			...
			else if(type == ITEMGET(14,211)) // Summon Book Giant Dragon
			{
				if(g_SummonBooks.CreateGiantDragonMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Giant Dragon Monster (Summon Book of Giant Dragon Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,212)) // Summon Book Kundun Boss
			{
				if(g_SummonBooks.CreateKundunBossMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Kundun Boss Monster (Summon Book of Kundun Boss Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,213)) // Summon Book Giant Drakan
			{
				if(g_SummonBooks.CreateGiantDrakanMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Giant Drakan Monster (Summon Book of Giant Drakan Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,214)) // Summon Book Iron Knight
			{
				if(g_SummonBooks.CreateIronKnightMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Iron Knight Monster (Summon Book of Iron Knight Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,215)) // Summon Book Selupan Boss
			{
				if(g_SummonBooks.CreateSelupanBossMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Selupan Boss Monster (Summon Book of Selupan Boss Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,216)) // Summon Book Thunder Nappin
			{
				if(g_SummonBooks.CreateThunderNappinMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Thunder Nappin Monster (Summon Book of Thunder Nappin Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if(type == ITEMGET(14,217)) // Summon Book Hell Maine
			{
				if(g_SummonBooks.CreateHellMaineBossMonster(aIndex))
				{
					LogAddTD("[Summon Book] [%s][%s] Success to Summon Hell Maine Monster (Summon Book of Hell Maine Serial:%u)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex,lpMsg->Ipos);
					g_SummonBooks.CreateSummonEffect(aIndex,LV_PLAYER_EFFECT);
					g_SummonBooks.CreateSummonEffect(aIndex,SD_PLAYER_EFFECT);
				}
			...
			else if (type == ITEMGET(14, 101) && level == 0) //Season 4.9 add-on Pandora Box (Custom)
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				PandoraBoxItemBagOpen(&gObj[aIndex], lpObj->MapNumber, lpObj->X, lpObj->Y);
				LogAddTD("[%s][%s][%d]%d/%d Used Pandora Box Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			}
			else if (type == ITEMGET(14, 102) && level == 0) //Season 4.9 add-on Legendary Box (1) (Custom)
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				LegendaryBox1ItemBagOpen(&gObj[aIndex], lpObj->MapNumber, lpObj->X, lpObj->Y);
				LogAddTD("[%s][%s][%d]%d/%d Used Legendary Box (1) Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			}
			else if (type == ITEMGET(14, 103) && level == 0) //Season 4.9 add-on Legendary Box (2) (Custom)
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				LegendaryBox2ItemBagOpen(&gObj[aIndex], lpObj->MapNumber, lpObj->X, lpObj->Y);
				LogAddTD("[%s][%s][%d]%d/%d Used Legendary Box (2) Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			}
			else if (type == ITEMGET(14, 104) && level == 0) //Season 4.9 add-on Legendary Box (3) (Custom)
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				LegendaryBox3ItemBagOpen(&gObj[aIndex], lpObj->MapNumber, lpObj->X, lpObj->Y);
				LogAddTD("[%s][%s][%d]%d/%d Used Legendary Box (3) Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			}
			else
			{
...
// I have fixed this function today, after i have send gs to you
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex) //00446250
{
	BYTE result;
	...
	{
		Log(LOG_LEVEL_ERROR, "[%s][%d] error-L3", __FILE__, __LINE__);
		::GCItemMoveResultSend(aIndex, -1, 0, ItemInfo);
		return;
	}
	...
	{
		int money = ::GetWarehouseUsedHowMuch(lpObj->Level, lpObj->MLevel, lpObj->WarehousePW);
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex) 
{
	...
	LogAdd("BAK SUNA %d",gObj[DealerNumber].Type); //todo
...
//Must be tested.. -> Current State Bugged Moss Buy
/*void CGBuyRequestRecv(PMSG_BUYREQUEST * lpMsg, int aIndex)
{
	...
	if ( ShopC[tShop].m_item[lpMsg->Pos].IsItem() == TRUE )
	{
		...
		else
		{
			...
			//Fix DL Pets in Shops
			if (ShopC[tShop].m_item[lpMsg->Pos].m_Type == ITEMGET(13, 4) || ShopC[tShop].m_item[lpMsg->Pos].m_Type == ITEMGET(13, 5))
			{
				PetItemSerialCreateSend(aIndex, 236, 0, 0, ShopC[tShop].m_item[lpMsg->Pos].m_Type, ShopC[tShop].m_item[lpMsg->Pos].m_Level, ShopC[tShop].m_item[lpMsg->Pos].m_Durability, ShopC[tShop].m_item[lpMsg->Pos].m_Option1, ShopC[tShop].m_item[lpMsg->Pos].m_Option2, ShopC[tShop].m_item[lpMsg->Pos].m_Option3, -1, ShopC[tShop].m_item[lpMsg->Pos].m_NewOption, ShopC[tShop].m_item[lpMsg->Pos].m_SetOption);
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos) 
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) &&itemtype  < ITEMGET(13,4) ) || 
		itemtype == ITEMGET(13,10) || (itemtype >= ITEMGET(12,7) && itemtype < ITEMGET(12,36)) || 
		(itemtype >= ITEMGET(12,240) && itemtype <= ITEMGET(12,255)) ||
		(itemtype > ITEMGET(12,43) && itemtype < ITEMGET(13,0)) ||
	...
	else
	{
		...
		DurItem->Convert(DurItem->m_Type, DurItem->m_Option1, DurItem->m_Option2, DurItem->m_Option3, DurItem->m_NewOption, DurItem->m_SetOption,DurItem->m_ItemOptionEx, NULL, 0xFF, CURRENT_DB_VERSION);
...
void CGTradeRequestSend(PMSG_TRADE_REQUEST * lpMsg, int aIndex) 
{
	...
	{
		int iBridgeIndex = g_BloodCastle.GetBridgeIndex(gObj[aIndex].MapNumber); //season 3.0 add-on
		...
		case 0x01:
			{
				...
				lpObj->WarehouseMoney -= money;
				lpObj->Money += money;
				int rZen = GetWarehouseUsedHowMuch(lpObj->Level, lpObj->MLevel, lpObj->WarehousePW);
...
void CGWarehouseUseEnd(int aIndex) 
{
	...
	//#pragma message("(2) Test this fix for Multi Warehouse System")
	/*if ((lpObj->m_IfState.use > 0) && (lpObj->m_IfState.type == 6) && (lpObj->TargetShopNumber == 100))
	{
		lpObj->m_IfState.use = 0;
		lpObj->m_IfState.state = 0;
		lpObj->WarehouseSave = FALSE;
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex) 
{
		...
		aRecv->Type == 23 )
	{
		...
		if(CheckInventoryEmptySpace(lpObj,iItemHeight,iItemWidth) != 0)
		{
	...
	switch ( iMixType )
	{
		...
		case CHAOS_TYPE_LOTTERY_MIX:
			...
			break;
		case CHAOS_TYPE_BLOOD_SCROLL_MIX:
			g_IllusionTempleEvent.TicketChaosMix(lpObj);
			break;
		case CHAOS_TYPE_CONDOR_FEATHER_MIX:
			g_MixSystem.ThirdWingMix1(lpObj);
			break;
		case CHAOS_TYPE_THIRD_WING_MIX:
			g_MixSystem.ThirdWingMix2(lpObj);
			break;
			//season 4.7 add-on (Level 4 Wing Mix)
		case CHAOS_TYPE_FOURTH_WING_MIX:
			g_MixSystem.FourthWingMix(lpObj);
			break;
			//season 4.9 add-on (Absolute Sword Mix)
		case CHAOS_TYPE_ABSOLUTE_SWORD_MIX:
			g_MixSystem.AbsoluteSwordMix(lpObj);
			break;
			//season 4.9 add-on (Absolute Scepter Mix)
		case CHAOS_TYPE_ABSOLUTE_SCEPTER_MIX:
			g_MixSystem.AbsoluteScepterMix(lpObj);
			break;
			//season 4.9 add-on (Absolute Crossbow Mix)
		case CHAOS_TYPE_ABSOLUTE_CROSSBOW_MIX:
			g_MixSystem.AbsoluteCrossbowMix(lpObj);
			break;
			//season 4.9 add-on (Absolute Staff Mix)
		case CHAOS_TYPE_ABSOLUTE_STAFF_MIX:
			g_MixSystem.AbsoluteStaffMix(lpObj);
			break;
			//season 4.7 add-on (Crystal Feather Mix)
		case CHAOS_TYPE_CRYSTAL_FEATHER_MIX:
			g_MixSystem.CrystalFeatherMix(lpObj);
			break;
		case CHAOS_TYPE_CHERRYBLOSSOM_MIX:
			g_MixSystem.CherryBlossomMix(lpObj);
			break;
			//season4 add-on start
		case CHAOS_TYPE_SOCKET_SEED_EXTRACT_MIX:
			g_MixSystem.SocketSeedExtract(lpObj);
			break;
		case CHAOS_TYPE_SOCKET_SPHERE_COMPOSITE_MIX:
			g_MixSystem.SocketSeedSphereComposite(lpObj);
			break;
		case CHAOS_TYPE_SOCKET_SPHERE_SET_MIX:
			g_MixSystem.SocketSetSeedSphere(lpObj, aRecv->SocketSlot);
			break;
		case CHAOS_TYPE_SOCKET_SPHERE_REMOVE_MIX:
			g_MixSystem.SocketRemoveSeedSphere(lpObj, aRecv->SocketSlot);
			break;
			//season4 add-on end
		default:
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
	...
	if(gObjSearchActiveEffect(lpObj, AT_ICE_ARROW) == 1 || gObjSearchActiveEffect(lpObj, AT_STUN) == 1 || gObjSearchActiveEffect(lpObj, AT_SLEEP) == 1) //Season 3.0 add-on
	{
		return;
	}
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage, int iShieldDamage)
{
	...
	// ----
	#ifdef __IS_HPBAR__
	gHealthBar.Send(aIndex,TargetIndex);
	#endif // __IS_HPBAR__
}
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		if ( MagicDamageC.SkillGetRequireClass(lpObj->Class, lpObj->ChangeUP, lpObj->ChangeUP3rd, lpMagic->m_Skill) < 1 )
		{
			return;
		}
	...
	if ( lpMagic->m_Skill != 24 && lpMagic->m_Skill != 490 && gObjUseSkill.SpeedHackCheck(aIndex) == FALSE )
	{
		return;
	}
...
void GCMagicAttackNumberSend(LPOBJ lpObj, WORD MagicNumber, int usernumber,  BYTE skillsuccess)
{
	...
	{
		if(g_MasterSkillSystem.CheckRequireStatus(MagicNumber) != FALSE) //Season3 add-on
		{
			int GetSkillID = g_MasterSkillSystem.GetSkillID(MagicNumber);
...
void CGMagicCancel(PMSG_MAGICCANCEL * lpMsg, int aIndex)
{
	...
	if ( MagicNumber == 77 )
	{
		if ( gObjSearchActiveEffect(lpObj, AT_INFINITY_ARROW) == 1 ) //S3 Addition -> Infinity Arrow
		{
...
void GCStateInfoSend(LPOBJ lpObj, BYTE state, BYTE btEffectIndex)
{
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex) 
{
	...
	if(gObjSearchActiveEffect(&gObj[aIndex], AT_STUN) == 1 || gObjSearchActiveEffect(&gObj[aIndex], AT_SLEEP) == 1) //Season3 update
	{
...
void CGTargetTeleportRecv(PMSG_TARGET_TELEPORT * lpMsg, int aIndex) 
{
	...
	if(gObjSearchActiveEffect(&gObj[aIndex], AT_STUN) == 1)
	{
		return;
	}

	if(gObjSearchActiveEffect(&gObj[aIndex], AT_SLEEP) == 1)
	{
		return;
	}
...
void CGBeattackRecv(BYTE* lpRecv, int aIndex, int magic_send)
{
	...
	if (lpMagic->m_Skill == 42 ||
		lpMagic->m_Skill == 43 ||
		g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 460 || //s4
		g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 465 || //s4
		lpMagic->m_Skill == 41 ||
		lpMagic->m_Skill == 455)
	{
		return;
	...
	if (lpMagic->m_Skill == 24 ||
		...
		g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 525)
	{
	...
	if (lpMagic->m_Skill == 9 ||
		g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 445 ||
		g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 530)
	{
	...
	for (int n = 0; n<lpCount->Count; n++)
	{
		...
		if (tNumber >= 0 && tNumber < OBJMAX - 1)
		{
			...
			else if (g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 500) //season4 add-on
			{
				g_MasterSkillSystem.MaGumSkillDefenseDown(aIndex, tNumber);
				gObjAttack(&gObj[aIndex], &gObj[tNumber], lpMagic, TRUE, 1, 0, FALSE);
			}
			else if (g_MasterSkillSystem.GetSkillID(lpMagic->m_Skill) == 525) //season4 add-on
			{
				g_MasterSkillSystem.SkillFireScream(aIndex, tNumber, lpMagic);
			}
			else if (magic_send)
			{
				gObjAttack(&gObj[aIndex], &gObj[tNumber], lpMagic, TRUE, 1, 0, FALSE);
...
void CGDurationMagicRecv(PMSG_DURATION_MAGIC_RECV* lpMsg, int aIndex)
{
	...
	if ( lpMagic->m_Skill != 24 && lpMagic->m_Skill != 490)
	{
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //0045C690
{
	...
	if ( citem->IsItem() )
	{
		...
		else if ( citem->m_Type == ITEMGET(14,39) ) 
		{
		...
		else if ( citem->m_Type == ITEMGET(14,40) )
		{
		...
		else if ( citem->m_Type == ITEMGET(14,8) )
		{
			if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
			...
			if(gObjSearchActiveEffect(&gObj[aIndex], AT_POISON) == 1) //S3 Addition -> Poison
			{
				gObjRemoveBuffEffect(&gObj[aIndex], AT_POISON);
				gObj[aIndex].lpAttackObj = NULL;
				GCMagicCancelSend(&gObj[aIndex], 1);
			}
			else if(gObjSearchActiveEffect(&gObj[aIndex], AT_ICE) == 1) //S3 Addition -> Ice
			{
		...
		else if ( citem->m_Type == ITEMGET(14,46) )
		{
			...
			gObjSetItemEffect(&gObj[aIndex], 35);
		}
		else if ( citem->m_Type == ITEMGET(14,47) )
		{
			...
			gObjSetItemEffect(&gObj[aIndex], 36);
		}
		else if ( citem->m_Type == ITEMGET(14,48) )
		{
			...
			gObjSetItemEffect(&gObj[aIndex], 37);
		}
		else if ( citem->m_Type == ITEMGET(14,49) )
		{
			...
			gObjSetItemEffect(&gObj[aIndex], 38);
		}
		else if ( citem->m_Type == ITEMGET(14,50) )
		{
			...
			gObjSetItemEffect(&gObj[aIndex], 39);
		}
		else if ( citem->m_Type == ITEMGET(14,63) || citem->m_Type == ITEMGET(14,64) )
		{
			...
			g_SocketOption.IsSocketOption(&gObj[aIndex].pInventory[lpMsg->invenrotyTarget]) == FALSE) //season4 add-on
		{
		...
		else if (citem->m_Type == ITEMGET(14,171)) // Legend Amulet
		{
			if(gObjWingUpgradeEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex,pos,-1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex,lpMsg->invenrotyTarget);
		...
		else if (citem->m_Type == ITEMGET(14,174)) // Deathless Amulet
		{
			if(gObjWingUpgradeEx2(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex,pos,-1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex,lpMsg->invenrotyTarget);
		...
		else if (citem->m_Type == ITEMGET(14,200)) // Gem of Life (Fire)
		{
			if (gObjDragonHelmEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex,pos,-1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjDragonArmorEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjDragonPantsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjDragonGlovesEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjDragonBootsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if (citem->m_Type == ITEMGET(14,201)) // Gem of Life (Earth)
		{
			if (gObjGuardianHelmEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex,pos,-1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjGuardianArmorEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjGuardianPantsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjGuardianGlovesEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjGuardianBootsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if (citem->m_Type == ITEMGET(14,202)) // Gem of Life (Water)
		{
			if (gObjLegendaryHelmEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex,pos,-1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjLegendaryArmorEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjLegendaryPantsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjLegendaryGlovesEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			else if (gObjLegendaryBootsEvolveEx(&gObj[aIndex],lpMsg->inventoryPos,lpMsg->invenrotyTarget) == TRUE)
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,240) ) // Jewel of Level
		{
			if ( gObjItemLevelUpEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,241) ) // Jewel of Luck
		{
			if ( gObjItemAddLuckEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,242) ) // Jewel of Skill
		{
			if ( gObjItemAddSkillEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,243) ) // Jewel of Option
		{
			if ( gObjItemAddOptionEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,244) ) // Jewel of Excellent
		{
			if ( gObjItemAddExcellentEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,245) ) // Jewel of Ancient
		{
			if ( gObjItemAddAncientEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(14,246) ) // Jewel of Socket
		{
			if ( gObjItemAddSocketEx(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
			...
			citem->m_Type == ITEMGET(12,48) ) //season4
		{
			...
			{
				int addskill = gObjMagicAdd(&gObj[aIndex], citem->m_Type>>(9), citem->m_Type %  MAX_SUBTYPE_ITEMS, citem->m_Level, (BYTE)skillnumber);
		...
		else if ( citem->m_Type == ITEMGET(14,10) )
		{
			...
			else if ( gObj[aIndex].MapNumber == MAP_INDEX_SWAMPOFCALMNESS ) //season 3.0 add-on
			{
				gObjMoveGate(aIndex, 273);
			}
			else if ( gObj[aIndex].MapNumber == MAP_INDEX_RAKLION ) //season 4.0 add-on
			{
		...
		else if ( citem->m_Type == ITEMGET(14,9) )
		{
		...
		else if ( citem->m_Type == ITEMGET(14,20) )
		{
			if ( citem->m_Level == 0 )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
		...
		else if ( citem->m_Type == ITEMGET(13,15) )
		{
			if ( iItemUseType == 0 )
			{
		...
		{
			gObjUseCashShopPkDownItem(&gObj[aIndex], pos);
		}
				...
				citem->m_Type == ITEMGET(14,87) ) //S3 Addition
		{
			...
			gObjSetItemEffect(&gObj[aIndex], BuffEffectID);
		}
		...
		else if ( citem->m_Type == ITEMGET(13,70) ) //season 4 add-on
		{
			...
			if( gObjCheckRaiseTalismanMap(gObj[aIndex].MapNumber) == TRUE && bCheckEquipmentToMove == TRUE)
			{
				if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
				{
					gObjTeleport(aIndex, gObj[aIndex].m_wTeleportTalismanSaveMap, 
		...
		else if ( citem->m_Type == ITEMGET(13,66) ) //season 4 add-on
		{
			...
			{
				gObjTeleport(aIndex, MAP_INDEX_SANTATOWN, 220, 20);
...
void GCReFillSend(int aIndex,int Life,BYTE Ipos,BYTE flag,WORD wShield) //todo
{
	PMSG_REFILL pMsg;
	// ----
...
void GCManaSend(int aIndex,int Mana,BYTE Ipos,BYTE flag,WORD BP)
{
	if( aIndex < 0 || aIndex > OBJMAX-1)
	{
...
/*void GCNPggCheckSumRecv(PMSG_NPROTECTGGCHECKSUM * lpMsg, int aIndex) 
{
	...
	if ( dwGGErrCode )
	{
		LogAddTD("[NPgg] Invalid Checksum Detect Checksum = %x,AuthValue = %x [%s][%s]", lpMsg->m_ggadCheckSum.dwIndex, lpObj->NPggCSAuth.m_AuthAnswer.dwIndex, lpObj->AccountID, lpObj->Name);
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex) //maybe identical to gs 56 common "new party shit for pk player"
{
	...
	LogAddTD("[Duel] [%s][%s] Requested to Start Duel to [%s][%s]", lpObj->AccountID, lpObj->Name, gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
}
...
void CGGuildAssignStatus(PMSG_GUILD_ASSIGN_STATUS_REQ * aRecv, int aIndex) 
{
	...
	if ( aRecv->btType == 1 || aRecv->btType == 2 )
	{
		...
		{
			if ( iBattleMasterCount < (((lpObj->Level+lpObj->MLevel) / 200 ) +1) )
			{
				GDGuildReqAssignStatus(aIndex, aRecv->btType, aRecv->szTagetName, aRecv->btGuildStatus);
				return;
			}
