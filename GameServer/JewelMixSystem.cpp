...
BOOL CJewelMixSystem::MixJewel( int iIndex, int iJewelType, int iMixType)
{
	...
	LPINTERFACE_STATE m_ifState = &gObj[iIndex].m_IfState;//Season 4.5 addon
	
	if ( (( m_ifState->use == 1 && m_ifState->type == 20 )?0:1) != 0 )//Season 4.5 addon
	{
	...
	for ( int x= INVETORY_WEAR_SIZE ; x< MAIN_INVENTORY_SIZE; x++)
	{
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			...
			{
				LogAddTD("[JewelMix] [%s][%s] Mix - Delete Jewel, Type:%d, Level:%d, Serial:%u", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].pInventory[x].m_Type, gObj[iIndex].pInventory[x].m_Level,	gObj[iIndex].pInventory[x].m_Number);
...
BOOL CJewelMixSystem::UnMixJewel(int iIndex, int iJewelType, int iJewelLevel, int iInventoryPos)
{
	...
	if ( gObj[iIndex].pInventory[iInventoryPos].IsItem() == TRUE )
	{
		...
		{
			LogAddTD("[JewelMix] [%s][%s] iJewelLevel is different from request : %d / %d",	gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].pInventory[iInventoryPos].m_Level, iJewelLevel);
	...
	LogAddTD("[JewelMix] [%s][%s] UnMix - Delete Jewel, Type:%d, Level:%d, Serial:%u", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].pInventory[iInventoryPos].m_Type, gObj[iIndex].pInventory[iInventoryPos].m_Level, gObj[iIndex].pInventory[iInventoryPos].m_Number);
