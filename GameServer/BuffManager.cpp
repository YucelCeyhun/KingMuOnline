...
void gObjSealUserSetExp(LPOBJ lpObj, __int64 & Experience, BOOL bDisableExtraGainExp) //006409A0
{
...
#ifdef ADD_PET_PANDA
	if( lpObj->pInventory[8].IsItem() && lpObj->pInventory[8].m_Type == ITEMGET(13,80)
		&& lpObj->pInventory[8].m_Durability > 0 )	// Panda Pet
	{
		if(gObjSearchActiveEffect(lpObj,AT_CSHOP_SEAL_OF_SUSTENANCE) || gObjSearchActiveEffect(lpObj,AT_PCBANG_SEAL_OF_SUSTENANCE))
		{
			iCAL_EXP = 0;
		}
		else
		{
			int iAddExpRate = 0;
			// ----
			if(bDisableExtraGainExp)
			{
				iAddExpRate = 100;
			}
			else
			{
				iAddExpRate = 150; //panda_exp
			}
			// ----
			iCAL_EXP = iCAL_EXP * iAddExpRate / 100;
		}
	}
#endif	// ADD_PET_PANDA
			 ...
			 gObjSearchActiveEffect(lpObj, AT_CSHOP_MASTER_SEAL_OF_WEALTH) != FALSE ) //Seal of Ascension & Seal of Wealth & Seal of Healing
	{
		...
		int iMasterLevelCharacter = g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpObj); //season 4.5 add-on

		if( iMasterLevelCharacter != 0 )
		{
			if(gObjSearchActiveEffect(lpObj, AT_CSHOP_MASTER_SEAL_OF_ASCENSION) != FALSE || 
				gObjSearchActiveEffect(lpObj, AT_CSHOP_MASTER_SEAL_OF_WEALTH) != FALSE)
			{
				if(bDisableExtraGainExp != FALSE)
				{
					iGAIN_EXP = 100; //100% Gain EXP from g_MLAddExperience (Default value) if using Master Seal and bDisableExtraGainExp = TRUE
				}
				//else will be X by ItemAddOption :)
			}
			else //in case of not Using Master Seal T_T = default EXP
			{
				iGAIN_EXP = 100; //100% Gain EXP from g_MLAddExperience (Default value)
			}
		}
		else if(gObjSearchActiveEffect(lpObj, AT_CSHOP_MASTER_SEAL_OF_ASCENSION) != FALSE || 
				gObjSearchActiveEffect(lpObj, AT_CSHOP_MASTER_SEAL_OF_WEALTH) != FALSE)
		{
			iGAIN_EXP = 100; //100% Gain EXP from gAddExperience (Default value) if using Master Seal and not Master Character
		}
		else if(bDisableExtraGainExp != FALSE)
		{
			iGAIN_EXP = 100; //100% Gain EXP from gAddExperience (Default value) if is not Master Character and bDisableExtraGainExp = TRUE
		}

		iCAL_EXP = iCAL_EXP * iGAIN_EXP / 100;
	}

	#ifdef ADD_GLADIATOR_GLORY

	else if(gObjSearchActiveEffect(lpObj, AT_NEWPVPSYSTEM_REWARD))	// -> 1.00.93 Gladiator Glory Buff Effect
	{
		if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpObj))
			return;
		// ----
		if(g_NewPVP.IsVulcanusMap(lpObj->MapNumber))
		{
			if(iCAL_EXP == Experience)
			{
				int iAddExpRate = gObjSearchActiveEffect(lpObj, AT_REDUCE_MAGIC);
				iCAL_EXP += iCAL_EXP * iAddExpRate / 100;
