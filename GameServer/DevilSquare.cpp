	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			else if ( type == 5 )
			{
				...
				if (strcmp("end", TokenString) == 0)
				{
					for( int i = 0; i < MAX_DEVILSQUARE_GROUND; i++) //clear
					{
				...
				int iLevel = TokenNumber;

				if ( DS_LEVEL_RANGE(iLevel) != FALSE )
				{
					Token = GetToken();
					this->m_BonusScoreTable[CLASS_SUMMONER][iLevel] = (int&)TokenNumber;
				}
				else
				{
					Token = GetToken();
...
int CDevilSquare::GetUserLevelToEnter(int iUserIndex, int& iMoveGate) //0046A880
{
	...
	if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(&gObj[iUserIndex]) != FALSE)
	{
...
void CDevilSquare::gObjExpParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, BOOL MSBFlag)
{
	...
	for (n=0;n<MAX_USER_IN_PARTY;n++)
	{
		...
		if ( number >= 0 )
		{
			...
			{
				if ( lpPartyObj->Level+lpPartyObj->MLevel > toplevel )//Season 4.5 fix
				{
					toplevel = lpPartyObj->Level+lpPartyObj->MLevel;//Season 4.5 fix
				}
			}
		}
	}
		...
		if ( number >= 0 )
		{
			...
			if ( lpTargetObj->MapNumber ==lpPartyObj->MapNumber )
			{
				...
				if ( dis[n] < 10 )
				{
					...
					if ( toplevel >= (lpPartyObj->Level + lpPartyObj->MLevel + 200 ) ) // Season 4.5 fix
					{
						totallevel += lpPartyObj->Level + lpPartyObj->MLevel + 200;//Season 4.5 fix
					}
					else
					{
						totallevel += lpPartyObj->Level + lpPartyObj->MLevel; // Season 4.5 fix
					}
	...
	if ( lpTargetObj->Level >= 65 )
	{
		if ( viewplayer == 1 )
		{
			level += ((lpTargetObj->Level+lpTargetObj->MLevel) - 64) * (lpTargetObj->Level/ 4);
		}
		else
		{
		...
		if ( number >= 0 )
		{
			...
			if ( lpTargetObj->MapNumber == lpPartyObj->MapNumber )
			{
				if ( dis[n] < 10 )
				{
					...
					if(g_MasterLevelSystem.CheckMinMonsterLevel(lpPartyObj,lpTargetObj) == 0)
					{
						exp = 0;
					}
					else 
					{
						if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpPartyObj) != 0)
						{
							myexp = lpPartyObj->MLNextExp;
						}
						else
						{
