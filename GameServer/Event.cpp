...
void EledoradoBoxOpenEven(LPOBJ lpObj, int boxtype, int addlevel, int money)
{
	...
	EledoradoBox->DropItem(lpObj->m_Index,lpObj->MapNumber,lpObj->X, lpObj->Y);
}
...
void EventChipOpenEven(LPOBJ lpObj)
{
	...
	if ( LuckboxItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < ::g_ItemDropRateForBoxOfGold )
		{
			...
			CItem NewItem;
			NewItem.Convert(type, Option1, Option2, Option3, 0, 1, 0, NULL, 0xFF, CURRENT_DB_VERSION);
...
void GoldMedalOpenEven(LPOBJ lpObj)
{
	...
	if ( GoldMedalItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < g_ItemDropRateForGoldMedal )
		{
			...
			EventItem.Convert(type, Option1, Option2, Option3, 0, 1, 0, NULL, 0xFF, 3);
...
{
	if(CherryBlossom1->DropItem(lpObj->m_Index, btMapNumber, cX, cY, 0) != 0)
	{
		PMSG_SERVERCMD ServerCmd;
...
{
	return CherryBlossom2->DropItem(lpObj->m_Index, btMapNumber, cX, cY, iDropZen);
}

BOOL CherryBlossomEventItemBoxBOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY, int * iDropZen)
{
	return CherryBlossom3->DropItem(lpObj->m_Index, btMapNumber, cX, cY, iDropZen);
}
...
{
	LuckyCoin10->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}

//004A0DD0  /> \55            PUSH EBP
void LuckyCoint20ItemBagOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	LuckyCoin20->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}
...
void PandoraBoxItemBagOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	PandoraBox->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}

void LegendaryBox1ItemBagOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	LegendaryBox1->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}

void LegendaryBox2ItemBagOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	LegendaryBox2->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}

void LegendaryBox3ItemBagOpen(LPOBJ lpObj, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	LegendaryBox3->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}
...
void EGResultRegEventChip(PMSG_ANS_REGISTER_EVENTCHIP * aRecv)
{
	...
	{
		Result.ChipCount = -1;
		LogAddTD("[EventChip] [%s][%s] RegEventServer Fail (RegEventchip) %d", lpObj->AccountID, lpObj->Name, aRecv->Pos);
	}
	else
	{
		...
		// ----
		if (aRecv->nEVENT_CHIPS == 10)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 30), 0, 0, 0, 0, 0, aIndex, 0, 0); // Bless x10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 20)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 30), 1, 0, 0, 0, 0, aIndex, 0, 0); // Bless x20
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 30)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 30), 2, 0, 0, 0, 0, aIndex, 0, 0); // Bless x 30
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 40)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 31), 0, 0, 0, 0, 0, aIndex, 0, 0); // Soul x 10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 50)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 31), 1, 0, 0, 0, 0, aIndex, 0, 0); // Soul x 20
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 60)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 31), 2, 0, 0, 0, 0, aIndex, 0, 0); // Soul x 30
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 70)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 151), 0, 0, 0, 0, 0, aIndex, 0, 0); // Luck x 10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 80)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 152), 0, 0, 0, 0, 0, aIndex, 0, 0); // Skill x 10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 90)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 153), 0, 0, 0, 0, 0, aIndex, 0, 0); // Option x 10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
		else if (aRecv->nEVENT_CHIPS == 100)
		{
			ItemSerialCreateSend(aIndex, 235, lpObj->X, lpObj->Y, ITEMGET(12, 150), 0, 0, 0, 0, 0, aIndex, 0, 0); // Level x 10
			GCServerMsgStringSend("Exchange has been made.", aIndex, 1);
			LogAddTD("[EventChip] [%s][%s] Inventory Item Reward", lpObj->AccountID, lpObj->Name);
		}
	}
...
void EGRecvChangeStones( PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		...
		LogAddTD("[Stones] [%s][%s] Change Stones AddMoney(%d)",lpObj->AccountID,lpObj->Name,lpObj->iStoneCount*3000);
	}
	else
	{
		LogAddTD("[Stones] [%s][%s] Change Stones Fail",lpObj->AccountID,lpObj->Name);
	}
...
void Japan1StAnivBoxOpen(LPOBJ lpObj, int iBoxLevel)
{
	...
	Japan1StAnivBox->DropItem(lpObj->m_Index,lpObj->MapNumber,lpObj->X, lpObj->Y);
}
#endif
...
{
	MoonHarvestItemBag->DropItem(lpObj->m_Index, btMapNumber, cX, cY);
}
	...
	case 0:
		{
			if(lpMsg->w_14 == 1)
			{
				...
				if ( (g_iXMasVisitCount % g_iXMasEvent_LuckNumber1st) == FALSE )
				{
					...
					{
						XMasEventC->DropItem(aIndex,lpObj->MapNumber,lpObj->X, lpObj->Y);
					}
					else
					{
						XMasEventB->DropItem(aIndex,lpObj->MapNumber,lpObj->X, lpObj->Y);
					}
				...
				{
					XMasEventA->DropItem(aIndex,lpObj->MapNumber,lpObj->X, lpObj->Y);
				}
...
//004A3F90 
void ReqNPCXMasMapMoveOpen(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
