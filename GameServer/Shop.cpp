...
BOOL CShop::InsertItem(int type, int index, int level, int dur, int op1, int op2 ,int op3, int exc, int anc)
{
	int itemp;
	int width;
	int height;
...
skiploop:
	...
	this->m_item[blank].Convert(itemp, op1, op2, op3, exc, anc, 0, NULL, 0xFF, CURRENT_DB_VERSION);
...
BOOL CShop::LoadShopItem(char* filename)
{
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			if (this->InsertItem(type, index, level, dur, op1, op2, op3, exc, anc) == FALSE )
			{
...
BOOL CShop::LoadShopItem(int ShopNumber)
{
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			Token = WzMemScript.GetToken();
			exc = WzMemScript.GetNumber();

			Token = WzMemScript.GetToken();
			anc = WzMemScript.GetNumber();
...
BOOL ShopDataLoad() //Season 3.0 identical Season 2.5 identical gs-cs 56
{
	...
	//season 4.9 add-on
	ShopC[18].LoadShopItem(35); //WzAG 0x23 -> Moss Merchant
	ShopC[19].LoadShopItem(36); //WzAG 0x24 -> Stadium Guard
	ShopC[20].LoadShopItem(37); //WzAG 0x25 -> Leo The Helper
	ShopC[21].LoadShopItem(38); //WzAG 0x26 -> Luke The Helper

	//season 5.2 add-on
	ShopC[22].LoadShopItem(39);	//WzAG 0x27 -> Bolo The Merchant
	ShopC[23].LoadShopItem(40);	//WzAG 0x28 -> Dina The Merchant
	ShopC[24].LoadShopItem(41);	//WzAG 0x29 -> Adniel Princess
	//ShopC[25].LoadShopItem(42); //Kundun Shop
