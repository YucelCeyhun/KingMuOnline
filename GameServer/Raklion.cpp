...
void CRaklion::Run()
{
	...
	if(this->isUsed == 0 && !today->tm_hour && !today->tm_min && !today->tm_sec)
	{
		...
		{
			if(gObj[i].MapNumber == MAP_INDEX_RAKLION || gObj[i].MapNumber == MAP_INDEX_RAKLION_BOSS)
			{
...
#include "LogToFile.h"
CLogToFile RAKLION_TEST_LOG("RAKLION_TEST_LOG", ".\\RAKLION_TEST_LOG", TRUE);//identical
