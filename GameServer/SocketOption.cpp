...
int g_iSeedSphereCompositeMixRate;//Added by Nemesis
...
void CSocketOption::Init()//00556C60 -> Modified by Nemesis
{
	...
	for(iClearIndex = 0; iClearIndex < MAX_SPHERE_LEVEL; iClearIndex++)
	{
		this->iSphereDropRate[iClearIndex] = 0;
		this->iSphereDropLevel[iClearIndex] = 0;
	}
		...
		if( Token == NUMBER )
		{
			...
			while(true)
			{
				if(m_0C == 0)// m_0c == 0 - identical
				{
					...
					Token = GetToken();		bt_24		= TokenNumber;	
					Token = GetToken();		bt_28		= TokenNumber;		
					Token = GetToken();		// Reserved for Name
					Token = GetToken();		bt_2C		= TokenNumber;		
					Token = GetToken();		bt_34[0]	= TokenNumber;	
					Token = GetToken();		bt_34[1]	= TokenNumber;	
					Token = GetToken();		bt_34[2]	= TokenNumber;		
					Token = GetToken();		bt_34[3]	= TokenNumber;	
					Token = GetToken();		bt_34[4]	= TokenNumber;	
					Token = GetToken();		m_38		= TokenNumber;
				...
				else if(m_0C == 1)//m0C == 1 - identical
				{
					...
					temp.m_btOptionIndex = TokenNumber;


					Token = GetToken();		temp.m_btElementIndex	= TokenNumber;
					Token = GetToken();		temp.m_btElementType	= TokenNumber;
					Token = GetToken();		// Reserved for Name
					Token = GetToken();		temp.m_btOptionType	= TokenNumber;
					Token = GetToken();		temp.m_btOptionValue	= TokenNumber;
					Token = GetToken();		temp.m_iOptionRate	= TokenNumber;
					Token = GetToken();		temp.m_btSocketSlot[0]= TokenNumber;
					Token = GetToken();		temp.m_btSocketSlot[1]= TokenNumber;
					Token = GetToken();		temp.m_btSocketSlot[2]= TokenNumber;
					Token = GetToken();		temp.m_btSocketSlot[3]= TokenNumber;
					Token = GetToken();		temp.m_btSocketSlot[4]= TokenNumber;

					for(int n = 0;n < MAX_SOCKET_SLOT;n++)
					{
				...
				else if(m_0C == 2)//m_0C == 2  - Identical
				{
					...
					Token = GetToken();		m_5C = TokenNumber;
					Token = GetToken();		// Reserved for Name
					Token = GetToken();		m_60 = TokenNumber;
					Token = GetToken();		m_64 = TokenNumber;
					Token = GetToken();		m_68 = TokenNumber;
					Token = GetToken();		m_70[0] = TokenNumber;
					Token = GetToken();		m_70[1] = TokenNumber;
					Token = GetToken();		m_70[2] = TokenNumber;
					Token = GetToken();		m_70[3] = TokenNumber;
					Token = GetToken();		m_70[4] = TokenNumber;
					Token = GetToken();		m_70[5] = TokenNumber;
				...
				else if(m_0C == 3)//updated:(08-08-16) added section: 3
				{
					Token = GetToken();
					// ----
					if (!strcmp("end", TokenString))
...
void CSocketOption::_InitOption()//00557A60 - identical
{
	...
	g_iSeedSphereCompositeMixRate	= GetPrivateProfileInt("GameServerInfo","SeedSphereCompositeMixRate",60,gDirPath.GetNewPath("commonserver.cfg"));
...
BYTE CSocketOption::CheckItemType(int iItemNumber)//005581B0 -> (Not Original)
{
	/*
	switch(iItemNumber)
	{
	...
	*/
	...
	{
		if (iItemNumber == ITEMGET(this->m_SocketItemList[i].ItemID, this->m_SocketItemList[i].ItemIndex))
		{
			return true;
		}
		else if (this->m_SocketItemList[i].ItemID == -1 && this->m_SocketItemList[i].ItemIndex != -1)
		{
			...
			{
				if (iItemNumber == ITEMGET(Category, this->m_SocketItemList[i].ItemIndex))
				{
					return true;
				}
			}
...
//00558600 -> Modified by Nemesis
BYTE CSocketOption::IsSeedItem(int m_Type)
{
	return m_Type >= ITEMGET(12, 60) && m_Type <= ITEMGET(12, 65);
}

//00558650 -> Modified by Nemesis
BYTE CSocketOption::IsSphereItem(int m_Type)
{
	return m_Type >= ITEMGET(12, 70) && m_Type <= ITEMGET(12, 74);
}

//005586A0 -> Modified by Nemesis
BYTE CSocketOption::IsSeedSphereItem(int m_Type)
{
	BYTE btItemType		= 0;	//ebp-8
	BYTE btItemIndex	= 0;	//ebp-0c
	// ----
	btItemIndex	= m_Type % 512;
	btItemType	= (m_Type - btItemIndex) / 512;
	// ----
	if (btItemType == 12)
	{
...
//00558960 -> Modified by Nemesis (Fix for Earth Seed Sphere Mix)
BYTE CSocketOption::GetItemSocketByteOption(WORD m_Type, BYTE m_Level)
{
	...
	// ----
	btSphereLevel			= (m_Type % 512 - 100) / MAX_BONUS_OPTION;
	btSeedType				= (m_Type % 512 - 100) % MAX_BONUS_OPTION + 1;
	// ----
	for (int i = 0; i < MAX_SEED_COUNT; i++)
	{
		if (this->SocketSeed[i].m_btElementIndex == btSeedType)
		{
			btSeedSphereIndex = btSphereLevel * 50 + this->SocketSeed[i].m_btOptionIndex;
...
//00558F20 -> Modified by Nemesis
bool CSocketOption::SetSeedSphereData(CSeedSphere * lpSeedSphere, BYTE OptionIndex, BYTE SocketSlotCount)
{
	CSeed * lpSeedData		= 0;
	CSphere * lpSphereData	= 0;
	// ----
	lpSeedData = this->GetSeedData(OptionIndex);
	// ----
	if (!lpSeedData)
	{
		return false;
	...
	// ----
	if (!lpSeedSphere->SetInfo(lpSeedData, SocketSlotCount))
	{
		return false;
	}
	// ----
...
void CSocketOption::ApplySocketIceEffect(LPOBJ lpObj,BYTE btEffectType,BYTE btOptionType,WORD wOptionValue)
{
	...
	{
	case 16:
		iIncValue += this->_CalcIncreaseValue(lpObj->AddLife + lpObj->MaxLife,wOptionValue,btOptionType);
		lpObj->m_wSocketOpAddLife += iIncValue;
		break;
	case 17:
		iIncValue += this->_CalcIncreaseValue(lpObj->AddMana + lpObj->MaxMana,wOptionValue,btOptionType);
