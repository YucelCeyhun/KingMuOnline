	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			if ( iType == 1 )
			{
				...
				if ( btItemExOption > 0 )
				{
					btExOptionValue = 64 << (int)(btItemExOption-1);
				...
				ItemStatus.btLuckOption,ItemStatus.btAddOption,ItemStatus.btItemLevel,1,
				ItemStatus.btExOption,0,0,0,NULL,0xFF);
				// ----
...
void SGAnsBuyCashItem( protocol::MSG_STOG_BUY_ITEM_ANS* aRecv)
{
	...
	for ( int i=0;i<iItemBuyTransactionCount;i++)
	{
		if ( aRecv->transactions[i].dwResult == 1 )
		{
			...
			/*else if (iItemCode == ITEMGET(14, 163)) //Vault Expansion Certificate (need to be before GivebuytoInventory function :D
			{
				lpObj->m_iWarehouseExpandCount++;
				gWareHouseManager.SaveExpansionCount(lpObj);
				// Maybe discount expansion price from game points too :D
			}*/
