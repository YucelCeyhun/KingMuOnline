//GameServer 1.00.90 JPN - Completed -> (MixSystem Remade by Nemesis)
//Upgraded Date: 19-09-16
//Upgraded Date: 08-10-16
//Upgraded Date: 17-10-16
//Upgraded Date: 10-11-16
#include "stdafx.h"
#include "MixSystem.h"
#include "logproc.h"
#include "GameMain.h"
...
CMixSystem::CMixSystem()
{
	...
	// ----
	this->iHeavenWings			= ITEMGET(12,240);
	this->iDemonicWings			= ITEMGET(12,241);
	this->iBlossomWings			= ITEMGET(12,242);
	this->iThunderWings			= ITEMGET(12,243);
	this->iRavenWings			= ITEMGET(12,244);
	this->iEternalWings			= ITEMGET(12,245);
	// ----
	this->i3rdWingRate			= GetPrivateProfileInt("MixSettings","Wings3rdSuccessRate",40,MIXSYSTEM_FILE);
	this->i3rdWingLuckRate		= GetPrivateProfileInt("MixSettings","Wings3rdLuckSuccessRate",400,MIXSYSTEM_FILE);
	// ----
	this->i4thWingRate			= GetPrivateProfileInt("MixSettings","Wings4thSuccessRate",60,MIXSYSTEM_FILE);
	this->i4thWingLuckRate		= GetPrivateProfileInt("MixSettings","Wings4thLuckSuccessRate",400,MIXSYSTEM_FILE);
	// ----
	this->iAbsoluteSwordRate	= GetPrivateProfileInt("MixSettings","AbsoluteSwordSuccessRate",75,MIXSYSTEM_FILE);
	this->iAbsoluteScepterRate	= GetPrivateProfileInt("MixSettings","AbsoluteScepterSuccessRate",75,MIXSYSTEM_FILE);
	this->iAbsoluteCrossbowRate = GetPrivateProfileInt("MixSettings","AbsoluteCrossbowSuccessRate",75,MIXSYSTEM_FILE);
	this->iAbsoluteStaffRate	= GetPrivateProfileInt("MixSettings","AbsoluteStaffSuccessRate",75,MIXSYSTEM_FILE);
	// ----
	this->iCrystalFeatherRate	= GetPrivateProfileInt("MixSettings","CrystalFeatherSuccessRate",75,MIXSYSTEM_FILE);
}
...
BOOL CMixSystem::PlusItemLevelChaosMix(LPOBJ lpObj, int mixType)
{
	...
	else if ( lpObj->pChaosBox[PlusItemPos].m_Level == 10 || lpObj->pChaosBox[PlusItemPos].m_Level == 11) //season4 changed
	{
		if(btMixLevel == 1) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = 60;
...
BOOL CMixSystem::IsMixPossibleItem(int iItemCode) //005B1CE0
{
	...
	case ITEMGET(14,13):
	case ITEMGET(14,14):
	case ITEMGET(14,27):  // Ancient Metal
	case ITEMGET(14,96):
	case ITEMGET(14,170): // Crystal Feather
	case ITEMGET(14,172): // Divine Stone
	case ITEMGET(14,173): // Jewel of Dark Chaos
	case ITEMGET(14,241): // Jewel of Luck
	case ITEMGET(14,242): // Jewel of Skill
	case ITEMGET(14,243): // Jewel of Option
	case ITEMGET(12,150): // Bundled Level
	case ITEMGET(12,151): // Bundled Luck
		return TRUE;
	}
...
{	
	...
	// ----
	lpObj->ChaosSuccessRate = this->i3rdWingRate;
	// ----
...
void CMixSystem::FourthWingMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iExItem				= 0;
	int iCrystalFeather		= 0;
	int iJewelOfChaos		= 0;
	int iBundledSoul		= 0;
	int iBundledBless		= 0;
	int iJewelofCreation	= 0;
	int iInvalidItemCount	= 0;
	int iExItemMoney		= 0;
	int iCharmOfLuckCount	= 0;
	// ----
	...
	for (int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if(this->CheckItemCondition(&lpObj->pChaosBox[n],9,0,0,1,0,1))
			{
				iExItem++;
				iExItemMoney += lpObj->pChaosBox[n].m_BuyMoney;
			}
			else if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,170))
			{
				iCrystalFeather++;
			}
			else if( lpObj->pChaosBox[n].m_Type == this->iJewelofChaos)
	...
	// ----
	if(iExItem != 1 || iCrystalFeather != 1 || iJewelOfChaos != 1 || iBundledBless != 1 || iBundledSoul != 1 || iJewelofCreation != 1 || iInvalidItemCount > 0)
	{
		pMsg.Result = CB_INCORRECT_MIX_ITEMS;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	// ----
	LogChaosItem(lpObj, "FourthWing_Mix");
	LogAddTD("[Fourth Wing Mix] Chaos Mix Start");
	// ----
	if(iExItemMoney > 0)
	{
	...
	// ----
	lpObj->ChaosSuccessRate = this->i4thWingRate;
	// ----
	if(lpObj->ChaosSuccessRate > 60)
	{
		lpObj->ChaosSuccessRate = 60;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		int i4thWingIndex = rand()%6;
		int iItemType = 0;

		if(i4thWingIndex == 0)
		{
			iItemType = this->iHeavenWings;
		}
		else if(i4thWingIndex == 1)
		{
			iItemType = this->iDemonicWings;
		}
		else if(i4thWingIndex == 2)
		{
			iItemType = this->iBlossomWings;
		}
		else if(i4thWingIndex == 3)
		{
			iItemType = this->iThunderWings;
		}
		else if(i4thWingIndex == 4)
		{
			iItemType = this->iRavenWings;
		}
		else if(i4thWingIndex == 5)
		{
			iItemType = this->iEternalWings;
		}

		int iOption1 = 0;	// -> Skill
		int iOption2 = 0;	// -> Luck
		
		if( (rand()%1000) <= this->i4thWingLuckRate )
		{
			iOption2 = 1;
		}
		...
		*/
		...
		switch( iRandOpt )			// -> Excellent Options
		{
		case 0:
			if(iRandOptRate < 40)
				iNOption |= 0x01;	// -> Ignore Defensive Power
			break;
		case 1:
			if(iRandOptRate < 20)
				iNOption |= 0x02;	// -> Damage Reflection
			break;
		case 2:
			if(iRandOptRate < 70)
				iNOption |= 0x04;	// -> Recover Full HP
			break;
		case 3:
			if(iRandOptRate < 70)
				iNOption |= 0x08;	// -> Recover Full MP
			break;
		}
		*/

		ItemSerialCreateSend(lpObj->m_Index,255,0,0,iItemType,0,0,iOption1,iOption2,iOption3,-1,iNOption,0);
		::gObjInventoryCommit(lpObj->m_Index);

		LogAddTD("[Fourth Wing Mix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,	lpObj->Money, iChaosNeedMoney, iCharmOfLuckCount );
	}
	else
	{
		this->FourthWingMixItemDown(lpObj);
		::GCUserChaosBoxSend(lpObj, 0);
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);

		LogAddTD("[Fourth Wing Mix] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,	lpObj->Money, iChaosNeedMoney, iCharmOfLuckCount );
		lpObj->ChaosLock = FALSE;
	}
}

void CMixSystem::FourthWingMixItemDown(LPOBJ lpObj)
{
	if(lpObj->pChaosBox == NULL)
	{
		return;
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if(CheckLevel3WingItem(lpObj->pChaosBox[n].m_Type) != FALSE)
			{
				FourthWingMixItemDown2(&lpObj->pChaosBox[n]);
			}
			else if(CheckItemCondition(&lpObj->pChaosBox[n],0,0,0,0,0,1) != FALSE)
			{
				FourthWingMixItemDown2(&lpObj->pChaosBox[n]);
...
void CMixSystem::FourthWingMixItemDown2(CItem * lpItem)
{
	if(lpItem == NULL)
	{
		return;
...
void CMixSystem::AbsoluteSwordMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iExItem				= 0;
	...
	for (int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if(this->CheckItemCondition(&lpObj->pChaosBox[n],4,0,0,1,0,0))
			{
				iExItem++;
				iExItemMoney += lpObj->pChaosBox[n].m_BuyMoney;
			}
			...
			else if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,172))
			{
				iDivineStone++;
			}
			else
	...
	// ----
	if(iExItem != 1 || iJewelOfLife != 1 || iBundledLuck != 1 || iBundledLevel != 1 || iJewelofCreation != 1 || iDivineStone != 1 || iInvalidItemCount > 0)
	{
		pMsg.Result = CB_INCORRECT_MIX_ITEMS;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	// ----
	LogChaosItem(lpObj, "Absolute_Mix");
	LogAddTD("[Absolute Sword Mix] Chaos Mix Start");
	// ----
	if(iExItemMoney > 0)
	{
		lpObj->ChaosSuccessRate = iExItemMoney / 3000000;
	...
	// ----
	lpObj->ChaosSuccessRate = this->iAbsoluteSwordRate;
	// ----
	if(lpObj->ChaosSuccessRate > 75)
	{
		lpObj->ChaosSuccessRate = 75;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		int iItemType = 0;
		// ----
		iItemType = ITEMGET(0,35);
		// ----
		int iOption1 = 1;	// -> Skill
		int iOption2 = 0;	// -> Luck
		int iOption3 = 0;	// -> Life Opt
		int iNOption = 0;	// -> Excellent
		// ----
		ItemSerialCreateSend(lpObj->m_Index,255,0,0,iItemType,0,0,iOption1,iOption2,iOption3,-1,iNOption,0);
		::gObjInventoryCommit(lpObj->m_Index);
		// ----
		LogAddTD("[Absolute Sword Mix] [%s][%s] CBMix Success %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		// ----
		LogAddTD("[Absolute Sword Mix] [%s][%s] CBMix Fail %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	}
}

void CMixSystem::AbsoluteScepterMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iExItem				= 0;
	...
	// ----
	LogChaosItem(lpObj, "Absolute_Mix");
	LogAddTD("[Absolute Scepter Mix] Chaos Mix Start");
	// ----
	if(iExItemMoney > 0)
	{
		lpObj->ChaosSuccessRate = iExItemMoney / 3000000;
	...
	// ----
	lpObj->ChaosSuccessRate = this->iAbsoluteScepterRate;
	// ----
	if(lpObj->ChaosSuccessRate > 75)
	{
		lpObj->ChaosSuccessRate = 75;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		::gObjInventoryCommit(lpObj->m_Index);
		// ----
		LogAddTD("[Absolute Scepter Mix] [%s][%s] CBMix Success %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		// ----
		LogAddTD("[Absolute Scepter Mix] [%s][%s] CBMix Fail %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	}
}

void CMixSystem::AbsoluteCrossbowMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iExItem				= 0;
	...
	// ----
	LogChaosItem(lpObj, "Absolute_Mix");
	LogAddTD("[Absolute Crossbow Mix] Chaos Mix Start");
	// ----
	if(iExItemMoney > 0)
	{
		lpObj->ChaosSuccessRate = iExItemMoney / 3000000;
	...
	// ----
	lpObj->ChaosSuccessRate = this->iAbsoluteCrossbowRate;
	// ----
	if(lpObj->ChaosSuccessRate > 75)
	{
		lpObj->ChaosSuccessRate = 75;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		::gObjInventoryCommit(lpObj->m_Index);
		// ----
		LogAddTD("[Absolute Crossbow Mix] [%s][%s] CBMix Success %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		// ----
		LogAddTD("[Absolute Crossbow Mix] [%s][%s] CBMix Fail %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	}
}

void CMixSystem::AbsoluteStaffMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iExItem				= 0;
	...
	// ----
	LogChaosItem(lpObj, "Absolute_Mix");
	LogAddTD("[Absolute Staff Mix] Chaos Mix Start");
	// ----
	if(iExItemMoney > 0)
	{
		lpObj->ChaosSuccessRate = iExItemMoney / 3000000;
	...
	// ----
	lpObj->ChaosSuccessRate = this->iAbsoluteStaffRate;
	// ----
	if(lpObj->ChaosSuccessRate > 75)
	{
		lpObj->ChaosSuccessRate = 75;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		::gObjInventoryCommit(lpObj->m_Index);
		// ----
		LogAddTD("[Absolute Staff Mix] [%s][%s] CBMix Success %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		// ----
		LogAddTD("[Absolute Staff Mix] [%s][%s] CBMix Fail %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	}
}

void CMixSystem::CrystalFeatherMix(LPOBJ lpObj)
{	
	lpObj->ChaosLock = TRUE;
	// ----
	int iJewelOfLuck		= 0;
	int iJewelOfSkill		= 0;
	int iJewelOfOption		= 0;
	int iJewelOfCreation	= 0;
	int iInvalidItemCount	= 0;
	// ----
	int iChaosNeedMoney		= 0;
	...
	for(int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,241) )
			{
				iJewelOfLuck++;
			}
			else if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,242) )
			{
				iJewelOfSkill++;
			}
			else if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,243) )
			{
				iJewelOfOption++;
			}
			else if( lpObj->pChaosBox[n].m_Type == ITEMGET(14,22) )
	...
	// ----
	if(iJewelOfLuck != 6 || iJewelOfSkill != 6 || iJewelOfOption != 6 || iJewelOfCreation != 6 || iInvalidItemCount > 0)
	{
		pMsg.Result = CB_INCORRECT_MIX_ITEMS;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	// ----
	LogChaosItem(lpObj,"CrystalFeather_Mix");
	LogAddTD("[Crystal Feather Mix] Chaos Mix Start");
	// ----
	lpObj->ChaosSuccessRate = this->iCrystalFeatherRate; //TEST
	// ----
	if(lpObj->ChaosSuccessRate > 75)
	{
		lpObj->ChaosSuccessRate = 75;
	...
	if( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		::gObjInventoryCommit(lpObj->m_Index);
		// ----
		LogAddTD("[Crystal Feather Mix] [%s][%s] CBMix Success %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
	}
	else
	{
		this->ChaosBoxInit(lpObj);
		...
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		// ----
		LogAddTD("[Crystal Feather Mix] [%s][%s] CBMix Fail %d Money : %d-%d",lpObj->AccountID,lpObj->Name,lpObj->ChaosSuccessRate,lpObj->Money,iChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	}
}
...
void CMixSystem::SetItemChaosMix(LPOBJ lpObj)
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			...
			else if ( lpObj->pChaosBox[n].m_Type == ITEMGET(14,173) )	// -> Dark Chaos Here
			{
				ChoasGemCount++;
			}
...
void CMixSystem::SocketSeedExtract(LPOBJ lpObj) //0062A5B0 identical
{
	...
	{
		pMsg.Result = 1;
		ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, SeedData->m_iItemIndex, SeedData->m_btItemLevel, 1, 0, 0, 0, lpObj->m_Index, 0, 0);
		gObjInventoryCommit(lpObj->m_Index);
...
void CMixSystem::SocketSeedSphereComposite(LPOBJ lpObj) //0062AE20 identical
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++) //loc21
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if( g_SocketOption.IsSeedItem(lpObj->pChaosBox[n].m_Type) == 1)
			{
				iSeedSphereItemCount++;
				iChaosMoney += lpObj->pChaosBox[n].m_BuyMoney;
				pSeedSphere = &lpObj->pChaosBox[n];
			}
			else if( g_SocketOption.IsSphereItem(lpObj->pChaosBox[n].m_Type) == 1)
			{
				iSeedItemCount++;
				iChaosMoney += lpObj->pChaosBox[n].m_BuyMoney;
	...
	iSeedSphereCompositeRate = g_iSeedSphereCompositeMixRate + (iChaosMoney / 200000);
	...
	else
	{
		...
		{
			pMsg.Result = 1;
			ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, SeedSphere.m_iItemIndex, SeedSphere.m_btItemLevel, 1, 0, 0, 0, lpObj->m_Index, 0, 0);
			gObjInventoryCommit(lpObj->m_Index);
...
void CMixSystem::SocketSetSeedSphere(LPOBJ lpObj, BYTE SocketSlot) //0062B4A0 identical
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++) //loc21
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			...
			else if( g_SocketOption.IsSeedSphereItem(lpObj->pChaosBox[n].m_Type) == 1)
			{
				iSeedSpearItemCount++;
