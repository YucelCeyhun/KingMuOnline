...
void CCrywolfAltar::SetAltarValidContract(int iAltarObjIndex, int iClass)
{
	...
	if ( AltarInfo->m_dwValidContractTime < GetTickCount() )
	{
		...
		gObjApplyBuffEffectDuration(&gObj[iAltarObjIndex], AT_CW_ALTAR_STATUS, 0, 0, 0, 0, -10);
	}
}
...
BOOL CCrywolfAltar::SetAltarUserIndex(int iAltarObjIndex, int iClass, int iUserIndex)
{
	...
	gObjApplyBuffEffectDuration(&gObj[iAltarObjIndex], AT_CW_ALTAR_ATTEMPT, 0, 0, 0, 0, -10);
...
void CCrywolfAltar::ResetAltarUserIndex(int iAltarObjIndex, int iClass)
{
	...
	if ( AltarInfo->m_iContractCount >= MAX_ALTAR_CONTRACTS )
	{
		...
		gObjApplyBuffEffectDuration(&gObj[iAltarObjIndex], AT_CW_ALTAR_INACTIVE, 0, 0, 0, 0, -10);
	}
	else
	{
		gObjApplyBuffEffectDuration(&gObj[iAltarObjIndex], AT_CW_ALTAR_ACTIVE, 0, 0, 0, 0, -10);
...
void CCrywolfAltar::SetAltarViewState(int iClass, int iAltarState)
{
	...
	gObjApplyBuffEffectDuration(&gObj[AltarInfo->m_iAltarIndex], AT_CW_ALTAR_ACTIVE, 0, 0, 0, 0, -10);
}
...
int CCrywolfAltar::GetPriestMaxHPSum()
{
	...
	for ( int i=0;i<MAX_CRYWOLF_ALTAR;i++)
	{
		...
		gObjUpdateAppliedBuffEffect(lpUserObj, AT_SWELL_LIFE, &AddLife, 0); //season 3.0 add-on
