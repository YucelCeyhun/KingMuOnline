...
void CEventManagement::Run()
{
	...
	if ( this->m_wToday_Year != sysTime.wYear || this->m_wToday_Month != sysTime.wMonth || this->m_wToday_Day != sysTime.wDay )
	{
		...
		LogAddTD("[Event Management] %02d %02d %02d", this->m_wToday_Year, this->m_wToday_Month, this->m_wToday_Day);
	...
	for ( it = this->m_vtEventTime.begin() ; it != this->m_vtEventTime.end() ; it++ )
	{
		...
		{
			LogAddTD("[Event Management] (%d) %02d %02d (state=%d)", (*(it)).m_eEventKind , (*(it)).m_iHour, sysTime.wMinute , (*(it)).m_bEventStarted);
