...
//Custom EventItemBags
CProbabilityItemBag * GoldenArcher;

//Custom Boxes
CProbabilityItemBag * PandoraBox;
CProbabilityItemBag * LegendaryBox1;
CProbabilityItemBag * LegendaryBox2;
CProbabilityItemBag * LegendaryBox3;
...
int	GameServerPort;
int GameServerUdpPort;
int	JoinServerPort;
int	DataServerPort;
int DataServerPort2;
int	CashShopServerPort;
int	GuildServerPort;
...
void GameMainInit(HWND hWnd)
{
	...
	gUdpSocCER.RecvSet( 60006 ); // Same as MuManager to JS ?
	...
	LPOBJ mpObj = &gObj[mIndex];
	g_ItemDrop.Process(aIndex,mIndex);
	#endif	// __IS_ITEMDROP__
	...
	char MapAttrName[MAX_NUMBER_MAP][15] =
	{
		...
		"Terrain66.att",	// Kalimdor (66 para cargar y cliente y 65 para Gmove)
		"Terrain67.att",	// Arkania
		"Terrain68.att",	// Loren Market
		"Terrain69.att",	// Karutan
		"Terrain70.att",	// Acheron
	};
...
BOOL ExDataServerConnect(char* ConnectServer, DWORD wMsg)
{
	...
	int result = wsExDbCli.Connect(ConnectServer, GuildServerPort, wMsg);
	if ( result == 0 )
	{
		LogAdd(lMsg.Get(MSGGET(1, 147)), ConnectServer, GuildServerPort);
		return FALSE;
	}

	ExDataServerLogin();
	LogAddC(2, "Guild Server Online");
	return TRUE;
}
...
void ReadCommonServerInfo()
{
	...
	g_MonsterItemMng.ItemDropLoadScript(MONSTERDROP_FILE);
	...
	switch(gLanguage)
	{
		case 0: // Korea
			...
			strcpy(szCheckSum,gDirPath.GetNewPath("CheckSum.dat"));
			strcpy(szMoveReqFileName,gDirPath.GetNewPath("lang\\kor\\movereq(kor).txt"));
			strcpy(szCommonlocIniFileName,gDirPath.GetNewPath("commonserver.cfg"));
			// ----
	...
	g_PCBangPointSystem.Load(gDirPath.GetNewPath("PointItemOption.txt"));
	...
	gCreateMGLevel = GetPrivateProfileInt("GameServerInfo","CreateMGLevel",220,gDirPath.GetNewPath("commonserver.cfg"));
	gCreateDLLevel = GetPrivateProfileInt("GameServerInfo","CreateDLLevel",250,gDirPath.GetNewPath("commonserver.cfg"));
	gCreateSUMLevel = GetPrivateProfileInt("GameServerInfo","CreateSUMLevel",150,gDirPath.GetNewPath("commonserver.cfg"));
	...
	if(iMaxUser >= 0 && iMaxUser <= OBJMAXUSER)
	{
		...
		LogAddTD("[Option Reload] Object Max User Reloaded: %d",gServerMaxUser);
	}
	...
	gItemDurationTime = GetPrivateProfileInt("GameServerInfo","ItemDurationTime",30,gDirPath.GetNewPath("commonserver.cfg"));
	...
	wsprintfA(szDiplayGSInfo, "%s [Season 4.9]", szTemp/*, dwgCheckSum[1]%1000, dwgCheckSum[17]%1000, dwgCheckSum[1004]%1000*/);
	...
	g_GambleSystem.Init("GamblingItemBag.txt"); //Set Gamble System Initialization

	//Season 4.9 add-on
	//gWareHouseManager.Init();
...
void LoadItemBag() //0053E2F0
{
	...
	PandoraBox = new CProbabilityItemBag;
	if (PandoraBox == NULL)
	{
		// Memory allocation error
		MsgBox("CProbabilityItemBag %s", lMsg.Get(MSGGET(0, 110)));
	...
	PandoraBox->Init("eventitembag60.txt");//Custom EventItemBag

	//Legendary Box (1) (Custom)
	if (LegendaryBox1 != NULL)
		delete LegendaryBox1;

	LegendaryBox1 = new CProbabilityItemBag;
	if (LegendaryBox1 == NULL)
	{
		// Memory allocation error
		MsgBox("CProbabilityItemBag %s", lMsg.Get(MSGGET(0, 110)));
	...
	LegendaryBox1->Init("eventitembag61.txt");//Custom EventItemBag

	//Legendary Box (2) (Custom)
	if (LegendaryBox2 != NULL)
		delete LegendaryBox2;

	LegendaryBox2 = new CProbabilityItemBag;
	if (LegendaryBox2 == NULL)
	{
		// Memory allocation error
		MsgBox("CProbabilityItemBag %s", lMsg.Get(MSGGET(0, 110)));
	...
	LegendaryBox2->Init("eventitembag62.txt");//Custom EventItemBag

	//Legendary Box (3) (Custom)
	if (LegendaryBox3 != NULL)
		delete LegendaryBox3;

	LegendaryBox3 = new CProbabilityItemBag;
	if (LegendaryBox3 == NULL)
	{
		// Memory allocation error
		MsgBox("CProbabilityItemBag %s", lMsg.Get(MSGGET(0, 110)));
	...
	LegendaryBox3->Init("eventitembag63.txt");//Custom EventItemBag
