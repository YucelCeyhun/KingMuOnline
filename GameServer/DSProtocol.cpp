...
void DataServerProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen)
{
	switch ( protoNum )
	{
		...
		case 0x30:
			g_MasterLevelSystem.DGRequestMasterLevelInfoLoad(aRecv);
			break;
		...
		case 0xD0:
			...
			break;
		/*case eMSG_HEADER_WAREHOUSE:
		{
			PMSG_DEFAULT2 * lpMsg = (PMSG_DEFAULT2 *)aRecv;

			switch (lpMsg->subcode)
			{
				case eMSG_DS_WAREHOUSE_INFO:
				{
					gWareHouseManager.DGAnsWareHouseInfo((MSG_ANS_DS_WAREHOUSE_INFO *)aRecv);
				}
				break;

				case eMSG_DS_WAREHOUSE_CHANGE:
				{
					gWareHouseManager.DGAnsWarehouseChange((MSG_ANS_DS_WAREHOUSE_CHANGE *)aRecv);
				}
				break;
			}
			break;
...
void JGPGetCharList(BYTE *lpRecv)
{
	...
	if( pCLCount.Count > 0 )
	{
		for(int n=0;n<pCLCount.Count;n++)
		{
			...
			// ----
			pCList.CharSet[CS_CLASS] = (((lpCL->Class) >> 4 << 5) & 0xE0);
			int changeup = CS_GET_CHANGEUP(lpCL->Class);
			pCList.CharSet[CS_CLASS] |= CS_SET_CHANGEUP(changeup);
			pCList.CharSet[CS_CLASS] |= (((changeup) >> 1 << 3) & 8); // -> 3rd Class
			// ----
			pCList.CharSet[9] = 0;
			// ----
			...
			if(TempInventory[7] == INVALID_INDEX)
			{
				index |= 0x0C;
			}
			else
			...
			{
				if( TempInventory[8] == INVALID_INDEX )
				{
					index |= 0x03;
				}
				else
				{
					if( TempInventory[8] != 67) // -> Fix Rudolf
					{
			...
			if((TempInventory[7] >= 3 && TempInventory[7] <= 6) || TempInventory[7] == 0x1E || TempInventory[7] == 41 || TempInventory[7] == 42 )
			{
				pCList.CharSet[5] |= 0x0C;

				if ( TempInventory[7] == 0x1E )			// -> Cape of Lord
				{
					pCList.CharSet[9] |= 0x05;
				}
				else if ( TempInventory[7] == 0x29 )	// -> Summoner Level 1 Wings
				{
					pCList.CharSet[9] |= 0x06;
				}
				else if ( TempInventory[7] == 0x2A )	// -> Summoner Level 2 Wings
				{
					pCList.CharSet[9] |= 0x07;
				}
				else
				{
					pCList.CharSet[9] |= (TempInventory[7] - 2) & 0x07;	// -> Another Level 2 Wings
				}
			}

			if((TempInventory[7] >= 36 && TempInventory[7] <= 40) || TempInventory[7] == 43)
			{
				pCList.CharSet[5] |= 0x0C;
			...
			if( IS_NEWWINGS(TempInventory[7]+ITEMGET(12,0)) )
			{
				...
				pCList.CharSet[17]	|= (BYTE)(TempInventory[7] - 236 + 1) << 2;
			}
			...
			switch(TempInventory[8])
			{
				...
				case eLYCANWOLF_PET:
				{
					pCList.CharSet[12] |= (BYTE)0x80;
					break;
				}
				// ----
				case eGIANTLYON_PET:
				{
					pCList.CharSet[12] |= (BYTE)0x40;
					break;
				}
				// ----
				case eMISTIGER_PET:
				{
					pCList.CharSet[12] |= (BYTE)0x10;
					break;
				}
				*/
				// ----
				case eSABLEFANG_PET:
				{
					pCList.CharSet[5] |= CS_SET_HELPER(-1);
					pCList.CharSet[12] |= (BYTE)0x08;
					break;
				}
				// ----
				case eDARKWOLF_PET:
				{
					pCList.CharSet[12] |= (BYTE)0x02;
					break;
				}
				// ----
				case eDARKHORSE_PET:
				...
				case eDEMON_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x01 << 5);
					break;
				}
				// ----
				case eFAIRY_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x02 << 5);
					break;
				}
				// ----
				case eSKELETON_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x03 << 5);
					break;
				}
				// ----
				case eRUDOLF_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x04 << 5);
					break;
				}
				// ----
				case ePANDA_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x07 << 5);
					break;
				}
				// ----
				case eUNICORN_PET:
				{
					pCList.CharSet[16] |= (BYTE)(0x05 << 5);
					break;
				}
				// ----
				case eSPARKY_PET:
				{
					pCList.CharSet[10] &= (BYTE)0xFE;
					pCList.CharSet[16] |= (BYTE)(0x06 << 5);
					break;
				}
				// ----
				case eFENRIR_PET:
				{
					...
					pCList.CharSet[10] &= (BYTE)0xFE;
					pCList.CharSet[12] &= (BYTE)0xFE;
					pCList.CharSet[12] |= (BYTE)0x04;
					pCList.CharSet[16] = 0;
					...
					if ( (btExcellentOption&8) == 8 ) // Orange Fenrir
					{
...
void JGGetCharacterInfo(SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	// ----
	//gResetSystem.SendResetData(lpObj);
	// ----
	GCItemListSend(aIndex);
	GCMagicListMultiSend(lpObj, 0);
	// ----
	g_MasterLevelSystem.GDRequestMasterLevelInfoLoad(lpObj);
	// ----
	CGRequestQuestInfo(aIndex);
	// ----
	g_QuestUtil.Quest3rdRequestInfo(lpObj); // -> Season 2.5 Quest New CALL
	// ----
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)
{
	...
	//Season 3.0 Master Level New CALL
	g_MasterLevelSystem.GDRequestMasterLevelInfoSave(lpObj);
}
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	for ( int n=0;n<WAREHOUSE_SIZE;n++)
	{
		...
		if ( itype != -1 )
		{
			...
			{
				if (item.m_Level > MAX_ITEM_LEVEL) // +14 & +15 Item Level Fix on WareHouse
				{
					item.m_Level = 0;
				}
			}
			...
			if ( dbsize >= 16 )
			{
				item.m_ItemOptionEx = DBI_GET_380OPTION(lpMsg->dbItems[n*dbsize+9]);
			...
			if ( g_SocketOption.CheckItemType(itype) == 1 )
			{
			...
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, 0, 0, 0, NULL, 0xFF, 3);
			}
			else if ( lpMsg->DbVersion <= 2 )
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, 0, 0xFF, 1);
			}
			else
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, SocketOption, SocketOptionIndex, 3);
			}
			...
			WORD hiword = MAKE_NUMBERW(lpMsg->dbItems[n*dbsize+3], lpMsg->dbItems[n*dbsize+4]);
			WORD loword = MAKE_NUMBERW(lpMsg->dbItems[n*dbsize+5], lpMsg->dbItems[n*dbsize+6]);
...
//00431A60 -> Modified Date: (03-09-16)
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg)
{
	...
	if ( lpMsg->MapNumber == (BYTE)-1 || lpMsg->MapNumber == (BYTE)-2 )
	{
		...
		NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, NULL, 0xFF, CURRENT_DB_VERSION);
	...
	else if ( lpMsg->MapNumber == 236 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			pCreateItem.m_Level = lpMsg->Level;//Season 4.5 changed position
			pCreateItem.m_Durability = lpMsg->Dur;//Season 4.5 changed position

			pCreateItem.Convert(ITEMGET(iItemType, iItemIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0, NULL, 0xFF, 3);
	...
	{
		int iRetMapNumber = lpMsg->MapNumber; //loc106
		...
		//Season 4.5 SocketSystem Addon Start
		...
		//Season 4.5 SocketSystem Addon end
...
{
	LPOBJ lpObj = &gObj[aIndex];
	char pbuffer[2048];//Season 4.5 change size in 2x times
...
{
	LPOBJ lpObj = &gObj[aIndex];
	char pbuffer[2048];//Season 4.5 changed size
...
void DGChangeNameResult(LPSDHP_CHANGENAME_RESULT lpMsg)
{
	...
	memcpy(szAccountID, lpMsg->szAccountID, sizeof(szAccountID));
	...
	memcpy(pMsg.szName, lpMsg->szName, sizeof(pMsg.szName));
    memcpy(pMsg.szName2, lpMsg->szName2, sizeof(pMsg.szName2));
	...
	if ( Result == 0)
    {
		...
		memcpy(Name2, lpMsg->szName2, sizeof(Name2));
		memcpy(Name, lpMsg->szName, sizeof(Name));
...
void DGSummonerStateUpdatedSend(LPPMSG_ANS_SUMMONER_CREATE lpMsg)
{
	...
	gObj[aIndex].Summoner = lpMsg->IsSummonerEnable&0x01; // Fix Create Summoner without Card
