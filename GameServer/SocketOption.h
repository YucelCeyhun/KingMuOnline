...
typedef struct
{
	...
	BYTE btElements[MAX_SOCKET_SLOT];	// 0x4A8
...
typedef struct
{
	short ItemID;
	short ItemIndex;
}SOCKET_ITEM_LIST, *LPSOCKET_ITEM_LIST;
...
public:
	...
	bool SetSeedSphereData(CSeedSphere * lpSeedSphere, BYTE OptionIndex, BYTE SocketSlotCount);
...
public:
	CSeed					SocketSeed[MAX_SEED_COUNT];						//this+004
	CSphere					SocketSphere[MAX_SPHERE_LEVEL];					//this+3EC
	CSocketSphere			SocketSpear[MAX_BONUS_OPTION];					//this+428
	...
	int						iSocketSlotCountRate[MAX_SOCKET_SLOT];			//this+508
	int						iTwoHandSlotCountRate[MAX_SOCKET_SLOT];			//this+51C
	BOOL					bSphereDrop;									//this+530
	int						iSphereDropRate[MAX_SPHERE_LEVEL];				//this+534
	int						iSphereDropLevel[MAX_SPHERE_LEVEL];				//this+548
	// ----
...
extern int g_iSeedSphereCompositeMixRate;
