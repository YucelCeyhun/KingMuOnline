...
BOOL TMonsterSkillElement::ApplyElementStun(int iIndex, int iTargetIndex)
{
	...
	gObjApplyBuffEffectDuration(lpTargetObj, AT_STUN, 0, 0, 0, 0, SkillStunTime);
...
BOOL TMonsterSkillElement::ApplyElementSummon(int iIndex, int iTargetIndex)
{
	...
	if (lpObj->Class == 459	&& lpObj->Connected == PLAYER_PLAYING && lpObj->MapNumber == MAP_INDEX_RAKLION_BOSS && iIndex == iTargetIndex )//Season 4.5 addon
	{
...
BOOL TMonsterSkillElement::ApplyElementImmuneSkill(int iIndex, int iTargetIndex)
{
	...
	gObjApplyBuffEffectDuration(lpTargetObj, AT_IMMUNE_MAGIC, 0, 0, 0, 0, SkillImmune);
	return FALSE;
}
...
BOOL TMonsterSkillElement::ApplyElementTeleportSkill(int iIndex, int iTargetIndex)
{
	...
	if(gObjSearchActiveEffect(&gObj[iIndex], AT_STUN) == 1)
	{
		return FALSE;
	}

	if(gObjSearchActiveEffect(&gObj[iIndex], AT_SLEEP) == 1)
	{
		return FALSE;
	}
	...
	if(gObjSearchActiveEffect(lpTargetObj, AT_POISON) == 0)
	{
		if ( this->m_iNullifiedSkill  < 1 )
		{
			if ( retResistance(lpTargetObj, 1) == 0 )
			{
				...
				gObjApplyBuffEffectDuration(lpTargetObj, AT_POISON, 19, 3, 0, 0, this->m_iIncAndDecValue);
			}
		}
		else
		{
...
BOOL TMonsterSkillElement::ApplyElementBerserk(int iIndex, int iTargetIndex)
{
	...
	gObjApplyBuffEffectDuration(lpTargetObj, 63, 0, 0, 0, 0, DurationTime);
	gObjApplyBuffEffectDuration(lpTargetObj, 64, 0, 0, 0, 0, DurationTime);
