...
void GDGuildMemberAdd(int aIndex, LPSTR Name, LPSTR MemberId)
{
	...
	pMsg.NumberH = SET_NUMBERH(aIndex);
	pMsg.NumberL = SET_NUMBERL(aIndex);
	pMsg.MemberID[MAX_GUILD_LEN] = 0;
	pMsg.GuildName[MAX_ACCOUNT_LEN] = 0;
...
void DGGuildAnsAssignStatus(EXSDHP_GUILD_ASSIGN_STATUS_RESULT * lpMsg)
{
	if ( lpMsg->btFlag == 1 )
	{
		...
		LogAddTD("[U.System][Guild Status][Assign] Guild : ( %s ) Member : ( %s ) Status : ( %d )", lpMsg->szGuildName, lpMsg->szTargetName, lpMsg->btGuildStatus);
...
void DGUnionBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT * lpMsg)
{
	...
	LogAddTD("[GS][U.System][Union][BreakOff] ( Union Master Guild: %s ), (Union Member Guild: %s)", lpTargetGuild->Name, lpReqGuild->Name);
		...
		if ( lpGuildInfo )
		{
			...
			for ( int n=0;n<MAX_USER_GUILD;n++)
			{
				if ( lpGuildInfo->Use[n] > 0 )
				{
					...
					{
						TNotice _Notice(G_NOTIFICATION_SET);
...
void DGUnionListRecv(LPBYTE aRecv)
{
	...
	PHeadSetW((LPBYTE)lpMsg, 0xE9, lpMsg->btCount * sizeof(PMSG_UNIONLIST) + 0x10);
...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
    ...
    {
        LogAddC(2,"[HACKTOOL] : NPC-FriendAddRequest npc:%d ip:%s account:%s name:%s State:%d",
			gObj[aTargetIndex].Class,gObj[aIndex].Ip_addr,gObj[aIndex].AccountID,
...
void WaitFriendAddResult(FHP_WAITFRIEND_ADD_RESULT * lpMsg)
{
	...
	LogAdd("[%s] WaitFriend add result (%d) [%s]", Name.GetBuffer(), lpMsg->Result, szFriendName.GetBuffer());
