...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic, int magicsend, BYTE MSBFlag, int AttackDamage, BOOL bCombo)
{
	...
	if ( AttackDamage == 0 )
	{
		...
		if(gObjSearchActiveEffect(lpTargetObj, AT_BERSERKER) == 1)
		{
			...
			float BerserkerEffect = gObjGetActiveEffect(lpTargetObj, 32);

			float loc24 = (((float)(40.0f) - (float)(BerserkerEffect))/100.0f);

			loc24 = (0.1f > loc24)?0.1f:loc24;

			targetdefense -= BerserkerDex * loc24;
		}
		...
		targetdefense -= targetdefense * iInnovationValue / 100;
		...
		if ( gObjFairySprite(lpTargetObj) == TRUE )
		{
			if ( AttackDamage > 1 )
			{
				float damage = AttackDamage * 7 / 10.0f;
		...
		if ( gObjSparkySprite(lpTargetObj) == TRUE )
		{
			if ( AttackDamage > 1 )
			{
				float damage = AttackDamage * 7 / 10.0f;
		...
		{
			AttackDamage += 30; //season 4.6 add-on (Panda Ring)
		}

		if(lpObj->pInventory[EQUIPMENT_RING_RIGHT].IsItem() && lpObj->pInventory[EQUIPMENT_RING_RIGHT].m_Type == ITEMGET(13,153))
		{
			AttackDamage += 150; // Ex406 add-on (Ring of Bless)
		}
		...
		if ( gObjWingSprite(lpObj) == TRUE )
		{
			...
			else
			{
				...
				else if ( Wing->m_Type >= ITEMGET(12,240) && Wing->m_Type <= ITEMGET(12,245) ) //season 4.7 add-on
				{
					AttackDamage = AttackDamage * (Wing->m_Level * 2 + 145) / 100;	// #formula
				}
				else if ( Wing->m_Type >= ITEMGET(12,246) && Wing->m_Type <= ITEMGET(12,255) )  //season 4.7 special wings add-on
				{
					AttackDamage = AttackDamage * (Wing->m_Level * 2 + 152) / 100;	// #formula
				}
			...
			if ( Wing->m_Type != ITEMGET(13,30) ) // Cape Of Lord
			{
				if ( AttackDamage > 1 )
				{
					if ( Wing->m_Type >= ITEMGET(12, 36) && Wing->m_Type <= ITEMGET(12, 40) || Wing->m_Type == ITEMGET(12, 43)) //season 2.5 add-on
					{
						...
						//4th Level Wings
						if(Wing->m_Type >= ITEMGET(12,240) && Wing->m_Type <= ITEMGET(12,245))
						{
							...
							if((rand()%100)< 7)
							{
								...
								if((WingOption&4) == 4) // 7% Chance of Recover Full Life
								{
									gObjAddMsgSendDelay(lpTargetObj,13,lpObj->m_Index,100,0);
								}
								else
								{
									if((WingOption&8) == 8) // 7% Chance of Recover Full Mana
									{
										gObjAddMsgSendDelay(lpTargetObj,14,lpObj->m_Index,100,0);
									}
								}
						...
						// Special Wings
						if(Wing->m_Type >= ITEMGET(12,246) && Wing->m_Type <= ITEMGET(12,249) || Wing->m_Type == ITEMGET(12,252))	// -> Wings of Deathless (Added: 22-12-16)
						{
							float damage = (float)(AttackDamage * (55 - (Wing->m_Level*2))) / 100.0f;
							AttackDamage = (int)(damage);	//  #formula
							...
							if((rand()%100)< 8)
							{
								...
								if((WingOption&4) == 4) // 8% Chance of Recover Full Life
								{
									gObjAddMsgSendDelay(lpTargetObj,13,lpObj->m_Index,100,0);
								}
								else
								{
									if((WingOption&8) == 8) // 8% Chance of Recover Full Mana
									{
										gObjAddMsgSendDelay(lpTargetObj,14,lpObj->m_Index,100,0);
									}
								}
						...
						// Season 8 Wings
						if(Wing->m_Type == ITEMGET(12,250) || Wing->m_Type == ITEMGET(12,251))
						{
							float damage = (float)(AttackDamage * (55 - (Wing->m_Level*2))) / 100.0f;
							AttackDamage = (int)(damage);	//  #formula
							...
							if((rand()%100)< 9)
							{
								...
								if((WingOption&4) == 4) // 9% Chance of Recover Full Life
								{
									gObjAddMsgSendDelay(lpTargetObj,13,lpObj->m_Index,100,0);
								}
								else
								{
									if((WingOption&8) == 8) // 9% Chance of Recover Full Mana
									{
										gObjAddMsgSendDelay(lpTargetObj,14,lpObj->m_Index,100,0);
									}
								}
				...
				case 214: //Drain Life
					{
						...
						if(lpTargetObj->Type == OBJ_MONSTER)
						{
							iRecvLife = (lpObj->Energy / 15) + (lpTargetObj->Level / 2.5);
						}
						...
						int CurrentLife = (iRecvLife + lpObj->Life);
					...
					if ( AttackDamage > 0 )
					{
...
#endif
						{
							...
							if ( lpEquipment && lpEquipment->IsItem() )
							{
								...
								int iDurEquipment = lpEquipment->m_Durability * value / 100.0f;
	...
	if ( AttackDamage > 0 )
	{
		...
		if ( (rand()%100) < 6 ) // season 4.9 add-on
		{
			if(gObjWingSprite(lpTargetObj) == TRUE)
			{
				CItem * Wing = &lpTargetObj->pInventory[7];

				if(Wing->m_Type >= ITEMGET(12, 236) && Wing->m_Type <= ITEMGET(12, 239)) // Wing 2.5
				{
					BYTE WingOption = lpTargetObj->pInventory[7].m_NewOption;

					if((WingOption&2) == 2) // 6% Chance of Return Damage
					{
						if(lpObj->Type == OBJ_MONSTER)
						{
		...
		if ( (rand()%100) < 7 ) // season 4.7 add-on
		{
			if(gObjWingSprite(lpTargetObj) == TRUE)
			{
				CItem * Wing = &lpTargetObj->pInventory[7];

				if(Wing->m_Type >= ITEMGET(12, 240) && Wing->m_Type <= ITEMGET(12, 245)) // Wing 4
				{
					BYTE WingOption = lpTargetObj->pInventory[7].m_NewOption;

					if((WingOption&2) == 2) // 7% Chance of Return Damage
					{
						if(lpObj->Type == OBJ_MONSTER)
						{
							gObjAddMsgSendDelay(lpTargetObj, 12, lpObj->m_Index, 10, lpObj->m_AttackDamageMax);
