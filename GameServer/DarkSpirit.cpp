...
void CDarkSpirit::Run()
{
	...
	if(this->m_pPetItem->m_RequireLeaderShip != 0 && //season4 add-on
		(lpObj->Leadership + lpObj->AddLeadership < this->m_pPetItem->m_RequireLeaderShip))
	{
		return;
	}
...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType)
{
	...
	if ( lpObj->pInventory[1].m_DurabilitySmall >= lpObj->m_MLPassiveSkill.m_iML_DecreasePetDurReduction + 800 ) //season4 changed
	{
		...
		if ( lpObj->pInventory[1].m_Durability < 1.0f )
		{
			...
			{
				LogAddTD("[%s][%s][PetItemExpDown] [%s] Level:[%d]Exp:[%d]DecExp[%d]",	lpObj->AccountID, lpObj->Name, lpObj->pInventory[1].GetName(),	lpObj->pInventory[1].m_PetItem_Level, lpObj->pInventory[1].m_PetItem_Exp, iPetExp - lpObj->pInventory[1].m_PetItem_Exp);

				this->Set(lpObj->m_Index, &lpObj->pInventory[1]);
				this->SendLevelmsg(lpObj->m_Index, 1, 0, 0xFF);
			}

			LogAddTD("[%s][%s] 다크스피릿의발톱 Item is Broken because durability is exhausted [%d]", lpObj->AccountID, lpObj->Name, lpObj->pInventory[1].m_Number);
		}
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 )
		{
			...
			if ( Wing->m_Type == ITEMGET(12, 236) || //season 4.9 add-on - Wing 2.5
				 Wing->m_Type == ITEMGET(12, 237) || 
				 Wing->m_Type == ITEMGET(12, 238) || 
				 Wing->m_Type == ITEMGET(12, 239) )
			{
				float damage = (float)(AttackDamage * (57 - (Wing->m_Level*2))) / 100.0f;
				AttackDamage = (int)(damage);	//  #formula
			}
			if ( Wing->m_Type == ITEMGET(12, 240) || //season 4.7 add-on - Wing 4
				 Wing->m_Type == ITEMGET(12, 241) || 
				 Wing->m_Type == ITEMGET(12, 242) || 
				 Wing->m_Type == ITEMGET(12, 243) ||
				 Wing->m_Type == ITEMGET(12, 244) ||
				 Wing->m_Type == ITEMGET(12, 245) )
			{
	...
	if ( lpTargetObj->Live )
	{
		...
		if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER )
		{
			...
			{
				if(gObjSearchActiveEffect(lpObj, AT_POTION_OF_BLESS) != 0 || gObjSearchActiveEffect(lpObj, AT_POTION_OF_SOUL) != 0)
				{
					AttackDamage += (AttackDamage * 20) / 100;
				}
				else
...
int CDarkSpirit::GetAttackDamage(LPOBJ lpObj, int targetDefense, BOOL bIsOnDuel, int crititcaldamage)
{
	...
	if((lpObj->pInventory[0].m_Type >= ITEMGET(2,8) && lpObj->pInventory[0].m_Type <= ITEMGET(2,12) ) ||
		...
		lpObj->pInventory[0].m_Type == ITEMGET(2,17)) //season4 add-on
	{
...
{
	int SuccessAttackRate = 0; //loc2
	int TargetSuccessfulBlocking = lpTargetObj->m_SuccessfulBlocking;
	int MsgDamage = 0; //loc4

	SuccessAttackRate += lpObj->m_MLPassiveSkill.m_iML_IncreaseAttackSuccessRate;
...
BOOL CDarkSpirit::MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss)
{
	...
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 45) / 10;	// #formula
	}
	else if ( lpObj->Class == CLASS_DARKLORD )
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 4 );	// #formula
	}
	else if ( lpObj->Class == CLASS_ELF )
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 6 ) / 10;	// #formula
	}
	else if ( lpObj->Class == CLASS_MAGUMSA )
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 35 ) / 10;	// #formula
	}
	else if ( lpObj->Class == CLASS_WIZARD )
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 4 );	// #formula
	}
	else if ( lpTargetObj->Class == CLASS_SUMMONER ) //webzen bug?? wtf??
	{
		iAttackRate = iBaseLevel * 3 + ( lpObj->Dexterity * 35 ) / 10;	// #formula
	}
	...
	{
		iDefenseRate = iTargetBaseLevel * 2 + lpTargetObj->Dexterity / 2;	// #formula
	}
	...
	{
		iDefenseRate = iTargetBaseLevel * 2 + lpTargetObj->Dexterity / 10;	// #formula
	}
	else if ( lpTargetObj->Class == CLASS_MAGUMSA )
	{
		iDefenseRate = iTargetBaseLevel * 2 + lpTargetObj->Dexterity / 4;	// #formula
	}
...
void __cdecl CDarkSpirit::SendLevelmsg(int aIndex, int nPos, int PetType, int InvenType)
{
	...
	pMsg.Life = gObj[aIndex].pInventory[nPos].m_Durability; //season4 add-on (possible fix)
...
int CDarkSpirit::GetShieldDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage)
{
	...
	if ( lpObj->m_JewelOfHarmonyEffect.HJOpDecreaseSDRate || lpTargetObj->m_JewelOfHarmonyEffect.HJOpAddSDRate || lpObj->m_JewelOfHarmonyEffect.HJOpAddIgnoreSDRate )
	{
