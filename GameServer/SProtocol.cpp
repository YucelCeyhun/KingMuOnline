...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg)
{
	...
	switch ( lpMsg->result  )
	{
		case 1:
		case 15:
			if(gObjSetAccountLogin(aIndex, szId,lpMsg->UserNumber,lpMsg->DBNumber,lpMsg->PcBangGuid,szJN,lpMsg->PcBangRoom) == 0 )//Season 4.5 addon
			{
				GJPAccountFail(aIndex, szId, lpMsg->DBNumber , lpMsg->UserNumber  );
				lpMsg->result  = 4;
			}
...
struct PMSG_REQ_MAPSVRAUTH
{
	...
	char szAccountID[11];
	char szCharName[11];
	USHORT wDstMapSvrCode;//WORD - USHORT
	int iJoinAuthCode1;
	int iJoinAuthCode2;
	int iJoinAuthCode3;
	int iJoinAuthCode4;
...
void JGAnsMapSvrAuth(PMSG_ANS_MAPSVRAUTH * lpMsg)
{
	...
	switch ( lpMsg->iResult )
	{
		case 0:
			...
			if(!gObjSetAccountLogin(iIndex,szId,lpMsg->iUserNumber,lpMsg->iDBNumber,lpMsg->PcBangGuid,szJN,lpMsg->PcBangRoom))//Season 4.5 addon
			{
