...
enum CHAOS_TYPE
{
	...
	CHAOS_TYPE_FOURTH_WING_MIX = 0x2E, //season 4.7 add-on
	CHAOS_TYPE_ABSOLUTE_SWORD_MIX = 0x2F, //season 4.9 add-on (Added: 19-09-16)
	CHAOS_TYPE_ABSOLUTE_SCEPTER_MIX = 0x30, //season 4.9 add-on (Added: 08-10-16)
	CHAOS_TYPE_ABSOLUTE_CROSSBOW_MIX = 0x31, //season 4.9 add-on (Added: 08-10-16)
	CHAOS_TYPE_ABSOLUTE_STAFF_MIX = 0x32, //season 4.9 add-on (Added: 08-10-16)
	CHAOS_TYPE_CRYSTAL_FEATHER_MIX = 0x33, //season 4.9 add-on (Added: 17-10-16)
};
...
public:
	...
	void FourthWingMix(LPOBJ lpObj); //Season 4.7 add-on (Custom)
	void FourthWingMixItemDown(LPOBJ lpObj); //Season 4.7 add-on (Custom)
	void FourthWingMixItemDown2(CItem * lpItem); //Season 4.7 add-on (Custom)
	void CrystalFeatherMix(LPOBJ lpObj); //Season 4.7 add-on (Custom)
	void AbsoluteSwordMix(LPOBJ lpObj); //Season 4.9 add-on (Custom) (Added: 19-09-16)
	void AbsoluteScepterMix(LPOBJ lpObj); //Season 4.9 add-on (Custom) (Added: 08-10-16)
	void AbsoluteCrossbowMix(LPOBJ lpObj); //Season 4.9 add-on (Custom) (Added: 08-10-16)
	void AbsoluteStaffMix(LPOBJ lpObj); //Season 4.9 add-on (Custom) (Added: 08-10-16)
...
private:
	...
	// ----
	// +Luck Success Rate
	int i3rdWingLuckRate;
	int i4thWingLuckRate;
	// ----
	// Normal Success Rate
	int i3rdWingRate;
	int i4thWingRate;
	int iAbsoluteSwordRate;
	int iAbsoluteScepterRate;
	int iAbsoluteCrossbowRate;
	int iAbsoluteStaffRate;
	int iCrystalFeatherRate;
	// ----
	// Normal Success Rate (Custom Pets)
	int iDarkWolfRate;
	int iSableFangRate;
};
