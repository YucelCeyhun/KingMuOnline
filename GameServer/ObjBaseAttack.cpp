...
int CObjBaseAttack::GetTargetDefense(LPOBJ lpObj, LPOBJ lpTargetObj, BYTE& MsgDamage) //Identical gs-cs 56
{
	...
	if ( lpObj->pInventory[7].IsItem() != FALSE )
	{
		...
		percentdamage += lpObj->pInventory[7].IsSeason7WingOpGetOnePercentDamage();
		percentdamage += lpObj->pInventory[7].IsFourthWingOpGetOnePercentDamage();
		percentdamage += lpObj->pInventory[7].IsSpecialWingOpGetOnePercentDamage();
		percentdamage += lpObj->pInventory[7].IsSeason8WingOpGetOnePercentDamage();
	}
