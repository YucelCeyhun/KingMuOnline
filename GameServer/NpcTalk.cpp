...
BOOL NpcEventChipNPC(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if ( lpNpc->MapNumber == 0 ) // Golden Archer in Lorencia
	{
	...
	if ( lpNpc->MapNumber == 2 ) // Golden Archer in Devias
	{
	...
	if (lpNpc->MapNumber == 3) // Golden Archer in Noria
	{
		PMSG_REQ_VIEW_EC_MN pMsgTha;
		// ----
		PHeadSetB((LPBYTE)&pMsgTha, 0x01, sizeof(pMsgTha));
...
BOOL NpcDarkSpiritTrainer(LPOBJ lpNpc, LPOBJ lpObj) //todo
{
	if ( bCanChaosBox == TRUE )
	{
		PMSG_TALKRESULT pMsg;
