...
{
	this->Load(gDirPath.GetNewPath("IllusionTemple.dat"));
...
BOOL CIllusionTempleEvent::IllusionTempleAddUser(int aIndex, BYTE FloorIndex, BYTE TicketPos)
{
	...
	if(loc8 == 1)
	{
		...
		LogAddTD("[Illusion Temple] (%d) EnterUser: (%s)(%s) class:%d (Serial:%u). PCRoom:%d", loc6, lpObj->AccountID, lpObj->Name, lpObj->DbClass, lpObj->pInventory[TicketPos].m_Number, lpObj->m_iPcBangRoom); //season4 changed
		DataSend(aIndex,(LPBYTE)&pResult,pResult.h.size);
...
void CIllusionTempleEvent::EGAnsIllusionTempleEnter(PMSG_ANS_ILLUSIONTEMPLE_ENTER_RESULT * lpMsg)
{
	...
	if(loc8 == 1)
	{
		...
		LogAddTD("[Illusion Temple] (%d) EnterUser: (%s)(%s) class:%d (Serial:%u). PCRoom:%d", loc6, lpObj->AccountID, lpObj->Name, lpObj->DbClass, lpObj->pInventory[lpMsg->iPos].m_Number, lpObj->m_iPcBangRoom); //season4 changed
		DataSend(lpMsg->Index,(LPBYTE)&pMsg,pMsg.h.size);
...
BOOL CIllusionTempleEvent::CheckWearingMOPH(int iUserIndex)
{
	...
	if ( gObj[iUserIndex].pInventory[10].IsItem()  )
	{
		...
		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,76) ) //season4.6 add-on
		{
			return TRUE;
		}
	}
	...
	if ( gObj[iUserIndex].pInventory[11].IsItem() )
	{
		...
		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,76) ) //season4.6 add-on
		{
			return TRUE;
		}
	}
