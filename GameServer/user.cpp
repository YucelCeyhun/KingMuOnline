...
void gObjSkillUseProcTime500(LPOBJ lpObj) //
{
	if ( lpObj->SkillHellFire2State != 0 )
	{
		...
		else
		{
			...
			mana -= mana * lpObj->m_MLPassiveSkill.m_iML_IncreaseManaReduction / 100; //season4 add-on
...
void gObjInit()
{
	...
	for ( int n = 0; n< OBJMAX ; n++)
	{
		...
		gObj[n].m_btSummonedbyGM = 0; //season4 add-on
	}
...
void gObjCharZeroSet(int aIndex)
{
	...
	lpObj->m_iSummonerBookSpell = 0; //S3 Addition
...
#if(ENABLE_CUSTOM_OPTMIZE == FALSE)
	...
	g_MasterSkillSystem.ClearPassiveSkill(lpObj); //Season3 add-on
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, int ItemDbByte, bool* bAllItemExist)
{
	...
	for(n = 0; n < INVENTORY_SIZE;n++)
	{
		...
		if(itype != -1)
		{
			...
			{
				if(item.m_Level > MAX_ITEM_LEVEL) // +14 & +15 Item Level Fix
				{
					item.m_Level = 0;
				}
			}
			...
			if(ItemDbByte == 7)
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,0,0,0,NULL, 0xFF, 3); //season4 changed
			}
			else if(ItemDbByte <= 10)
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+7]),lpMsg->dbInventory[n*ItemDbByte+8],item.m_ItemOptionEx,SocketOption, SocketOptionIndex, 1); //season4 changed
			}
			else
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+7]),lpMsg->dbInventory[n*ItemDbByte+8],item.m_ItemOptionEx,SocketOption, SocketOptionIndex, 3); //season4 changed
			}
			...
			lpObj->pInventory[n].m_SocketOptionIndex = item.m_SocketOptionIndex; //season4 add-on
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if( lpObj->MapNumber == MAP_INDEX_SANTATOWN && lpObj->Level < 15) //season4 add-on
	{
		if ( lpObj->Class == CLASS_ELF )
		{
			lpObj->MapNumber = MAP_INDEX_NORIA;
	...
	{
		lpObj->MapNumber = MAP_INDEX_ELBELAND; //season3 changed
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}
	...
	if( g_QuestInfo.GetQuestState(lpObj, 2) == 2 ) // Marlon Quest Fix (Stat)
	{
		lpObj->PlusStatQuestClear = true;
	}

	if( g_QuestInfo.GetQuestState(lpObj, 3) == 2 ) // Marlon Quest Fix (Combo)
	{
	...
	if ( lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_SUMMONER || lpObj->Class == CLASS_MAGUMSA )
	{
		...
		for ( n=0;n<MAX_MAGIC;n++)
		{
			...
			if ( itype != 0xFF )
			{
				if(g_MasterSkillSystem.CheckRequireStatus(itype) != FALSE) //Season3 add-on
				{
					BYTE level = lpMsg->dbMagicList[n*3+1]>>3;

					if(gObjMagicAdd(lpObj, itype, level) > -1)
					{
						g_MasterSkillSystem.AddPassiveSkill(lpObj, itype, level);
					}
				}
				else
				{
	...
	if ( (lpMsg->CtlCode & 0x20 ) == 0x20 )
	{
		...
		LogAddC(2, "(%s)(%s) The Game Master has entered the server.", lpObj->AccountID, lpObj->Name);
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 1, "없는 아이템 발견 !!!");
		...
		if ( lpObj->pInventory[n].IsItem() == TRUE )
		{
			//if ( lpObj->pInventory[n].m_serial != 0 ) //Season 2.5 removed
			{
				...
				LogAddTD(lMsg.Get(MSGGET(1, 248)), lpObj->AccountID, lpObj->Name, n, lpObj->pInventory[n].GetName(),
					...
					lpObj->pInventory[n].m_SocketOption[4], lpObj->pInventory[n].m_SocketOptionIndex);
			}
		}
	}
		...
		if ( lpObj->pWarehouse[n].IsItem() == TRUE )
		{
			//if ( lpObj->pWarehouse[n].m_serial != 0 ) //Season 2.5 removed
			{
				...
				LogAddTD(lMsg.Get(MSGGET(1, 250)), lpObj->AccountID, lpObj->Name, n, lpObj->pWarehouse[n].GetName(),
					...
					lpObj->pWarehouse[n].m_SocketOption[4], lpObj->pWarehouse[n].m_SocketOptionIndex);
			}
		}
	}
...
BOOL gObjSetMonster(int aIndex, int MonsterClass)
{
	...
	switch(MonsterClass)
	{
		...
		case 239://Stadium Guard
			lpObj->ShopNumber = 19;
			break;

		case 253://Potion Girl
			lpObj->ShopNumber = 5;
			break;
...
short gObjDel(int index)
{
	...
	lpObj->m_bSkillKeyRecv = 0; //Season 2.5 add-on
	lpObj->m_btSummonedbyGM = 0; //Season 4.5 add-on
	return 1;
}
...
BOOL gObjSetAccountLogin(int aIndex,char*szId,int aUserNumber,int aDBNumber,int PcBangGuid,char*JoominNumber,BYTE PcBangRoom)
{
	...
	gObj[aIndex].PcBangGuid = PcBangGuid;
...
BOOL gObjBackSpring(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	int EffectID = -1;
	for(int i=0;i<lpObj->m_btViewStateCount;i++) //season4 add-on
	{
		EffectID = lpObj->m_BuffEffectState[i].btBuffIndex;
...
{
	if(g_MasterLevelSystem.MasterLevelUp(lpObj, addexp) != 0) //season3 add-on
	{
		return false;
	}
	...
	{
		if(gObjCheckIsInPCBang(lpObj) != FALSE)
		{
	...
	if(lpObj->Class == CLASS_WIZARD)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForDW;
	}
	else if(lpObj->Class == CLASS_KNIGHT)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForDK;
	}
	else if(lpObj->Class == CLASS_ELF)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForFE;
	}
	else if(lpObj->Class == CLASS_MAGUMSA)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForMG;
	}
	else if(lpObj->Class == CLASS_DARKLORD)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForDL;
	}
	else if(lpObj->Class == CLASS_SUMMONER)
	{
		lpObj->LevelUpPoint += g_Configs.LevelUpPointsForSM;
	}
	else
	{
...
BOOL checkMaxStat(BYTE type,LPOBJ lpObj,int addStats){
	switch ( type )
	{
		case 0:
			return lpObj->Strength+addStats <= MAX_STATS;
			break;
		case 1:
			return lpObj->Dexterity+addStats <= MAX_STATS;
			break;
		case 2:
			return lpObj->Vitality+addStats <= MAX_STATS;
			break;
		case 3:
			return lpObj->Energy+addStats <= MAX_STATS;
			break;
		case 4:
			return lpObj->Leadership+addStats <= MAX_STATS;
			break;
	}
}
...
BYTE gObjWarehouseInsertItem(LPOBJ lpObj, int type, int index, int level, int pos)
{
	...
	item.m_Level =level;
	item.Convert(item_type, 0 , 0, 0, 0, 0,0, NULL, 0xFF, CURRENT_DB_VERSION);
...
BOOL retResistance(LPOBJ lpObj, int Resistance_Type)
{
	...
	if ( r > 0 && (gObjSearchActiveEffect(lpObj, AT_POTION_OF_SOUL) == 1) && (Resistance_Type == 2 || Resistance_Type == 0 ))
	{
		r += r * 50 / 100;
	}
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(gObjSearchActiveEffect(lpObj, AT_CSHOP_GUARDIAN_AMULET) != 0) //season4.5 add-on
	{
		return;
	}
	...
	{
		minexp = g_MasterLevelSystem.GetLevelExperienceTable(lpObj->MLevel);
		maxexp = g_MasterLevelSystem.GetLevelExperienceTable(lpObj->MLevel+1);

		decexprate = g_MasterLevelSystem.CheckMasterLevelCharacterPkLevel(lpObj);
	}
	...
	if(lpObj->Money > 0)
	{
		iMoneyLevel = g_MasterLevelSystem.CheckMasterLevelCharacterMoneyLevel(lpObj);
...
BOOL gObjHorrocruxFireSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( lpObj->pInventory[1].IsItem() == FALSE )
	{
		return FALSE;
	}

	CItem * Horrocrux = &lpObj->pInventory[1];

	if ( Horrocrux->m_Type == ITEMGET(13,140) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjHorrocruxWaterSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Horrocrux->m_Type == ITEMGET(13,141) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjHorrocruxEarthSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Horrocrux->m_Type == ITEMGET(13,142) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjHorrocruxWindSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Horrocrux->m_Type == ITEMGET(13,143) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjHorrocruxThunderSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Horrocrux->m_Type == ITEMGET(13,144) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjHorrocruxIceSprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Horrocrux->m_Type == ITEMGET(13,145) && Horrocrux->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjSparkySprite(LPOBJ lpObj) // Season 4.9 Add-on
{
	if (lpObj->Type != OBJ_USER)
	{
		return FALSE;
	...
	CItem * Sparky = &lpObj->pInventory[8];

	if (Sparky->m_Type == ITEMGET(13,79) && Sparky->m_Durability > 0.0f)
	{
		return TRUE;
	}
...
BOOL gObjSkeletonSprite(LPOBJ lpObj) // Season 4.9 Add-on
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Skeleton->m_Type == ITEMGET(13,77) && Skeleton->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjUnicornSprite(LPOBJ lpObj) // Season 4.9 Add-on
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	if ( Unicorn->m_Type == ITEMGET(13,78) && Unicorn->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjFairySprite(LPOBJ lpObj)
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
	...
	CItem * Fairy = &lpObj->pInventory[8];

	if ( Fairy->m_Type == ITEMGET(13,65) && Fairy->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ( (Wing->m_Type >= ITEMGET(12,0) && Wing->m_Type <= ITEMGET(12,6)) || Wing->m_Type <= ITEMGET(13,30) || 
		 ...
		 (Wing->m_Type >= ITEMGET(12,240) && Wing->m_Type <= ITEMGET(12,255)) || //Season 4.7 All Wings
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	else if ( sprite->m_Type == ITEMGET(13,123) ) //skeleton
	{
		fdamage = (damage*3)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	if ( sprite->m_Type == ITEMGET(13,64) || //season4 add-on 
		...
		sprite->m_Type == ITEMGET(13,106) || //4.9 unicorn
		sprite->m_Type == ITEMGET(13,110) || //4.9 dragon
		sprite->m_Type == ITEMGET(13,77) ) //4.9 skeleton
	{
		fN += 10.0f;
	}

	if ( sprite->m_Type == ITEMGET(13,67) // Ruloph (Season 4.5 add on)
		|| sprite->m_Type == ITEMGET(13,64) ) // Demon (Season 3.5 add on)
	{
		fdamage = (damage*3)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	else if (sprite->m_Type == ITEMGET(13, 106)) //4.9 unicorn
	{
		fdamage = (damage * 2) / 10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	else if (sprite->m_Type == ITEMGET(13, 110)) //4.9 dragon
	{
		fdamage = (damage * 2) / 10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	else if (sprite->m_Type == ITEMGET(13, 123)) //4.9 skeleton
	{
		fdamage = (damage * 2) / 10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	else if ( sprite->m_Type == ITEMGET(13,4) )
	{
		...
		if ( sprite->m_DurabilitySmall > lpObj->m_MLPassiveSkill.m_iML_DecreasePetDurReduction + 1500 )
		{
			...
			if ( sprite->m_Durability < 1.0f )
			{
				...
				LogAddTD("[%s][%s] 다크호스의뿔 Item is Broken because durability is exhausted [%u]", lpObj->AccountID,lpObj->Name,lpObj->pInventory[8].m_Number);
			}
		...
		if ( sprite->m_Durability < 1.0f )
		{
			...
			if ( BC_MAP_RANGE(lpObj->MapNumber) || CC_MAP_RANGE(lpObj->MapNumber) )
			{
				if ( itemnum == ITEMGET(13,0) || itemnum == ITEMGET(13,1) || 
					 ...
					 itemnum == ITEMGET(13,77) || itemnum == ITEMGET(13,78) )
				{
					int pos = -1;
...
void gObjSecondDurDown(LPOBJ lpObj)
{
	...
	if((attr&1)==0)
	{
		if(lpObj->pInventory[10].IsItem() == 1)
		{
			...
			{
				if(gObjCheckIsInPCBang(lpObj) != FALSE) //season4.5 add-on
				{
...
void gObjWeaponDurDown(LPOBJ lpObj, LPOBJ lpTargetObj, int type)
{
	...
	if(gObjSearchActiveEffect(lpObj, AT_CSHOP_PROTECT_AMULET) != 0) //season4.5 add-on
	{
		return;
	}
	...
	if(type == 0)
	{
		...
		if((lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_DARKLORD) && (Right->m_Type >= 0 && Right->m_Type < ITEMGET(4,0) && Left->m_Type >= 0 && Left->m_Type < ITEMGET(4,0)))
		{
			if(rand()%2==0)
			{
			...
			Left->m_Type == ITEMGET(4,22) || Left->m_Type == ITEMGET(4,23) || Left->m_Type == ITEMGET(4,24) || //season 4.6 add-on
			Left->m_Type >= ITEMGET(4,25) && Left->m_Type <= ITEMGET(4,128)) //season 4.7 add-on
		{
...
{
	return gclassObjAttack.Attack(lpObj, lpTargetObj, lpMagic, magicsend, MSBFlag, AttackDamage, bCombo); //todo
}
			...
			if(lpMonObj->sHD[n].HitDamage > 0)
			{
				...
				if(bSendExp == 1)
				{
					if(lpTargetObj->m_Index == lpObj->m_Index)
					{
						...
						if(lpObj->Level <= 150) //season 4.5 add-on
						{
							if(gObjCheckIsInPCBang(lpObj) != FALSE)
							{
								lpMonObj->Money += exp;
...
{
	if(g_MasterLevelSystem.CheckMinMonsterLevel(lpObj, lpTargetObj) == 0) //Season3 add-on
	{
		bSendExp = 0;
		return 0;
	}
	...
	if(exp > 0)
	{
		if(lpObj->Type == OBJ_USER)
		{
			gObjSealUserSetExp(lpObj, exp, FALSE); //Seal Exp (Season3 add-on)
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	for(n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		...
		if(number >= 0)
		{
			...
			{
				if(lpPartyObj->Level+lpPartyObj->MLevel > toplevel)
				{
					toplevel = lpPartyObj->Level+lpPartyObj->MLevel;
				}
			}
		}
	}
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber)
			{
				...
				if(dis[n] < 10)
				{
					...
					if(toplevel >= (lpPartyObj->Level+lpPartyObj->MLevel+200))
					{
						totallevel += lpPartyObj->Level+lpPartyObj->MLevel+200;
					}
					else
					{
						totallevel += lpPartyObj->Level+lpPartyObj->MLevel;
					}
					viewplayer += 1;
	...
	if(lpTargetObj->Level >= 65)
	{
		...
		{
			level += (200.0 - ((lpObj->Level + lpObj->MLevel) * 0.2));
		}
	}
	...
	for(n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				if(g_MasterLevelSystem.CheckMinMonsterLevel(lpPartyObj, lpTargetObj) == 0) //Season3 add-on
				{
					exp = 0;
				}
				else
				...
				if(exp > 0)
				{
					...
					{
						gObjSealUserSetExp(lpPartyObj, exp, FALSE); //Seal Exp (Season3 add-on)
				...
				if(lpPartyObj->Type == OBJ_USER && g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpPartyObj) == FALSE)
				{
...
void gObjMonsterDieLifePlus(LPOBJ lpObj, LPOBJ lpTartObj)
{
	...
	{
		lpObj->Life = lpObj->Life + lpObj->m_wSocketOpAddLife;
	}

	if(lpObj->m_wSocketOpAddMana > 0) //season4 add-on
	{
		lpObj->Mana = lpObj->Mana + lpObj->m_wSocketOpAddMana;
	}

	if(lpObj->m_MLPassiveSkill.m_iML_MonsterAttackManaIncrement > 0) //Season3 add-on
	{
		lpObj->Mana = lpObj->Mana + (lpObj->MaxMana + lpObj->AddMana)/lpObj->m_MLPassiveSkill.m_iML_MonsterAttackManaIncrement;
	}

	if(lpObj->m_MLPassiveSkill.m_iML_MonsterAttackLifeIncrement > 0) //Season3 add-on
	{
		lpObj->Life = lpObj->Life + (lpObj->MaxLife + lpObj->AddLife)/lpObj->m_MLPassiveSkill.m_iML_MonsterAttackLifeIncrement;
	}

	if(lpObj->m_MLPassiveSkill.m_iML_MonsterAttackSDIncrement > 0) //Season3 add-on
	{
		lpObj->iShield = (float)lpObj->iShield + (lpObj->iMaxShield + lpObj->iAddShield)/lpObj->m_MLPassiveSkill.m_iML_MonsterAttackSDIncrement;
	}

	if(lpObj->m_MLPassiveSkill.m_iML_MonsterAttackSDIncrement > 0) //Season4 add-on
	{
	...
	if(lpObj->MonsterDieGetMana != 0
		//season4 add-on
		|| lpObj->m_MLPassiveSkill.m_iML_MonsterAttackManaIncrement > 0
		|| lpObj->m_wSocketOpAddMana > 0)
	{
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, WORD Skill, int iShieldDamage)
{
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
			...
			{
				int iTargetBridgeIndex = g_BloodCastle.GetBridgeIndex(lpTargetObj->MapNumber);
		...
		for(w = 0; w < 8; w++)
		{
			if(*(BYTE*)(gObj[aIndex].pInventoryMap+h*8+w) == 255)
			{
				...
				if(blank != 255)
				{
					...
					copyitem.Convert(item->m_Type,item->m_Option1,item->m_Option2,item->m_Option3,item->m_NewOption,item->m_SetOption,item->m_ItemOptionEx,item->m_SocketOption, item->m_SocketOptionIndex, CURRENT_DB_VERSION);
...
BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck)
{
	...
	if(pos < 12)
	{
		...
		if(lpItem->m_Option1)
		{
			int result = gObjWeaponMagicAdd(lpObj,(BYTE)lpItem->m_Special[0],lpItem->m_Level);
...
BYTE gObjInventoryMoveItem(int aIndex, BYTE source, BYTE target, int& durSsend, int& durTsend, BYTE sFlag, BYTE tFlag, LPBYTE siteminfo)
{
	...
	__try
	{
				...
				else
				{
					if(sitem->m_Level < 4 && sitem->m_Option3*4 < 4)
					{
						if( sitem->m_Type == ITEMGET(12,15) || 
							sitem->m_Type == ITEMGET(14,13) || 
							sitem->m_Type == ITEMGET(14,14) || 
							sitem->m_Type == ITEMGET(14,27)	|| //?? add-on Ancient Metal
							sitem->m_Type == ITEMGET(14,170) || //Season 4.7 add-on (Crystal Feather)
							sitem->m_Type == ITEMGET(14,172) || //Season 4.9 add-on (Divine Stone)
							sitem->m_Type == ITEMGET(14,173) || //Season 4.9 add-on (Jewel of Dark Chaos)
							sitem->m_Type == ITEMGET(12,150) || //Season 4.9 add-on (Bundled Level)
							sitem->m_Type == ITEMGET(12,151) || //Season 4.9 add-on (Bundled Luck)
							sitem->m_Type == ITEMGET(14,241) || //Season 4.7 add-on (Jewel of Luck)
							sitem->m_Type == ITEMGET(14,242) || //Season 4.7 add-on (Jewel of Skill)
							sitem->m_Type == ITEMGET(14,243) || //Season 4.7 add-on (Jewel of Option)
							sitem->m_Type == ITEMGET(14,22) || 
						...
						else if(sitem->m_Type == ITEMGET(12,41)) //Season 3 add-on (summoner wing level 1)
						{
						...
						else if(sitem->m_Type == ITEMGET(12,30) || sitem->m_Type == ITEMGET(12,31)) //season 2.5 add-on
						{
						...
						else if(sitem->m_Type == ITEMGET(14,96)) //Talisman of Chaos Assembly (Season3.5 addon)
						{

						}
						else if(sitem->m_Type == ITEMGET(13,49) || sitem->m_Type == ITEMGET(13,50)) //Season 2.5 add-on
						{
							
						}
						else if(g_MixSystem.IsCondorItemCheck(sitem->m_Type) == 1) //Season 2.5 add-on
						{

						}
						else if( //Season4.5 add-on
							sitem->m_Type == ITEMGET(13,83) || 
							sitem->m_Type == ITEMGET(13,84) ||
							sitem->m_Type == ITEMGET(13,85) ||
							sitem->m_Type == ITEMGET(13,86) ||
		...
		if(titem->IsItem() == 1)
		{
			...
			if(sitem->m_Type == ITEMGET(14,100)) //season4.5 add-on (lucky coin)
			{
			...
			//Elite Potions
			else if((sitem->m_Type == ITEMGET(14,70) || sitem->m_Type == ITEMGET(14,71) || sitem->m_Type >= ITEMGET(14,94)) && titem->m_Durability < 50.0f)
			{
				max_count = 50;
			}
			//Cherry Blossom Mix
			...
			else if((sitem->m_Type == ITEMGET(14,89)) && titem->m_Durability < 30.0f) //Season 3 add-on
			{
				max_count = 30;
			}
			else if((sitem->m_Type == ITEMGET(14,90)) && titem->m_Durability < 50.0f) //Season 3 add-on
			{
				max_count = 50;
			}
			//Cherry Blossom Effect Item
			else if((sitem->m_Type == ITEMGET(14,85)) && titem->m_Durability < 3.0f) //Season 3 add-on
			{
				max_count = 3;
			}
			else if((sitem->m_Type == ITEMGET(14,86)) && titem->m_Durability < 3.0f) //Season 3 add-on
			{
				max_count = 3;
			}
			else if((sitem->m_Type == ITEMGET(14,87)) && titem->m_Durability < 3.0f) //Season 3 add-on
			{
				max_count = 3;
			}
		...
		case 4:
			...
			if(target < INVETORY_WEAR_SIZE)
			{
				...
				if(lpItem->m_Option1)
				{
					int s_pos = gObjWeaponMagicAdd(&gObj[aIndex],(BYTE)lpItem->m_Special[0],lpItem->m_Level);
					if(s_pos >= 0)
					{
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pChaosBox[target].m_Type == ITEMGET(13,76)) //Season 4.6 add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,76)) //Season 4.6 add-on
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
...
BYTE gObjTradeTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	int h,w;
	int iwidth,iheight;
	BYTE TempTradeMap[TRADE_BOX_MAP_SIZE];				// 교환창 체크용 버퍼
	int blank;
...
void gObjMakePreviewCharSet(int aIndex)
{
	...
	if ( lpObj->pInventory[8].m_Type != MAKE_ITEMNUM(13,4) )
	{
		...
		else if( lpObj->pInventory[8].m_Type != MAKE_ITEMNUM(13,67) ) // -> Fix Rudolf
		{
	...
	if ( lpObj->pInventory[8].m_Type & 0x03 && lpObj->pInventory[8].m_Type > 0) // -> Dinorant
	{
	...
	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,97) ) // -> Dark Wolf
	{
	...
	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,37) ) // -> Fenrir
	{
		...
		if ( lpObj->pInventory[8].IsFenrirEvolve() != FALSE )
		{
			lpObj->CharSet[17] |= 0x02;
		}
	}

	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,98) ) // -> Sable Fang
	{
		lpObj->CharSet[5] |= CS_SET_HELPER(-1);
		lpObj->CharSet[10] &= (BYTE)0xFE;
		lpObj->CharSet[12] &= (BYTE)0xFE;
		lpObj->CharSet[12] |= (BYTE)0x08;
	}

	/*
	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,99) ) // -> Mistic Tiger
	{
		//lpObj->CharSet[5] |= CS_SET_HELPER(-1);
		//lpObj->CharSet[10] &= (BYTE)0xFE;
		//lpObj->CharSet[12] &= (BYTE)0xFE;
		lpObj->CharSet[12] |= (BYTE)0x10;
	}

	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,100) ) // -> Giant Lyon
	{
		//lpObj->CharSet[5] |= CS_SET_HELPER(-1);
		//lpObj->CharSet[10] &= (BYTE)0xFE;
		//lpObj->CharSet[12] &= (BYTE)0xFE;
		lpObj->CharSet[12] |= (BYTE)0x40;
	}

	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,101) ) // -> Lycan Wolf
	{
		//lpObj->CharSet[5] |= CS_SET_HELPER(-1);
		//lpObj->CharSet[10] &= (BYTE)0xFE;
		//lpObj->CharSet[12] &= (BYTE)0xFE;
		lpObj->CharSet[12] |= (BYTE)0x80;
	}
	*/

	/*if ( lpObj->pInventory[8].m_Type >= ITEMGET(13,99) && lpObj->pInventory[8].m_Type <= ITEMGET(13,255) )
	{
		lpObj->CharSet[5] |= CS_SET_HELPER(-1);
		lpObj->CharSet[10] &= (BYTE)0xFE;
		lpObj->CharSet[12] &= (BYTE)0xFE;
	...
	if ((lpObj->pInventory[7].m_Type >= ITEMGET(12,236) && lpObj->pInventory[7].m_Type <= ITEMGET(12,255)))	// Fix Wings 2.5 + Wings 4 + Golden Fenrir
	{
		lpObj->CharSet[5] |= CS_SET_WING1(3);
		lpObj->CharSet[17] |= ((lpObj->pInventory[7].m_Type - 236) + 1 << 2);
	}
	...
	{
		case ITEMGET(13,64):	// -> Fairy Pet
			btNewValue = (BYTE)(0x01 << 5);
		break;
		case ITEMGET(13,65):	// -> Demon Pet
			btNewValue = (BYTE)(0x02 << 5);
		break;
		case ITEMGET(13,77):	// -> Skeleton Pet
			btNewValue = (BYTE)(0x03 << 5);
		break;
		case ITEMGET(13,67):	// -> Rudolf Pet
			btNewValue = (BYTE)(0x04 << 5);
		break;
		case ITEMGET(13,80):	// -> Panda Pet
			btNewValue = (BYTE)(0x07 << 5);
		break;
		case ITEMGET(13,78):	// -> Unicorn Pet
			btNewValue = (BYTE)(0x05 << 5);
		break;
		case ITEMGET(13,79):	// -> Sparky Pet
			//lpObj->CharSet[5] |= CS_SET_HELPER(-1);
			lpObj->CharSet[10] &= (BYTE)0xFE;
			// ----
			btNewValue = (BYTE)(0x06 << 5);
		break;
	}
...
void gObjViewportPaint(HWND hWnd, short aIndex)
{
	...
	wsprintf(szTemp, "Monsters: (%d/9000) || Players: (%d/200)", count, totalplayer);
...
void gObjViewportListCreate(short aIndex)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		...
		lpObj->Type == OBJ_MONSTER
#if(GS_CASTLE==1)
		|| lpObj->Type == OBJ_NPC || lpObj->Type == OBJ_NPC
#endif
		)
	{
...
void gObjSetState()
{
	...
	for(n = 0; n < OBJMAX; n++)
	{
		...
		if(lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if(lpObj->DieRegen == 2)
			{
				if(lpObj->m_btSummonedbyGM == 1) //season4.5 add-on
				{
					gObjDel(lpObj->m_Index);
					continue;
				}
				...
				if(State != 0)
				{
					...
					if(lpObj->Type == OBJ_USER)
					{
						...
						else if(lpObj->MapNumber == MAP_INDEX_RAKLION || lpObj->MapNumber == MAP_INDEX_RAKLION_BOSS) //season4 add-on
						{
							int mgt = 286;
							short x,y,level;
							BYTE map = lpObj->MapNumber,dir;
...
void gObjManaPotionFill(LPOBJ lpObj)
{
	...
	if(lpObj->m_ManaFillCount > 2)
	{
		...
		if(lpObj->MaxMana + lpObj->AddMana > lpObj->Mana)
		{
			...
			{
				loc4 = gObjGetActiveEffect(lpObj, ADD_OPTION_MANA_AUTO_RECOVERY); //season3 add-on
			...
			if(lpObj->m_wSocketOpIncMaxMana > 0) //season4 add-on
			{
				tmana += lpObj->m_wSocketOpIncMaxMana;
			}
		...
		if(lpObj->BP < BP)
		{
			...
			if(gObjSearchActiveEffect(lpObj, AT_XMASS_AG_RECOVERY) == 1) //season4 add-on
			{
				rBP = 5;
			...
			if(gObjSearchActiveEffect(lpObj, AT_POTION_OF_SOUL) == 1)
			{
				AddBP += 8;
			}
...
void gObjRestPotionFill(LPOBJ lpObj)
{
	...
	if(lpObj->m_Rest != 0)
	{
		...
		if(lpObj->m_LifeFillCount > 4)
		{
			...
			if(percent > 0)
			{
				if(lpObj->m_MLPassiveSkill.m_iML_HPAutoIncrement > 0)
				{
					percent += lpObj->m_MLPassiveSkill.m_iML_HPAutoIncrement;
				}
			}

			loc4 = gObjGetActiveEffect(lpObj, ADD_OPTION_LIFE_AUTO_RECOVERY);
			...
			if(lpObj->m_wSocketOpIncLifePower > 0) //season4 add-on
			{
				tlife += lpObj->m_wSocketOpIncLifePower;
			}
			...
			loc6 = gObjGetActiveEffect(lpObj, ADD_OPTION_MANA_AUTO_RECOVERY);
	...
	else
	{
		...
		if(!(BC_MAP_RANGE(lpObj->MapNumber) || 
			...
			lpObj->MapNumber == MAP_INDEX_CASTLEHUNTZONE))
		{
			tlife1 = gObjGetActiveEffect(lpObj, ADD_OPTION_LIFE_AUTO_RECOVERY);
		...
		if(percent1 != 0)
		{
			if(lpObj->m_MLPassiveSkill.m_iML_HPAutoIncrement > 0)
			{
				percent1 += lpObj->m_MLPassiveSkill.m_iML_HPAutoIncrement;
			}
			...
			if(lpObj->m_LifeFillCount > 6)
			{
				...
				if(lpObj->m_wSocketOpIncLifePower > 0) //season4 add-on
				{
					tlife2 += lpObj->m_wSocketOpIncLifePower;
				}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		if(lpObj->m_Change >= 0)
		{
			...
			iViewSkillCount = gObjCountAppliedBuffEffect(lpObj, pViewportCreateChange.btViewSkillState); //Season3
		...
		else
		{
			...
			iViewSkillCount = gObjCountAppliedBuffEffect(lpObj, pViewportCreate.btViewSkillState); //Season3
	...
	else
	{
		...
		if(lpObj->m_RecallMon >= 0)
		{
			...
			iViewSkillCount = gObjCountAppliedBuffEffect(lpObj, pCallMonsterViewportCreate.btViewSkillState); //Season3
		...
		else
		{
			...
			iViewSkillCount = gObjCountAppliedBuffEffect(lpObj, pMonsterViewportCreate.btViewSkillState); //Season3
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	if(BC_MAP_RANGE(mapNumber) || CC_MAP_RANGE(mapNumber) || DS_MAP_RANGE(mapNumber)
		|| IT_MAP_RANGE(mapNumber)) //season 4 add-on (fix)
	{
		if(gObjSearchActiveEffect(lpObj, AT_NPC_HELP) == 1)
		{
...
int	gObjMagicManaUse(LPOBJ lpObj, CMagicInf * lpMagic)
{
	...
	loc2 -= lpObj->m_MLPassiveSkill.m_iML_IncreaseManaReduction * loc2 / 100.0f;
...
int  gObjMagicBPUse(LPOBJ lpObj, CMagicInf * lpMagic)
{
	...
	{
		loc2 -= loc2 * lpObj->m_btSocketOpDecreaseAG / 100;
	}
...
int gObjMagicAdd(LPOBJ lpObj, BYTE Type, BYTE Index, BYTE Level, BYTE & SkillNumber)
{
	...
	if(MagicDamageC.SkillGetRequireClass(lpObj->Class,lpObj->ChangeUP,lpObj->ChangeUP3rd,skill) < 1)
	{
		LogAdd(lMsg.Get(550));
		return -1;
	}

	if(g_MasterSkillSystem.CheckPrimarySkill(lpObj, skill) == 1)
	{
		return -1;
	}
...
{
	if(g_MasterSkillSystem.WeaponMagicDel(lpObj, (BYTE)aSkill, Level) != FALSE) //season4 add-on
	{
		return -1;
	}
...
int gObjWeaponMagicAdd(LPOBJ lpObj, WORD aSkill, BYTE Level)
{
	...
	if(MagicDamageC.SkillGetRequireClass(lpObj->Class,lpObj->ChangeUP,lpObj->ChangeUP3rd,(WORD)aSkill) < 1)
	{
		return -1;
	}

	if(g_MasterSkillSystem.WeaponMagicAdd(lpObj,(BYTE)aSkill, Level) != FALSE) //season 4 add-on
	{
		return -1;
	}
...
BYTE GetAncientOpt(BYTE btMinAncOpt, BYTE btMaxAncOpt)
{
	BYTE btRandOpt = btMinAncOpt + ( rand() % ((btMaxAncOpt + 1) - btMinAncOpt));
	return btRandOpt;
}

BOOL gObjItemAddSocketEx(LPOBJ lpObj, int source, int target) // Jewel of Socket
{
	BYTE btMinSocketSlot = (BYTE)GetPrivateProfileInt("JewelOfSocket","JewelOfSocketMinSlot",1,JEWELS_FILE);
	BYTE btMaxSocketSlot = (BYTE)GetPrivateProfileInt("JewelOfSocket","JewelOfSocketMaxSlot",5,JEWELS_FILE);
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfSocket","JewelOfSocketSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(!( (lpObj->pInventory[target].m_Type < ITEMGET(12,7) || lpObj->pInventory[target].m_Type >=  ITEMGET(12,36)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,43) || lpObj->pInventory[target].m_Type >= ITEMGET(12,240)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,255) || lpObj->pInventory[target].m_Type == ITEMGET(13,30)) &&
	...
	{
		for (int i = btMinSocketSlot; i < btMaxSocketSlot; i++)
		{
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Socket] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemAddAncientEx(LPOBJ lpObj, int source, int target) // Jewel of Ancient
{
	BYTE btMinAncOpt = (BYTE)GetPrivateProfileInt("JewelOfAncient","JewelOfAncientFirstValue",5,JEWELS_FILE);
	BYTE btMaxAncOpt = (BYTE)GetPrivateProfileInt("JewelOfAncient","JewelOfAncientSecondValue",10,JEWELS_FILE);
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfAncient","JewelOfAncientSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(!( (lpObj->pInventory[target].m_Type < ITEMGET(12,7) || lpObj->pInventory[target].m_Type >=  ITEMGET(12,36)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,43) || lpObj->pInventory[target].m_Type >=  ITEMGET(12,240)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,255) || lpObj->pInventory[target].m_Type == ITEMGET(13,30)) ||
		  (lpObj->pInventory[target].m_Type == ITEMGET(13,8)) || (lpObj->pInventory[target].m_Type == ITEMGET(13,9)) || 
		  (lpObj->pInventory[target].m_Type == ITEMGET(13,10)) || (lpObj->pInventory[target].m_Type >= ITEMGET(13,21) && lpObj->pInventory[target].m_Type <= ITEMGET(13,28)) ||
		  (lpObj->pInventory[target].m_Type != ITEMGET(4,7) && lpObj->pInventory[target].m_Type != ITEMGET(4,15)) ))
	{
		return false;
	}

	if(lpObj->pInventory[target].m_SetOption == btMinAncOpt || lpObj->pInventory[target].m_SetOption == btMaxAncOpt)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Ancient] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}

	if ((rand() % 100) < btSuccessRate)
	{
		lpObj->pInventory[target].m_SetOption = GetAncientOpt(btMinAncOpt,btMaxAncOpt);
	}

	gObjMakePreviewCharSet(lpObj->m_Index);

	float ancitemdur = ItemGetDurability(lpObj->pInventory[target].m_Type,lpObj->pInventory[target].m_Level,lpObj->pInventory[target].IsExtItem(),lpObj->pInventory[target].IsSetItem());

	lpObj->pInventory[target].m_Durability = ancitemdur * lpObj->pInventory[target].m_Durability / lpObj->pInventory[target].m_BaseDurability;
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Ancient] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemAddExcellentEx(LPOBJ lpObj, int source, int target) // Jewel of Excellent
{
	BYTE btMaxExcOptions = (BYTE)GetPrivateProfileInt("JewelOfExcellent","JewelOfExcellentMaxOptions",6,JEWELS_FILE);
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfExcellent","JewelOfExcellentSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(lpObj->pInventory[target].m_NewOption > btMaxExcOptions)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Excellent] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}

	if ((rand() % 100) < btSuccessRate)
	{
		lpObj->pInventory[target].m_NewOption = GetRandomExcOption(btMaxExcOptions, FALSE);
	}

	gObjMakePreviewCharSet(lpObj->m_Index);

	float excitemdur = ItemGetDurability(lpObj->pInventory[target].m_Type,lpObj->pInventory[target].m_Level,lpObj->pInventory[target].IsExtItem(),lpObj->pInventory[target].IsSetItem());

	lpObj->pInventory[target].m_Durability = excitemdur * lpObj->pInventory[target].m_Durability / lpObj->pInventory[target].m_BaseDurability;
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Excellent] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemAddOptionEx(LPOBJ lpObj, int source, int target) // Jewel of Option
{
#if(GS_LIFE_28_UP == 1)
	BYTE btMaxOption = 7;
#else
	BYTE btMaxOption = 4;
#endif
	BYTE btTypeOption = (BYTE)GetPrivateProfileInt("JewelOfOption","JewelOfOptionTypeValue",0,JEWELS_FILE);
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfOption","JewelOfOptionSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(lpObj->pInventory[target].m_Option3 > btMaxOption)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Option] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}
	...
	if ((rand() % 100) < btSuccessRate)
	{
		if(btTypeOption == 1)
		{
			lpObj->pInventory[target].m_Option3 = btMaxOption;
		}
		else
		{
			lpObj->pInventory[target].m_Option3++;
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Option] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemAddSkillEx(LPOBJ lpObj, int source, int target) // Jewel of Skill
{
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfSkill","JewelOfSkillSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(lpObj->pInventory[target].m_Option1 == 1)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Skill] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}

	if ((rand() % 100) < btSuccessRate)
	{
		lpObj->pInventory[target].m_Option1 = 1;
	}
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Skill] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemAddLuckEx(LPOBJ lpObj, int source, int target) // Jewel of Luck
{
	BYTE btSuccessRate = (BYTE)GetPrivateProfileInt("JewelOfLuck","JewelOfLuckSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(lpObj->pInventory[target].m_Option2 == 1)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Luck] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Luck] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjItemLevelUpEx(LPOBJ lpObj, int source, int target) // Jewel of Level
{
	BYTE btMinLevel		= (BYTE)GetPrivateProfileInt("JewelOfLevel","JewelOfLevelMinValue",9,JEWELS_FILE);
	BYTE btMaxLevel		= (BYTE)GetPrivateProfileInt("JewelOfLevel","JewelOfLevelMaxValue",15,JEWELS_FILE);
	BYTE btTypeLevel	= (BYTE)GetPrivateProfileInt("JewelOfLevel","JewelOfLevelTypeValue",0,JEWELS_FILE);
	BYTE btSuccessRate	= (BYTE)GetPrivateProfileInt("JewelOfLevel","JewelOfLevelSuccessRate",100,JEWELS_FILE);
	// ----
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	if(!( (lpObj->pInventory[target].m_Type < ITEMGET(12,7) || lpObj->pInventory[target].m_Type >= ITEMGET(12,36)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,43) || lpObj->pInventory[target].m_Type >= ITEMGET(12,240)) &&
		  (lpObj->pInventory[target].m_Type <= ITEMGET(12,255) || lpObj->pInventory[target].m_Type == ITEMGET(13,30)) && 
		  (lpObj->pInventory[target].m_Type == ITEMGET(13,99)) || (lpObj->pInventory[target].m_Type == ITEMGET(13,8)) ||
		  (lpObj->pInventory[target].m_Type == ITEMGET(13,9)) || (lpObj->pInventory[target].m_Type == ITEMGET(13,10)) ||
	...
	if(lpObj->pInventory[target].m_Level < btMinLevel || lpObj->pInventory[target].m_Level > btMaxLevel)
	{
		LogAddC(4, "[ JEWELS ] [Jewel of Level] jewel putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
		return false;
	}
	...
	if ((rand() % 100) < btSuccessRate)
	{
		if(btTypeLevel == 1)
		{
			lpObj->pInventory[target].m_Level = btMaxLevel;
		}
		else
		{
	...
	if(lpObj->pInventory[target].m_Level > btMaxLevel)
	{
		lpObj->pInventory[target].m_Level = btMaxLevel;
	}
	...
	LogAddC(4, "[ JEWELS ] [Jewel of Level] jewel putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
	return true;
}

BOOL gObjDragonHelmEvolveEx(LPOBJ lpObj, int source, int target) // Dragon Helm Upgrading
{
	BYTE btLifeGemFireHelmSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeFireOnHelmSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(7,1)) // -> Dragon Helm
	{
		if ((rand() % 100) < btLifeGemFireHelmSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(7,100),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
		{
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			GCServerMsgStringSend("Gem of Life (Fire) upgrading fail",lpObj->m_Index,1);
			return true;
		}
	}
	else
...
BOOL gObjDragonArmorEvolveEx(LPOBJ lpObj, int source, int target) // Dragon Armor Upgrading
{
	BYTE btLifeGemFireArmorSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeFireOnArmorSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(8,1)) // -> Dragon Armor
	{
		if ((rand() % 100) < btLifeGemFireArmorSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(8,100),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjDragonPantsEvolveEx(LPOBJ lpObj, int source, int target) // Dragon Pants Upgrading
{
	BYTE btLifeGemFirePantsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeFireOnPantsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(9,1)) // -> Dragon Pants
	{
		if ((rand() % 100) < btLifeGemFirePantsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(9,100),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjDragonGlovesEvolveEx(LPOBJ lpObj, int source, int target) // Dragon Gloves Upgrading
{
	BYTE btLifeGemFireGlovesSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeFireOnGlovesSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(10,1)) // -> Dragon Gloves
	{
		if ((rand() % 100) < btLifeGemFireGlovesSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(10,100),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjDragonBootsEvolveEx(LPOBJ lpObj, int source, int target) // Dragon Boots Upgrading
{
	BYTE btLifeGemFireBootsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeFireOnBootsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(11,1)) // -> Dragon Boots
	{
		if ((rand() % 100) < btLifeGemFireBootsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(11,100),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Fire)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjGuardianHelmEvolveEx(LPOBJ lpObj, int source, int target) // Guardian Helm Upgrading
{
	BYTE btLifeGemEarthHelmSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeEarthOnHelmSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(7,14)) // -> Guardian Helm
	{
		if ((rand() % 100) < btLifeGemEarthHelmSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(7,101),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
		{
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			GCServerMsgStringSend("Gem of Life (Earth) upgrading fail",lpObj->m_Index,1);
			return true;
		}
	}
	else
...
BOOL gObjGuardianArmorEvolveEx(LPOBJ lpObj, int source, int target) // Guardian Armor Upgrading
{
	BYTE btLifeGemEarthArmorSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeEarthOnArmorSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(8,14)) // -> Guardian Armor
	{
		if ((rand() % 100) < btLifeGemEarthArmorSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(8,101),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjGuardianPantsEvolveEx(LPOBJ lpObj, int source, int target) // Guardian Pants Upgrading
{
	BYTE btLifeGemEarthPantsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeEarthOnPantsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(9,14)) // -> Guardian Pants
	{
		if ((rand() % 100) < btLifeGemEarthPantsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(9,101),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjGuardianGlovesEvolveEx(LPOBJ lpObj, int source, int target) // Guardian Gloves Upgrading
{
	BYTE btLifeGemEarthGlovesSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeEarthOnGlovesSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(10,14)) // -> Guardian Gloves
	{
		if ((rand() % 100) < btLifeGemEarthGlovesSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(10,101),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjGuardianBootsEvolveEx(LPOBJ lpObj, int source, int target) // Guardian Boots Upgrading
{
	BYTE btLifeGemEarthBootsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeEarthOnBootsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(11,14)) // -> Guardian Boots
	{
		if ((rand() % 100) < btLifeGemEarthBootsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(11,101),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Earth)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjLegendaryHelmEvolveEx(LPOBJ lpObj, int source, int target) // Legendary Helm Upgrading
{
	BYTE btLifeGemWaterHelmSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeWaterOnHelmSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(7,3)) // -> Legendary Helm
	{
		if ((rand() % 100) < btLifeGemWaterHelmSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(7,102),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
		{
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			GCServerMsgStringSend("Gem of Life (Water) upgrading fail",lpObj->m_Index,1);
			return true;
		}
	}
	else
...
BOOL gObjLegendaryArmorEvolveEx(LPOBJ lpObj, int source, int target) // Legendary Armor Upgrading
{
	BYTE btLifeGemWaterArmorSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeWaterOnArmorSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(8,3)) // -> Legendary Armor
	{
		if ((rand() % 100) < btLifeGemWaterArmorSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(8,102),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjLegendaryPantsEvolveEx(LPOBJ lpObj, int source, int target) // Legendary Pants Upgrading
{
	BYTE btLifeGemWaterPantsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeWaterOnPantsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(9,3)) // -> Legendary Pants
	{
		if ((rand() % 100) < btLifeGemWaterPantsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(9,102),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjLegendaryGlovesEvolveEx(LPOBJ lpObj, int source, int target) // Legendary Gloves Upgrading
{
	BYTE btLifeGemWaterGlovesSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeWaterOnGlovesSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(10,3)) // -> Legendary Gloves
	{
		if ((rand() % 100) < btLifeGemWaterGlovesSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(10,102),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjLegendaryBootsEvolveEx(LPOBJ lpObj, int source, int target) // Legendary Boots Upgrading
{
	BYTE btLifeGemWaterBootsSuccessRate = GetPrivateProfileInt("GemLifeSettings","GemOfLifeWaterOnBootsSuccessRate",50,ELEMENTALGEMS_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	gObjMakePreviewCharSet(lpObj->m_Index);
	// ----
	if (lpObj->pInventory[target].m_Type == ITEMGET(11,3)) // -> Legendary Boots
	{
		if ((rand() % 100) < btLifeGemWaterBootsSuccessRate)
		{
			lpObj->pInventory[target].m_Durability = 98.f;
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(11,102),lpObj->pInventory[target].m_Option1 = 0,lpObj->pInventory[target].m_Option2 = 0,lpObj->pInventory[target].m_Option3 = 0,lpObj->pInventory[target].m_NewOption = 0,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ ElementalGemSystem ] [Gem of Life (Water)] gem putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
...
BOOL gObjWingUpgradeEx2(LPOBJ lpObj, int source, int target) // Deathless Amulet
{
	BYTE btDeathlessSuccessRate = GetPrivateProfileInt("MixSettings","DeathlessAmuletSuccessRate",50,MIXSYSTEM_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	if (lpObj->pInventory[target].m_Type == ITEMGET(12,240) || lpObj->pInventory[target].m_Type == ITEMGET(12,241) || lpObj->pInventory[target].m_Type == ITEMGET(12,242) || lpObj->pInventory[target].m_Type == ITEMGET(12,243) || lpObj->pInventory[target].m_Type == ITEMGET(12,244) || lpObj->pInventory[target].m_Type == ITEMGET(12,245))
	{
		if ((rand() % 100) < btDeathlessSuccessRate)
		{
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(12,252),lpObj->pInventory[target].m_Option1,lpObj->pInventory[target].m_Option2,lpObj->pInventory[target].m_Option3,lpObj->pInventory[target].m_NewOption,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ UpgradeWing ] [Deathless Amulet] amulet putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
		{
			LogAddC(4,"[ UpgradeWing ] [Deathless Amulet] amulet putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			GCServerMsgStringSend("Deathless Amulet upgrading fail",lpObj->m_Index,1);
			return true;
		}
	}
	else
...
BOOL gObjWingUpgradeEx(LPOBJ lpObj, int source, int target) // Legend Amulet
{
	BYTE btLegendSuccessRate = GetPrivateProfileInt("MixSettings","LegendAmuletSuccessRate",50,MIXSYSTEM_FILE);
	// ----
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1)
	{
		return false;
	...
	if (lpObj->pInventory[target].m_Type == ITEMGET(12,240) || lpObj->pInventory[target].m_Type == ITEMGET(12,241) || lpObj->pInventory[target].m_Type == ITEMGET(12,242) || lpObj->pInventory[target].m_Type == ITEMGET(12,243) || lpObj->pInventory[target].m_Type == ITEMGET(12,244) || lpObj->pInventory[target].m_Type == ITEMGET(12,245))
	{
		if ((rand() % 100) < btLegendSuccessRate)
		{
			lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type = ITEMGET(12,246),lpObj->pInventory[target].m_Option1,lpObj->pInventory[target].m_Option2,lpObj->pInventory[target].m_Option3,lpObj->pInventory[target].m_NewOption,lpObj->pInventory[target].m_SetOption = 0,lpObj->pInventory[target].m_ItemOptionEx = 0,NULL,0xFF,CURRENT_DB_VERSION);
			LogAddC(4,"[ UpgradeWing ] [Legend Amulet] amulet putting success. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			return true;
		}
		else
		{
			LogAddC(4,"[ UpgradeWing ] [Legend Amulet] amulet putting fail. [%s][%s] - Map:[%d]", lpObj->AccountID, lpObj->Name, lpObj->MapNumber);
			GCServerMsgStringSend("Legend Amulet upgrading fail",lpObj->m_Index,1);
			return true;
		}
	}
	else
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target) // Jewel of Bless
{
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target) // Jewel of Soul
{
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	...
	{
		_r += g_Configs.m_iSoulRate-g_Configs.m_iSoulRateWithLuck;	// -> Here Modify Jewel of Soul on Item With Luck and Without Luck
	}
	...
	{
		if(_r < g_Configs.m_iSoulRate+10)	// -> Here Modified Jewel of Soul on Items Excellent, Ancient or With 380 Option ( < ConfigValue + 10% > ) DEFAULT: 50 + 10 = 60
		{
			bSuccess = true;
		}
	}
	...
	{
		if(g_SocketOption.IsSocketOption(&lpObj->pInventory[target]) == TRUE)
		{
			if(_r < g_Configs.m_iSoulRate)	// -> Here Modified Jewel of Soul on Socket Items DEFAULT: 40
			{
				bSuccess = true;
			}
		}
		else if(_r < g_Configs.m_iSoulRate)	// -> If Item is not: Socket Item, then Restore: Jewel of Soul Success Rate: 60
		{
			bSuccess = true;
		}
	}
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target) // Jewel of Life
{
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
...
#endif
	{
		if(_r < (100 - g_Configs.m_iLifeRate)) // -> Custom Config Jewel of Life Rate
		{
		...
		else
		{
			...
			{
				if(lpObj->pInventory[target].m_Type >= ITEMGET(12,36) && lpObj->pInventory[target].m_Type <= ITEMGET(12,40) || /*lpObj->pInventory[target].m_Type >= ITEMGET(12,240) && lpObj->pInventory[target].m_Type <= ITEMGET(12,255) ||*/lpObj->pInventory[target].m_Type == ITEMGET(12,43)) // -> Here Fix JoL on: 4th Wings
				{
...
BOOL gObjFind10JewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;

	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 240)) // Jewel of Level
			{
				LevelCount++;
...
BOOL gObjFind20JewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
...
BOOL gObjDelete10JewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 240)) // Jewel of Level
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Level (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjDelete20JewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
...
BOOL gObjFind10JewelLuck(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 241)) // Jewel of Luck
			{
				LevelCount++;
...
BOOL gObjDelete10JewelLuck(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 241)) // Jewel of Luck
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Luck (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFind10JewelSkill(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 242)) // Jewel of Skill
			{
				LevelCount++;
...
BOOL gObjDelete10JewelSkill(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 242)) // Jewel of Skill
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Skill (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFind10JewelOption(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 243)) // Jewel of Option
			{
				LevelCount++;
...
BOOL gObjDelete10JewelOption(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 243)) // Jewel of Option
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Option (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFind10JewelExcellent(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 244)) // Jewel of Excellent
			{
				LevelCount++;
...
BOOL gObjDelete10JewelExcellent(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 244)) // Jewel of Excellent
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Excellent (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFind10JewelAncient(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 245)) // Jewel of Ancient
			{
				LevelCount++;
...
BOOL gObjDelete10JewelAncient(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 245)) // Jewel of Ancient
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Ancient (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFind10JewelSocket(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 246)) // Jewel of Socket
			{
				LevelCount++;
...
BOOL gObjDelete10JewelSocket(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int y = 0; y < MAIN_INVENTORY_SIZE; y++)
	{
		if (lpObj->pInventory[y].IsItem() == 1)
		{
			if (lpObj->pInventory[y].m_Type == ITEMGET(14, 246)) // Jewel of Socket
			{
				gObjInventoryDeleteItem(aIndex, y);
				GCInventoryItemDeleteSend(aIndex, y, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Jewel of Socket (%d)", lpObj->AccountID, lpObj->Name, y);
				LevelCount++;
...
BOOL gObjFindBundledJewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,150)) // Bundled Jewel of Level
			{
				LevelCount++;
...
BOOL gObjDeleteBundledJewelLevel(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int LevelCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,150)) // Bundled Jewel of Level
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Level (%d)", lpObj->AccountID, lpObj->Name, n);
				LevelCount++;
...
BOOL gObjFindBundledJewelLuck(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JPLuckCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,151)) // Bundled Jewel of Luck
			{
				JPLuckCount++;
...
BOOL gObjDeleteBundledJewelLuck(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JPLuckCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,151)) // Bundled Jewel of Luck
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Luck (%d)",lpObj->AccountID,lpObj->Name,n);
				JPLuckCount++;
...
BOOL gObjFindBundledJewelSkill(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JSkillCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,152)) // Bundled Jewel of Skill
			{
				JSkillCount++;
...
BOOL gObjDeleteBundledJewelSkill(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JSkillCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,152)) // Bundled Jewel of Skill
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Skill (%d)",lpObj->AccountID,lpObj->Name,n);
				JSkillCount++;
...
BOOL gObjFindBundledJewelOption(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JOptionCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,153)) // Bundled Jewel of Option
			{
				JOptionCount++;
...
BOOL gObjDeleteBundledJewelOption(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JOptionCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,153)) // Bundled Jewel of Option
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Option (%d)",lpObj->AccountID,lpObj->Name,n);
				JOptionCount++;
...
BOOL gObjFindBundledJewelExcellent(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JExcellentCount = 0;

	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,154)) // Bundled Jewel of Excellent
			{
				JExcellentCount++;

				if (JExcellentCount == 1)
				{
					return true;
				}
			}
...
BOOL gObjDeleteBundledJewelExcellent(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JExcellentCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,154)) // Bundled Jewel of Excellent
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Excellent (%d)",lpObj->AccountID,lpObj->Name,n);
				JExcellentCount++;
...
BOOL gObjFindBundledJewelAncient(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JAncientCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,155)) // Bundled Jewel of Ancient
			{
				JAncientCount++;
...
BOOL gObjDeleteBundledJewelAncient(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JAncientCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,155)) // Bundled Jewel of Ancient
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Ancient (%d)",lpObj->AccountID,lpObj->Name,n);
				JAncientCount++;
...
BOOL gObjFindBundledJewelSocket(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JSocketCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,156)) // Bundled Jewel of Socket
			{
				JSocketCount++;
...
BOOL gObjDeleteBundledJewelSocket(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int JSocketCount = 0;
	...
	for (int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(12,156)) // Bundled Jewel of Socket
			{
				gObjInventoryDeleteItem(aIndex, n);
				GCInventoryItemDeleteSend(aIndex, n, 1);
				LogAddTD("[PackSystem] [%s][%s] Delete Bundled Jewel of Socket (%d)",lpObj->AccountID,lpObj->Name,n);
				JSocketCount++;
...
BOOL gObjFind10Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
BOOL gObjDelete10Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
BOOL gObjFind20Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
BOOL gObjDelete20Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
BOOL gObjFind30Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
BOOL gObjDelete30Rena(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	int count = 0;
...
//New CashShop Function (Season3 add-on)
...
void gObjCalcMaxLifePower(int aIndex)	// -> Modified by Nemesis (14-03-2017)
{
	LPOBJ lpObj = &gObj[aIndex];
	// ----
	int iLevelBase = lpObj->Level;
	int iMLevelBase = lpObj->Level + lpObj->MLevel;
	// ----
	switch(lpObj->Class)
	{
		case CLASS_WIZARD:
			if(lpObj->MLevel)
				lpObj->m_MaxLifePower = iMLevelBase + lpObj->Vitality + 100;
			else
				lpObj->m_MaxLifePower = iLevelBase + lpObj->Vitality + 100;
		break;
		case CLASS_KNIGHT:
			if(lpObj->MLevel)
				lpObj->m_MaxLifePower = iMLevelBase + lpObj->Vitality + 100;
...
void gObjNotifyUseWeaponV1(LPOBJ lpOwnerObj, LPOBJ lpWeaponObj, int iTargetX, int iTargetY)
{
	...
	for(int n = 0; n < OBJMAX;n++)
	{
		...
		lpMsgBody[iVp1Count].btViewSkillStateCount = gObjCountAppliedBuffEffect(lpTargetObj, lpMsgBody[iVp1Count].btViewSkillState); //Season3 changed
...
void gObjUseBlessAndSoulPotion(int aIndex, int iItemLevel)
{
	...
	{
		gObjApplyBuffEffectDuration(lpObj, AT_POTION_OF_BLESS, 0, 0, 0, 0, 120);
	}
	else if ( iItemLevel == 1 ) // Soul Potion
	{
		gObjApplyBuffEffectDuration(lpObj, AT_POTION_OF_SOUL, 0, 0, 0, 0, 60);
...
void gObjWeaponDurDownInCastle(LPOBJ lpObj, LPOBJ lpTargetObj, int iDecValue)
{
	...
	if(Left->m_Type >= ITEMGET(4,0) && Left->m_Type < ITEMGET(4,7)
		...
		|| Left->m_Type == ITEMGET(4,23)//season 4.0 add-on
		|| Left->m_Type == ITEMGET(4,24)//season 4.6 add-on
		|| Left->m_Type >= ITEMGET(4,25)//season 4.7 add-on
		&& Left->m_Type <= ITEMGET(4,128))
	{
		bIsLeftDurDown = 1;
	}
	else if(Right->m_Type >= ITEMGET(4,8) && Right->m_Type < ITEMGET(4,15) || Right->m_Type >= ITEMGET(4,16) && Right->m_Type < ITEMGET(5,0))
...
//00554CE0
BOOL gObjCheckIsInPCBang(LPOBJ lpObj) //season4.5 add-on
{
	if(g_bPCBangServer == FALSE)
		return FALSE;
...
void gObjShieldAutoRefill(LPOBJ lpObj)
{
	...
	iRefillPoint += iRefillPoint * lpObj->m_MLPassiveSkill.m_iML_IncreaseSDRecovery / 100; //season4 add-on
...
//identical (modified added Vulcanus)
