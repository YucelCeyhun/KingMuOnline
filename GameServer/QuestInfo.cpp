			...
			if ( Token == 1 )
			{
				...
				if ( Type == 0 )
				{
					...
					while ( true )
					{
						...
						Token = GetToken();
						this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[5] = TokenNumber;
...
BOOL CQuestInfo::QuestClear(LPOBJ lpObj, int QuestIndex)
{
	...
	for ( int subquest=0;subquest<subquestcount;subquest++)
	{
		...
		if ( lpSubInfo != NULL )
		{
			...
			else if ( lpSubInfo->RewardType == QUEST_COMPENSATION_CHANGEUP3RD ) //season 2.5 add-on
			{
				lpObj->ChangeUP3rd = 1;
				lpObj->DbClass |= 2;
				gObjMakePreviewCharSet(lpObj->m_Index);
				BYTE btClass = (lpObj->Class<<5)&0xE0;
				btClass |= (lpObj->ChangeUP<<4)&0x10;
				btClass |= (lpObj->ChangeUP3rd<<0x03)&0x08;
				GCSendQuestPrize(lpObj->m_Index, QUEST_COMPENSATION_CHANGEUP3RD, btClass );
...
{	
	...
	if(QuestIndex == 6)
	{
		lpObj->ChangeUP3rd = true;
		lpObj->DbClass |= 2;
		g_MasterLevelSystem.GDRequestMasterLevelInfoLoad(lpObj); //season 4.5 add-on
	}
