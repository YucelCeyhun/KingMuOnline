...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL) //0054B2A0
{
	...
	LogAddTD("[Blood Castle] ≈ı∏Ì∏¡≈‰ Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);

	char szTemp[17];
	wsprintf(szTemp, "≈ı∏Ì∏¡≈‰ Mix,%d", iLEVEL); //Season 2.5 add-on
	...
	if ( iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100 )
	{
		...
		LogAddTD("[Blood Castle] ≈ı∏Ì∏¡≈‰ Mix Chaos Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
		return false;
	}
	...
	{
		DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		LogAddTD("[Blood Castle] ≈ı∏Ì∏¡≈‰ Mix Chaos Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
		return false;
	}
	...
	if ( (gObj[iIndex].Money - iMIX_NEED_MONEY) < 0 )
	{
		...
		DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		LogAddTD("[Blood Castle] ≈ı∏Ì∏¡≈‰ Mix Chaos Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
		return false;
	}
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )
	{
		...
		LogAddTD("[≈ı∏Ì∏¡≈‰ Mix] [%s][%s] CBMix Success %d Money : %d-%d", gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
	}
	else
	{
		...
		DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		LogAddTD("[≈ı∏Ì∏¡≈‰ Mix] [%s][%s] CBMix Fail %d Money : %d-%d", gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
		return false;
	}
...
void CBloodCastle::SetSaintStatue(int iBridgeIndex) //0054D1E0
{
	...
	if ( nSaintStatuePosNum != -1 )
	{
		...
		{
			LogAddTD("[Blood Castle][Bug Tracer] (%d) It's not Saint Statue( Map:%d, BloodCastleIndex:%d, Type:%d )", iBridgeIndex+1, btMapNumber, this->GetBridgeIndex(btMapNumber), wMonsterType); //season3 changed
		}
	}
...
int CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	if(g_MasterLevelSystem.MasterLevelUp(&gObj[iIndex], iAddExp) != 0) //season3 add-on
	{
		return 0;
	}
...
int CBloodCastle::CalcSendRewardEXP(int iIndex, int iEXP) //00552510
{
	...
	if ( gObj[iIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				gObjSealUserSetExp(&gObj[iIndex], iCAL_EXP, TRUE); //Seal Exp
		...
		if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(&gObj[iIndex]) == FALSE) //season3 add-on
		{
			GCKillPlayerExtSend(iIndex, (WORD)-1, iRET_EXP, 0, 0);
		}
	}
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex) //005535E0
{
	...
	if ( bRET_VAL == false )
	{
		int result = gObjAddMonster(this->GetBridgeMapNumber(iBridgeIndex)); //season3 changed

		if ( result >= 0 )
		{
			gObjSetPosMonster(result, this->m_BridgeData[iBridgeIndex].m_iAngelKingPosNum);
			gObjSetMonster(result, 232);
			gObj[result].m_cBloodCastleIndex = this->GetBridgeIndex(gObj[result].MapNumber);
			bRET_VAL = true;
		}
		else
		{
...
BOOL CBloodCastle::DropItemDirectly(int iBridgeIndex, int iIndex, int iItemType, int iItemPos) //0059CA30
{
	...
	g_SocketOption.GetActiveSocketOption(&gObj[iIndex].pInventory[iItemPos],SocketOption,SocketIndex);
