...
BOOL CKalimaGate::CreateKalimaGate(int iIndex, BYTE btLevel, BYTE cTX, BYTE cTY)
{
	...
	__try
	{
		...
		if ( IT_MAP_RANGE(gObj[iIndex].MapNumber) )//4.5
		{
			return false;
		}
...
BOOL CKalimaGate::DeleteKalimaGate(int iKalimaGateIndex, int iCallOwnerIndex)
{
	...
	__try
	{
		...
		{
			LogAddTD("[Kalima] DeleteKalimaGate() - out of Index (iKalimaGateIndex:%d, iCallOwnerIndex:%d)", iKalimaGateIndex, iCallOwnerIndex);
			return 0;
		}
...
BOOL CKalimaGate::DeleteKalimaGate(int iCallOwnerIndex)
{
	...
	{	
		LogAddTD("[Kalima] DeleteKalimaGate() - out of Index (iCallOwnerIndex:%d)",	iCallOwnerIndex);
		return false;
	}
...
{	
	...
	__try
	{
		...
		LogAddTD("[Kalima] [%s][%s] Try to Create Kalima Gate (NextMap)", gObj[iIndex].AccountID, gObj[iIndex].Name);

		if ( iMonMapNumber < MAP_INDEX_KALIMA1 || iMonMapNumber > MAP_INDEX_KALIMA6-1 )
		{
			LogAddTD("[Kalima] [%s][%s] Failed to Summon Kalima Gate (NextMap) - MapNumber Error : %d",	gObj[iIndex].AccountID, gObj[iIndex].Name, iMonMapNumber);
		...
		if ( gObj[iIndex].m_cKalimaGateExist == TRUE )
		{
			...
			LogAddTD("[Kalima] [%s][%s] Failed to Summon Kalima Gate (NextMap) - Already Have Gate (SummonIndex:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].m_iKalimaGateIndex);
		...
		if ( btMapAttr != 0 )
		{
			...
			{
				LogAddTD("[Kalima] [%s][%s] Failed to Summon Kalima Gate (NextMap) - Failed to get Location (MapNumber:%d, X:%d, Y:%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iMonMapNumber, cX, cY);
		...
		if ( iMonsterIndex >= 0 )
		{
			...
			if ( !lpMATTR )
			{
				...
				LogAddTD("[Kalima] [%s][%s] Kalima Gate Vanished (NextMap) - lpMATTR == NULL (SummonIndex:%d)",	gObj[iIndex].AccountID, gObj[iIndex].Name, iMonsterIndex);
