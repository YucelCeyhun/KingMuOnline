...
void Log(BYTE type, char* msg, ...)
{
	...
	{
		wsprintf(tmpbuf, "[%.8s][ERROR] ", asctime(today) + 11);
		break;
	}
	case LOG_LEVEL_WARNING:
	{
		wsprintf(tmpbuf, "[%.8s][WARNING] ", asctime(today) + 11);
		break;
	}
	case LOG_LEVEL_NOTIFY:
	{
		wsprintf(tmpbuf, "[%.8s][NOTIFY] ", asctime(today) + 11);
		break;
	}
	case LOG_LEVEL_INFO:
	{
		wsprintf(tmpbuf, "[%.8s][INFO] ", asctime(today) + 11);
		break;
	}
	default:
	{
		wsprintf(tmpbuf, "[%.8s][DEBUG] ", asctime(today) + 11);
		break;
	}
	}
	...
	LogTextAdd(type, tmpbuf, strlen(tmpbuf));
