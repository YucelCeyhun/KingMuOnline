...
#if(USE_RESETSYS > 0)
    bool bRet = gDataBase.ExecFormat("SELECT AccountID, cLevel, Class, LevelUpPoint, Experience, Strength, Dexterity, Vitality, Energy, Money, Life, MaxLife, Mana, MaxMana, MapNumber, MapPosX, MapPosY, MapDir, PkCount, PkLevel, PkTime, CtlCode, DbVersion, Leadership, ChatLimitTime, FruitPoint, Reset, GrandReset FROM Character WHERE Name = '%s'",szName);
#else
...
#if(FIX_MAXSTAT == 1)
		pResult.Str								= (WORD)gDataBase.GetInt("Strength");
		pResult.Dex								= (WORD)gDataBase.GetInt("Dexterity");
		pResult.Vit								= (WORD)gDataBase.GetInt("Vitality");
		pResult.Energy							= (WORD)gDataBase.GetInt("Energy");
#else
...
#if(USE_RESETSYS > 0)
		pResult.Reset							= gDataBase.GetInt("Reset");
		pResult.GrandReset						= gDataBase.GetInt("GrandReset");
#endif
#if(SEASON > 5)
...
void DataServer_Protocol::DGMoveOtherServer(DataServer_Manager * Service, int aIndex, SDHP_CHARACTER_TRANSFER * aRecv)
{
    ...
    pResult.Number = aRecv->Number;
	// ----
    Service->DataSend(aIndex,(BYTE*)&pResult,pResult.h.Size);
}
