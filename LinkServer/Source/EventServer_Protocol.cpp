...
void EventServer_Protocol::ProtocolCore(int aIndex, BYTE HeadCode, LPBYTE aRecv, int iSize)
{
    ...
    case 0x29: // [OK] - [FINE]
    {
        this->RequestNewPCBangInfo(aIndex, reinterpret_cast<PMSG_REQ_PCBANGUSER_INFO *>(aRecv));
    }
    break;

    case 0x30: // [OK] - [FINE]
    {
        this->ResultNewPCBangInfo(aIndex, reinterpret_cast<PMSG_PCBANGUSER_UPDINFO *>(aRecv));
    }
    break;
...
void EventServer_Protocol::RequestPCBangInfo(int aIndex, PMSG_REQ_PCBANG_POINT_INFO * lpMsg)
{
    ...
    if(who)
    {
        if (gDataBase.Fetch() != SQL_NO_DATA)
        {
            tMsg.nPoint = gDataBase.GetInt("Result");
        }
        else
        {
            tMsg.nPoint = -2;
...
void EventServer_Protocol::RequestNewPCBangInfo(int aIndex,PMSG_REQ_PCBANGUSER_INFO * lpMsg)
{
	PMSG_PCBANGUSER_RECVINFO tMsg		= {0};
    tMsg.h.c							= 0xC1;
    tMsg.h.headcode						= 0x29;
    tMsg.h.Size							= sizeof(PMSG_PCBANGUSER_RECVINFO);
    tMsg.aIndex							= lpMsg->aIndex;
	// ----
    strcpy(tMsg.szAccountID,lpMsg->szAccountID);
	// ----
    if(gDataBase.ExecFormat("EXEC SP_REQ_PCBANG_POINT_INFO2 %d, '%s'",lpMsg->btServerType,lpMsg->szAccountID))
    {
        if(gDataBase.Fetch() != SQL_NO_DATA)
        {
            tMsg.nPoint			= gDataBase.GetInt("Result");
            tMsg.nResetYear		= (short)gDataBase.GetInt("ResetYear");
            tMsg.nResetMonth	= (short)gDataBase.GetInt("ResetMonth");
            tMsg.nResetDay		= (short)gDataBase.GetInt("ResetDay");
            tMsg.nResetHour		= (short)gDataBase.GetInt("ResetHour");
        }
        else
        {
            tMsg.nPoint = 2;
...
void EventServer_Protocol::ResultNewPCBangInfo(int aIndex,PMSG_PCBANGUSER_UPDINFO * lpMsg)
{
	PMSG_PCBANGUSER_SENDINFO tMsg			= {0};
    tMsg.h.c								= 0xC1;
    tMsg.h.headcode							= 0x30;
    tMsg.h.Size								= sizeof(PMSG_PCBANGUSER_SENDINFO);
    tMsg.aIndex								= lpMsg->aIndex;
	// ----
    strcpy(tMsg.szAccountID,lpMsg->szAccountID);
	// ----
    if(gDataBase.ExecFormat("EXEC SP_REQ_PCBANG_POINT_UPDATE2 %d, '%s', %d, '%04d-%02d-%02d %02d:00:00'",lpMsg->btServerType,lpMsg->szAccountID,lpMsg->nNewPoint,lpMsg->nResetYear,lpMsg->nResetMonth,lpMsg->nResetDay,lpMsg->nResetHour))
    {
        if(gDataBase.Fetch() != SQL_NO_DATA)
        {
            tMsg.btResult = (BYTE)gDataBase.GetInt("Result");
