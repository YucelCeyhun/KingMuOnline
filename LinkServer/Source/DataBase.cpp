...
short DataBase::GetShort(short sCol)
{
	int sReturn = CQUERYNOCOL;

	if(sCol == m_nCol)
	{
		if(m_lCol[sCol] != SQL_NULL_DATA)
			sReturn = atoi(m_Col[sCol]);
		else
			sReturn = CQUERYNULL;
	}
...
short DataBase::GetShort(const char *sCol)
{
	short sReturn = CQUERYNOCOL;
	short s = FindCol(sCol);
