...
short CMapServerManager::CheckMoveMapSvr(WORD wServerCode, int iIndex, int iMAP_NUM, short sSVR_CODE_BEFORE)
{
	...
	switch ( sMAP_MOVE_INFO )
	{
		...
		case -3:
			LogAddTD("[MapServerMng] CheckMoveMapSvr() - MapServer Check OK [%s][%s] : MAP-%d / SVR-%d (State Map:%d X:%d Y:%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, iMAP_NUM, wServerCode,
				gObj[iIndex].MapNumber, gObj[iIndex].X, gObj[iIndex].Y);
