...
BOOL g_bCanCreateDeleteGuild = (BOOL)GetPrivateProfileInt("CreateDeleteGuild","CONFIG",0,CONFIG_INI);
...
void EDSProtocolCore(int aIndex, BYTE HeadCode, LPBYTE aRecv, int iSize)
{
...
#endif
	gDisplay.CheckEXDSConnection(TRUE);
	switch ( HeadCode )
	{
		case 0x00:
	...
	gDisplay.CheckEXDSConnection(FALSE);
}
...
void GDGuildCreateSend(int aIndex, SDHP_GUILDCREATE * aRecv)
{
	...
	if ( SpaceSyntexCheck(szMaster) != FALSE )
	{
		if ( SQLSyntexCheck(szMaster) != FALSE )
		{
			...
			{
				g_MuOnlineDB.ExecQuery("SELECT Name FROM GuildMember WHERE Name='%s'",
					szMaster);
				...
				if ( g_MuOnlineDB.Fetch() == SQL_NO_DATA )
				{
					...
					if ( g_MuOnlineDB.ExecQuery("WZ_GuildCreate '%s', '%s'", szGuild, szMaster) == TRUE )
					{
						if ( g_MuOnlineDB.Fetch() != SQL_NO_DATA )
						{
							if ( g_MuOnlineDB.GetResult(0) == 0 )
							{
								g_MuOnlineDB.Close();
								char szMsg[128];
					...
					g_MuOnlineDB.ExecQuery("SELECT Number FROM Guild WHERE G_Name='%s'", szMaster);
					g_MuOnlineDB.Fetch();
...
//						pGuildInfo.m_GuildStatus[i] = 0x00;
					...
					g_MuOnlineDB.ExecQuery("UPDATE Guild SET G_Union=%d WHERE Number=%d", 0, pResult.GuildNumber);
					g_MuOnlineDB.ExecQuery("UPDATE GuildMember SET G_Status	=%d WHERE Name='%s'", 0x80, szMaster);
					g_MuOnlineDB.ExecQuery("UPDATE Guild SET G_Type=%d WHERE G_Name='%s'", aRecv->btGuildType, szGuild);
					pResult.Result = 1;
...
void GDGuildDestroyRecv(int aIndex, SDHP_GUILDDESTROY * aRecv)
{
	...
	if ( pResult.Result != 3 )
	{
		...
		g_MuOnlineDB.ExecQuery("WZ_CS_CheckSiegeGuildList '%s'", szGuildName);
		if ( g_MuOnlineDB.Fetch() != SQL_NO_DATA )
		...
		if ( pResult.Result != 3 )
		{
			g_MuOnlineDB.ExecQuery("DELETE FROM GuildMember WHERE G_Name='%s'", szGuildName);
			g_MuOnlineDB.Close();
			g_MuOnlineDB.ExecQuery("DELETE FROM Guild WHERE G_Name='%s'", szGuildName);
			g_MuOnlineDB.Close();
			pResult.Result = 4;
		}
	}
...
void GDGuildMemberAdd(int aIndex, SDHP_GUILDMEMBERADD * aRecv)
{
	...
	if ( g_MuOnlineDB.ExecQuery("SELECT Name FROM GuildMember WHERE Name='%s'", szGuildMember) == TRUE )
	{
		if ( g_MuOnlineDB.Fetch() != SQL_NO_DATA )
		{
			g_MuOnlineDB.Close();
	...
	// Add Member
	if ( g_MuOnlineDB.ExecQuery("INSERT INTO GuildMember (Name, G_Name) VALUES ('%s', '%s')", szGuildMember, szGuildName) == FALSE )
	{
		g_MuOnlineDB.Close();
		pResult.Result = 4;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
...
void GDGuildMemberDel(int aIndex, SDHP_GUILDMEMBERDEL * aRecv)
{
	...
	// Delete FROM Guild
	if ( g_MuOnlineDB.ExecQuery("DELETE FROM GuildMember WHERE Name='%s'", szGuildMember) == FALSE)
	{
		g_MuOnlineDB.Close();
		pResult.Result = 3;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
...
void DGGuildMemberInfoRequest(int aIndex, SDHP_GUILDMEMBER_INFO_REQUEST * aRecv)
{
	...
	if ( g_MuOnlineDB.ExecQuery("SELECT G_Name, G_Level, G_Status FROM GuildMember WHERE Name='%s'", szGuildMember) != TRUE )
	{
		g_MuOnlineDB.Close();
		return;
	}
	...
	g_MuOnlineDB.GetAsString("G_Name", pResult.GuildName);
	...
	if ( true )
	{
		...
		{
			g_MuOnlineDB.ExecQuery("SELECT G_Name FROM Guild WHERE G_Rival=%d", pGuildInfo.m_iRivalGuild);
			
			if ( g_MuOnlineDB.Fetch() != SQL_NO_DATA )
			{
				g_MuOnlineDB.GetAsString("G_Name", pListCount->szGuildRivalName);
			}
...
void DGGuildScoreUpdate(int aIndex, SDHP_GUILDSCOREUPDATE * aRecv)
{
	...
	if ( g_MuOnlineDB.ExecQuery("UPDATE Guild SET G_Score=%d WHERE G_Name='%s'", aRecv->Score, szGuildName) == TRUE )
	{
...
void GDGuildReqAssignStatus(int aIndex, EXSDHP_GUILD_ASSIGN_STATUS_REQ * aRecv)
{
	...
	if ( g_MuOnlineDB.ExecQuery("UPDATE GuildMember SET G_Status=%d WHERE Name='%s'",
...
void GDGuildReqAssignType(int aIndex, EXSDHP_GUILD_ASSIGN_TYPE_REQ * aRecv)
{
	...
	if ( g_MuOnlineDB.ExecQuery("UPDATE GuildMember SET G_Type=%d WHERE G_Name='%s'",
