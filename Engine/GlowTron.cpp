...
int IsGlt = GetPrivateProfileInt("Engine","UseCustomGlowTron",0,ENGINE_INI);
...
void InitScreenGlow(void)
{
	...
	// VP LOADING
	if(!LoadProgram(&g_cgVP_GlowDarken,g_cgVertProfile,CG_DARKEN_VP))
		return;

	if(!LoadProgram(&g_cgVP_GlowBlur,g_cgVertProfile,CG_GLOWBLUR_VP))
		return;

	if(!LoadProgram(&g_cgVP_GlowCombine,g_cgVertProfile,CG_GLOWCOMBINE_VP))
		return;
	...
	// FP LOADING
	if(!LoadProgram(&g_cgFP_GlowDarken,g_cgFragProfile,CG_DARKEN_FP))
		return;

	if(!LoadProgram(&g_cgFP_GlowBlur,g_cgFragProfile,CG_GLOWBLUR_FP))
		return;

	if(!LoadProgram(&g_cgFP_GlowCombine,g_cgFragProfile,CG_GLOWCOMBINE_FP))
		return;
}
...
void cGlowTron::SwitchState()
{
	if(IsGlt == GlowTronOn && pPlayerState == GameProcess)
	{
		if(g_bEnabled)
		{
			g_bEnabled = false;
			gGlowTron.Enabled = false;
