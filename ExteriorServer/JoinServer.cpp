// JoinServer.cpp: implementation of the JoinServer class.
//////////////////////////////////////////////////////////////////////
#include "StdAfx.h"
...
void JoinServerInit()
{
	...
	pMd5.MD5_EncodeKeyVal("2222",(char *)pass,0x6E);


	if(GetPrivateProfileInt("ExteriorServer","UseJoinServer",1,nConfig) != TRUE)
	{
		return;
	}
	else if (GetPrivateProfileInt("ExteriorServer","UseJoinServer",1,nConfig)==TRUE && CreateGIocp(g_JoinServerListPort, ST_JOINSERVER) == TRUE )
	{
		LogAddTD("[IOCP] JOINSERVER Start! PORT [ %d ]", g_JoinServerListPort);
	}
	//Sleep(100);
	...
	{
		LogAddC(4, "[JoinServer] Connection Successfull!");
	}
	else
	{
		LogAddC(2, "[JoinServer] Error On Connection!");
	}
...
{
	g_Displayer.CheckJSConnection(TRUE);
#if (TRACE_PACKET == 1 )
	...
	g_Displayer.CheckJSConnection(FALSE);
}
...
BOOL MuLoginAddServer(LPTSTR ServerName, WORD ServerCode, WORD Port)
{
	...
	{
		LogAddC(2, "[JoinServer] GameServer %s trying to connect with a ServerCode %d already used",
...
void MuLoginDeleteServer(WORD ServerCode)
{
	for(int i=0;i<MAX_LOGIN_SERVER;i++)
	{
		if ( g_MuLoginServerData[i].m_wServerCode == ServerCode )
		{
			...
			{
				if(g_MuLoginUserData[n].m_ServerCode == ServerCode)
				{
					memset(g_MuLoginUserData[n].m_AccoundID,0x00,sizeof(g_MuLoginUserData[n].m_AccoundID));
					g_MuLoginUserData[n].iUserNumber = -1;
					g_MuLoginUserData[n].m_bRequestMapSvrMove = FALSE;
					g_MuLoginUserData[n].m_ServerCode = -1;
				}
			}
			break;
		}
...
void JoinServerLogin(int aIndex, SDHP_SERVERINFO * lpMsg)
{
	...
	{
		LogAddTD("[Server Manager] %s PORT: %d CODE: %d GameServer Connect",lpMsg->ServerName, lpMsg->Port, lpMsg->ServerCode);
		//LogAddTD("[%s][%s:%d][%d] GameServer Connect",lpMsg->ServerName, Ip,lpMsg->Port,lpMsg->ServerCode);
	...
	for(int i=0;i<MAX_LOGIN_USER;i++)
	{
		if ( g_MuLoginUserData[i].m_AccoundID[0] == 0 )
		{
			if ( g_MuLoginUserData[i].m_AccoundID[1] == 0 )
			{
				...
				g_MuLoginUserData[i].m_AccoundID[10] = 0;
...
void JGPAccountRequest(int aIndex, SDHP_IDPASS * aRecv)
{
	...
	pResult.PcBangGuid = 0;
	pResult.bUserInPCBang = 1;
		...
		if ( bErrorFlag == FALSE )
		{
			if (g_MeMuOnlineDB.ExecQuery("SELECT * FROM MEMB_INFO WHERE memb___id='%s'", szAccountID) == TRUE )
			{
				...
				if ( /*Result == SQL_SUCCESS ||*/ Result == SQL_NO_DATA )
				{
					...
					if(g_MeMuOnlineDB.ExecQuery("EXEC EDS_AUTO_REG '%s', '%s', '%s', %d",szAccountID,szPass,aRecv->IpAddress,g_UseMD5) == TRUE && g_MeMuOnlineDB.Fetch() != SQL_NO_DATA && g_MeMuOnlineDB.GetAsInteger("Result") == TRUE)
					{
						g_MeMuOnlineDB.Close();

						if(g_MeMuOnlineDB.ExecQuery("SELECT * FROM MEMB_INFO WHERE memb___id='%s'", szAccountID) != TRUE || g_MeMuOnlineDB.Fetch() == SQL_NO_DATA)
						{
							pResult.result = 2;
							bErrorFlag = TRUE;
				...
				if(bErrorFlag == FALSE)
				{
					...
					g_MeMuOnlineDB.GetAsString("sno__numb", szJoominNumber,sizeof(szJoominNumber)-1);
					...
					if ( g_UseMD5 == TRUE )
					{
						...
						if ( g_MeMuOnlineDB.GetAsBinary(szTemp, btBinaryPass) < 0 )
						{
							bErrorFlag = TRUE;
							pResult.result = 0;
							LogAddTD("[JoinServer] Password field is blank - ID : %s", szAccountID);
						}
						...
						if ( pMD5Hash.MD5_CheckValue(szPass, (char*)btBinaryPass, dwAccKey) == false )
						{
							pResult.result = 0;
							bErrorFlag = TRUE;
							LogAddTD("[JoinServer] Wrong Password - ID : %s", szAccountID);
						}
					}
					else
					{
						TCHAR szPassword[11];
						g_MeMuOnlineDB.GetAsString("memb__pwd", szPassword,sizeof(szPassword)-1);
...
//#error There is a return -2 
			...
			else
			{
				...
				g_MeMuOnlineDB.ExecQuery("EXEC WZ_CONNECT_MEMB '%s','%s','%s'", szAccountID, g_MuLoginServerData[MuLoginFindServer(g_Server[aIndex].m_ServerCode)].m_szServerName, szIp);
...
void LoveHeartEventRecv(int aIndex, SDHP_LOVEHEARTEVENT * aRecv)
{
	...
	g_MeMuOnlineDB.ExecQuery("INSERT INTO LoveHeartAll ( Number, Id, Server, Name ) Values (%d, '%s','%s','%s')",
		dwHeartCount, szAccountID, g_MuLoginServerData[g_Server[aIndex].m_ServerCode].m_szServerName, szName);
	g_MeMuOnlineDB.Fetch();
	g_MeMuOnlineDB.Close();
	...
	if ( iIndex >= 0 )	// if Is Connected
	{
		...
		{
			g_MeMuOnlineDB.ExecQuery("INSERT INTO LoveHeart ( Number, Id, Server, Name ) Values (%d, '%s','%s','%s')",
				dwHeartCount, szAccountID, g_MuLoginServerData[g_Server[aIndex].m_ServerCode].m_szServerName, szName);
			g_MeMuOnlineDB.Fetch();
			g_MeMuOnlineDB.Close();
			pResult.Result = 1;

			LogAddC(2, "[JoinServer] [LOVE HEART] Event Winner : %s:%s - Server : %s",
				szAccountID, szName, g_MuLoginServerData[g_Server[aIndex].m_ServerCode].m_szServerName);
		}
	}	
...
void GJReqMapSvrMove(int aIndex, PMSG_REQ_MAPSVRMOVE * aRecv)
{
	...
	strcpy(szAccountID, aRecv->szAccountID);
	...
	{
		fResult = 2;
		LogAddTD("[JoinServer] GJReqMapSvrMove() -> ServerIndex = -1 ( cur:%d | dest: %d) Account:(%s)",aRecv->wCurMapSvrCode,aRecv->wDstMapSvrCode,aRecv->szAccountID);
	}
	else
	{
		// Limit User reached
		if ( g_MuLoginServerData[iNewServerIndex].m_wCurrentUser >= g_MuLoginServerData[iNewServerIndex].m_wMaxUser )
		{
			fResult = 3;
			LogAddTD("[JoinServer] GJReqMapSvrMove -> Server(%d) UserLimit reached (cur:%d | max:%d)",aRecv->wDstMapSvrCode,g_MuLoginServerData[iNewServerIndex].m_wCurrentUser,g_MuLoginServerData[iNewServerIndex].m_wMaxUser);
		}
		else
		{
			...
			{
				fResult = 1;
				LogAddTD("[JoinServer] GJReqMapSvrMove -> iUserIndex == -1 (%s)(%s)",aRecv->szAccountID,aRecv->szCharName);
			}
			else
			{
				// User Requested the Move
				if ( g_MuLoginUserData[iUserIndex].m_bRequestMapSvrMove != false )
				{
					fResult = 4;
					LogAddTD("[JoinServer] GJReqMapSvrMove -> m_bRequestMapSvrMove == true (%s)(%s)",aRecv->szAccountID,aRecv->szCharName);
				}
				else
				{
					...
					LogAddTD("[JoinServer] GJReqMapSvrMove -> fResult == SUCCESS, (%s)(%s) dest:(%d) map(%d)",
...
void GJReqMapSvrAuth(int aIndex, PMSG_REQ_MAPSVRAUTH * aRecv)
{
	...
	pResult.iResult = fResult;
	pResult.iPcBangGuid = 0;
	pResult.bPcBangUser = 1;

	DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);

	LogAddTD("[JoinServer] GJReqMapSvrAuth() -> User(%s) Character(%s) fResult(%d)",
