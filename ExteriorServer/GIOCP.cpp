...
BOOL CreateListenSocket(DWORD dwServerCountIndex)
{
	...
	if ( nRet == -1 )
	{
		MsgBox("bind error : nExtraServer can't be launched twice");
...
unsigned long __stdcall ServerWorkerThread(HANDLE CompletionPortID)
{
	...
	while ( true )
	{
		...
		if ( dwIoSize == 0 )
		{
			...
			int criticalSection = g_Server[ClientIndex].m_Type;
			CloseClient(lpPerSocketContext, 0);
			//LeaveCriticalSection(&criti[dwServerCountIndex]);
			LeaveCriticalSection(&criti[criticalSection]);
			continue;
		}
...
BOOL RecvDataParse(_PER_IO_CONTEXT * lpIOContext, int uIndex)	
{
	...
	while ( true )
	{
		...
		{
			g_Server[uIndex].ProtocolCore(uIndex, headcode, &recvbuf[lOfs], size);
